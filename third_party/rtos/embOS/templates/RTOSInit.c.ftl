...
#ifndef OS_TICK_FREQ
    #define OS_TICK_FREQ  ( ${CONFIG_EMBOS_OS_TICK_FREQ} )              /* Tick frequency             */
#endif
...
*/
    ...
    #define OS_VIEW_ENABLE    (<#if CONFIG_EMBOS_OS_VIEW_ENABLE == true>1<#else>0</#if>)     // Global enable of communication
#endif
...
<#else>
  IPC0SET |= (SYSTICK_PRIORITY << 2) | SYSTICK_SUBPRIORITY;        // Set priority
  IEC0SET |= (1 << CTIE);                                          // Enable core time interrupt in interrupt vector controller
</#if>
  //
