...
** emWin V4.08 - Graphical user interface for embedded applications **
...
#if (SHOW_GUIDEMO_ICONVIEW && GUI_WINSUPPORT)
...
static GUI_CONST_STORAGE GUI_BITMAP _bmWrite = {
  48, /* XSize */
  48, /* YSize */
  192, /* BytesPerLine */
  32, /* BitsPerPixel */
...
static const BITMAP_ITEM _aBitmapItem[] = {
  ...
  { &_bmEmail,    "E-Mail",   "Read an e-mail"},
  { &_bmSystem,   "System",   "Change system settings"},
  { &_bmRead,     "Read",     "Read a document"},
  { &_bmWrite,    "Write",    "Write an e-mail"},
  { &_bmPassword, "Password", "Determine the system password"}
};

static const ALIGNMENT_ITEM _aAlignment[] = {
  { GUI_TA_RIGHT   | GUI_TA_BOTTOM,  ICONVIEW_IA_HCENTER | ICONVIEW_IA_TOP    },
  { GUI_TA_HCENTER | GUI_TA_BOTTOM,  ICONVIEW_IA_HCENTER | ICONVIEW_IA_TOP    },
  { GUI_TA_LEFT    | GUI_TA_BOTTOM,  ICONVIEW_IA_HCENTER | ICONVIEW_IA_TOP    },
  { GUI_TA_HCENTER | GUI_TA_BOTTOM,  ICONVIEW_IA_HCENTER | ICONVIEW_IA_TOP    },
  { GUI_TA_RIGHT   | GUI_TA_TOP,     ICONVIEW_IA_HCENTER | ICONVIEW_IA_BOTTOM },
  { GUI_TA_HCENTER | GUI_TA_TOP,     ICONVIEW_IA_HCENTER | ICONVIEW_IA_BOTTOM },
  { GUI_TA_LEFT    | GUI_TA_TOP,     ICONVIEW_IA_HCENTER | ICONVIEW_IA_BOTTOM },
  { GUI_TA_HCENTER | GUI_TA_TOP,     ICONVIEW_IA_HCENTER | ICONVIEW_IA_BOTTOM },
};
...
void GUIDEMO_IconView(void) {
  ...
  yOff = ySize - YSIZE_MIN;
  //
  // Create TEXT widget to display explanation text.
  //
  Sel   = 0;
  hText = TEXT_CreateEx(CONTROL_SIZE_X, ySize - CONTROL_SIZE_Y - 1, xSize - CONTROL_SIZE_X * 2, CONTROL_SIZE_Y, WM_HBKWIN, WM_CF_SHOW, 0, 0, _aBitmapItem[Sel].pExplanation);
  TEXT_SetTextAlign(hText, GUI_TA_VCENTER | GUI_TA_HCENTER);
  TEXT_SetTextColor(hText, GUI_WHITE);
  TEXT_SetWrapMode(hText, GUI_WRAPMODE_WORD);
  TEXT_SetFont(hText, &GUI_FontRounded16);
  //
  // Create ICONVIEW widget.
  //
  TitleSize = GUIDEMO_GetTitleSizeY();
  hIcon     = ICONVIEW_CreateEx(20 + xOff / 2, TitleSize + yOff / 2, xSize - 41 - xOff, ySize - CONTROL_SIZE_Y - TitleSize - yOff, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0, GUI_ID_ICONVIEW0, 55, 60);
  for (i = 0; i < GUI_COUNTOF(_aBitmapItem); i++) {
    ICONVIEW_AddBitmapItem(hIcon, _aBitmapItem[i].pBitmap, _aBitmapItem[i].pText);
  }
  ICONVIEW_SetBkColor(hIcon, ICONVIEW_CI_SEL, 0x880000FF);
  ...
  // Change selection.
  //
  GUIDEMO_SetInfoText("Change selection.");
  GUIDEMO_CheckCancelDelay(1000);
  for (Sel = 0; Sel < GUI_COUNTOF(_aBitmapItem); Sel++) {
    ICONVIEW_SetSel(hIcon, Sel);
    TEXT_SetText(hText, _aBitmapItem[Sel].pExplanation);
    if (GUIDEMO_CheckCancelDelay(Sel == GUI_COUNTOF(_aBitmapItem) - 1 ? 2000 : 600)) {
      break;
    }
  }
  //
  // Change icon and text alignment.
  //
  ICONVIEW_SetTextColor(hIcon, ICONVIEW_CI_SEL,   GUI_YELLOW);
  ICONVIEW_SetTextColor(hIcon, ICONVIEW_CI_UNSEL, GUI_YELLOW);
  GUIDEMO_SetInfoText("Change icon- and text alignment.");
  GUIDEMO_CheckCancelDelay(1000);
  TEXT_SetText(hText, "");
  for (Sel = 0; Sel < GUI_COUNTOF(_aAlignment); Sel++) {
    ICONVIEW_SetTextAlign(hIcon, _aAlignment[Sel].TextAlign);
    ICONVIEW_SetIconAlign(hIcon, _aAlignment[Sel].IconAlign);
    if (GUIDEMO_CheckCancelDelay(Sel == GUI_COUNTOF(_aBitmapItem) - 1 ? 2000 : 600)) {
      break;
    }
  }
  GUIDEMO_CheckCancelDelay(500);
  //
  // Clean up
  //
...
void GUIDEMO_IconView_C(void);
void GUIDEMO_IconView_C(void) {}

#endif  // SHOW_GUIDEMO_ICONVIEW && GUI_WINSUPPORT
