...
static void _RadialMenu(void) {
  ...
  //
  hDraw    = WM_CreateWindowAsChild(10, 20, 300, 180, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, _cbDraw, sizeof(pPara));
  
  xSizeWin        = WM_GetWindowSizeX(hDraw);
  ySizeWin        = WM_GetWindowSizeY(hDraw);
  ...
  for (i = 0; i < 5; i++) {
      ...
      return;
        ...
        GUIDEMO_CheckCancelDelay(500);
  //
        // Turn left (accelerating and decelerating)
        //
        for (i = 0; (i <= 500) ; i+= 50) {
          WM_MOTION_SetSpeed(hMotion, GUI_COORD_X, i);
          GUIDEMO_CheckCancelDelay(100);
        }
        GUIDEMO_CheckCancelDelay(2000);
        WM_MOTION_SetDeceleration(hMotion, GUI_COORD_X, i);
        GUIDEMO_CheckCancelDelay(2000);
        //
        // Turn right (accelerating and decelerating)
        //
        for (i = 0; (i <= 500) ; i+= 50) {
          WM_MOTION_SetSpeed(hMotion, GUI_COORD_X, -i);
          GUIDEMO_CheckCancelDelay(100);
        ...
        GUIDEMO_CheckCancelDelay(2000);
        //
        // Turn left (velocity and decelerating)
        //
        WM_MOTION_SetSpeed(hMotion, GUI_COORD_X, 300);
        GUIDEMO_CheckCancelDelay(2000);
        WM_MOTION_SetDeceleration(hMotion, GUI_COORD_X, 300);
        GUIDEMO_CheckCancelDelay(2000);
        //
        // Turn right (velocity and decelerating)
        //
        WM_MOTION_SetSpeed(hMotion, GUI_COORD_X, -300);
        GUIDEMO_CheckCancelDelay(2000);
        WM_MOTION_SetDeceleration(hMotion, GUI_COORD_X, 300);
...
void GUIDEMO_RadialMenu(void) {
  //GUIDEMO_ConfigureDemo("Radial Menu", "", GUIDEMO_SHOW_CURSOR | GUIDEMO_SHOW_CONTROL);
  WM_MOTION_Enable(1);
