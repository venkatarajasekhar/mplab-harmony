...
void LCD_X_Config(void) {
  ...
  if( DRV_HANDLE_INVALID == DRV_GFX_LCC_Open( 0, DRV_IO_INTENT_EXCLUSIVE ) )
  {
      return;
  }
  
  Addr = (U32)DRV_GFX_LCC_GetBuffer();//Cached address access will be offset by the attribute coherent in MHC to make it uncached access
  LCD_SetVRAMAddrEx(0, (void *)Addr);
  //
