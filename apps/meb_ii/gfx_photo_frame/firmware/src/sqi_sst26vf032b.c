...
int SQI_DMA_PageRead(uint32_t deviceAddress, uint32_t count, uint8_t* destAddr)
{
    ...
    sqiCmdBuffer[0] = (uint8_t) SST26VF032_FAST_READ;
    ...
    // Command Descriptor 2 - BD2 (to send fast read command and address)
...
void SQI_PIO_PageWrite(uint32_t devAddress, uint8_t* pageBuffer)
{
    ...
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF032B_WEN << 24) | 0x00000000);      // wen = 0x06
    ...
    PLIB_SQI_TransmitData(SQI_ID_0, devAddress << 8 | SST26VF032B_PAGE_WRITE);   // page_write = 0x02
...
void SQI_DMA_PageWrite(uint32_t deviceAddress, uint8_t* pageBuffer)
{
    ...
    // Write the command to the transfer buffer
    PLIB_SQI_TransmitData(SQI_ID_0, (deviceAddress << 8) | SST26VF032B_PAGE_WRITE);
...
int SQI_FlashID_Read(void)
{
    ...
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF032B_QJID << 24) | 0x00000000);
    ...
    if (JedecID == SST26VF032B_JEDECID)
        return true;
    else
        return false;
}
...
void SQI_Flash_Setup(void)
{
    ...
    PLIB_SQI_ControlWordSet(SQI_ID_0, 1, SQI_CS_1, SQI_LANE_SINGLE, SQI_CMD_TRANSMIT, 1);
    PLIB_SQI_ControlWordSet(SQI_ID_0, 1, SQI_CS_1, SQI_LANE_SINGLE, SQI_CMD_TRANSMIT, 1);
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF032B_EQIO << 24) | 0x00000000); // 0x38
    ...
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF032B_BLKUP << 24) | (SST26VF032B_WEN << 16) | 0x00000000);        // BLKUP = 0x42
...
void SQI_Flash_Erase(void)
{
    ...
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF032B_ERASE << 24) | (SST26VF032B_WEN << 16) | 0x00000000); // WEN = 0x06, ERASE = 0xC7
...
uint8_t SQI_Flash_ReadStatus(void)
{
    ...
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF032B_RDSR << 24) | (SST26VF032B_EQIO << 16) | 0x00000000);
