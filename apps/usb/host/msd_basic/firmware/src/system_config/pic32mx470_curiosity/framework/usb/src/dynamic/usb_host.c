...
void _USB_HOST_TimerCallback(uint32_t context, uint32_t currtick)
{
    ...
    else
    {
        switch (deviceObj->deviceState)
        {
            ...
            case USB_HOST_DEVICE_STATE_ERROR:
                ...
                /* The device has entered an error state. 
                 * The control pipe should have been closed already. 
                 * We will only assign an invalid value to the pipe handle. 
                 * Note that we are doing this in an atomically, just to be safe.
                 *  The device then enters the Error holding state. In this state,
