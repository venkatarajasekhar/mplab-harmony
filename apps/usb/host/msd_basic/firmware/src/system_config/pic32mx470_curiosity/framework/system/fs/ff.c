...
/
/ May 15,'10 R0.08  Added a memory configuration option. (FAT_FS_USE_LFN = 3)
...
#if FAT_FS_MAX_SS != 512 && FAT_FS_MAX_SS != 1024 && FAT_FS_MAX_SS != 2048 && FAT_FS_MAX_SS != 4096
...
#if SYS_FS_VOLUME_NUMBER
static FATFS_VOLUME_OBJECT __attribute__ ((coherent, aligned(16))) FAFFSVolume[SYS_FS_VOLUME_NUMBER];
static FATFS  *FatFs[SYS_FS_VOLUME_NUMBER];	/* Pointer to the file system objects (logical drives) */
#else
#error Number of volumes must not be 0.
#endif

static FATFS_FILE_OBJECT __attribute__ ((coherent, aligned(16))) FATFSFileObject[SYS_FS_MAX_FILES];
...
#if FAT_FS_USE_LFN == 0			/* No LFN feature */
#define	DEF_NAMEBUF			uint8_t sfn[12]
#define INIT_BUF(dobj)		(dobj).fn = sfn
#define	FREE_BUF()

#elif FAT_FS_USE_LFN == 1			/* LFN feature with static working buffer */
static uint16_t LfnBuf[FAT_FS_MAX_LFN+1];
#define	DEF_NAMEBUF			uint8_t sfn[12]
...
uint16_t ff_wtoupper(uint16_t wc) {
    char c = (char)wc;
    if (c >= 'a' && c <= 'z') wc -= ' ';
    return wc;
}

#elif FAT_FS_USE_LFN == 2 		/* LFN feature with dynamic working buffer on the stack */
#define	DEF_NAMEBUF			uint8_t sfn[12]; uint16_t lbuf[FAT_FS_MAX_LFN+1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()

#elif FAT_FS_USE_LFN == 3 		/* LFN feature with dynamic working buffer on the heap */
#define	DEF_NAMEBUF			uint8_t sfn[12]; uint16_t *lfn
#define INIT_BUF(dobj)		{ lfn = ff_memalloc((FAT_FS_MAX_LFN + 1) * 2); \
...
)
{
	...
	do {
		...
		if (wc) {	/* Last char has not been processed */
			...
			if (i >= FAT_FS_MAX_LFN || wc != ff_wtoupper(lfnbuf[i++]))	/* Compare it */
...
)
{
	...
	do {
		uc = LD_WORD(dir+LfnOfs[s]);		/* Pick an LFN character from the entry */
		if (wc) {	/* Last char has not been processed */
			if (i >= FAT_FS_MAX_LFN) return 0;	/* Buffer overflow? */
...
#if FAT_FS_USE_LFN	/* LFN configuration */
...
)
{
	...
	for (;;) {
		...
		if (di >= FAT_FS_MAX_LFN)				/* Reject too long name */
			return FR_INVALID_NAME;
#if !_LFN_UNICODE
		w &= 0xFF;
...
)
{
	...
	if (dj->sect) {
		...
		for (i = 0; i < 8; i++) {	/* Copy name body */
			...
			if (FAT_FS_USE_LFN && (nt & NS_BODY) && IsUpper(c)) c += 0x20;
#if _LFN_UNICODE
		...
		if (dir[8] != ' ') {		/* Copy name extension */
			*p++ = '.';
			for (i = 8; i < 11; i++) {
				c = dir[i];
				if (c == ' ') break;
				if (FAT_FS_USE_LFN && (nt & NS_EXT) && IsUpper(c)) c += 0x20;
#if _LFN_UNICODE
...
)
{
	...
	/* Check if the file system object is valid or not */
	*rfs = 0;
	if (vol >= SYS_FS_VOLUME_NUMBER) 				/* Is the drive number valid? */
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	if (!_FS_READONLY && wmode && (stat & STA_PROTECT))	/* Check disk write protection if needed */
		return FR_WRITE_PROTECTED;
#if FAT_FS_MAX_SS != 512						/* Get disk sector size (variable sector size cfg only) */
...
int f_mount (
	uint8_t vol		/* Logical drive number to be mounted */
)
{
	FATFS *rfs;
        FATFS *fs = (FATFS *)0;
	...
	if (vol >= SYS_FS_VOLUME_NUMBER)		/* Check if the drive number is valid */
		return FR_INVALID_DRIVE;

        /* If the volume specified is already in use, then return failure, as we cannot mount it again */
        if(FAFFSVolume[vol].inUse == 1)
        {
            return FR_INVALID_DRIVE;
        }
        else
        {
            FAFFSVolume[vol].inUse = 1;  // take the volume
            fs = &FAFFSVolume[vol].volObj;
        }
...
)
{
	...
	if (vol >= SYS_FS_VOLUME_NUMBER)		/* Check if the drive number is valid */
        ...
        {
            FAFFSVolume[vol].inUse = 0;  // free the volume
        }
    ...
    for(hFATfs = 0; hFATfs < SYS_FS_MAX_FILES; hFATfs++)
    {
        ...
        {
            if(LD2PD(vol) == FATFSFileObject[hFATfs].fileObj.fs->drv)
            {
        ...
        {
            if(LD2PD(vol) == FATFSDirObject[hFATfs].dirObj.fs->drv)
            {
...
)
{
	...
	DEF_NAMEBUF;
        ...
        for(hFATfs = 0; hFATfs < SYS_FS_MAX_FILES; hFATfs++)
        {
            if(FATFSFileObject[hFATfs].inUse == 0)
            {
                FATFSFileObject[hFATfs].inUse = 1;
                fp = &FATFSFileObject[hFATfs].fileObj;
                *(uintptr_t *)handle = (uintptr_t)&FATFSFileObject[hFATfs];
                break;
            }
        }
        
        if(hFATfs >= SYS_FS_MAX_FILES)
        {
            return FR_INVALID_OBJECT;
        }
...
{
	if (drv >= SYS_FS_VOLUME_NUMBER) return FR_INVALID_DRIVE;
	...
	*buff = 0;
	...
	if (res == FR_OK) {
		...
		while ((ccl = dj.sclust) != 0) {	/* Repeat while current dir is a sub-dir */
			...
			tp = fno.fname;
			if (FAT_FS_USE_LFN && *buff) tp = buff;
			for (n = 0; tp[n]; n++) ;
			if (i < n + 3) {
...
)
{
	...
	DEF_NAMEBUF;
        ...
        for(hFATfs = 0; hFATfs < SYS_FS_MAX_FILES; hFATfs++)
        {
            if(FATFSDirObject[hFATfs].inUse == 0)
            {
                FATFSDirObject[hFATfs].inUse = 1;
                dj = &FATFSDirObject[hFATfs].dirObj;
                *(uintptr_t *)handle = (uintptr_t)&FATFSDirObject[hFATfs];
                break;
            }
        }
...
int f_closedir (
	uintptr_t handle       /* Pointer to directory object to close */
)
{
...
)
{
	FRESULT res = FR_OK;
	FATFS *fs = (FATFS *)NULL;
	uint32_t n = 0, clst = 0, sect = 0, stat = 0;
	uint32_t i = 0;
	uint8_t fat = 0, *p = NULL;
...
)
{
	FRESULT res = FR_OK;
	DIR dj = {};
	uint8_t *dir = NULL, n = 0;
	uint32_t dsc = 0, dcl = 0, pcl = 0, tm = get_fattime();
	DEF_NAMEBUF;
...
int f_getlabel (
	...
	uint32_t* sn			/* Pointer to a variable to return the volume serial number */
)
{
	FRESULT res = FR_OK;
	DIR dj = {};
...
#endif
#endif
			if (!w || chk_chr("\"*+,.:;<=>\?[]|\x7F", w) || j >= (uint32_t)((w >= 0x100) ? 10 : 11)) /* Reject invalid chars for volume label */
...
)
{
	...
	uint8_t fmt = 0, md = 0, sys = 0, *tbl = NULL, pdrv = 0, part = 0;
	uint32_t n_clst = 0, vs = 0, n = 0, wsect = 0;
	uint32_t i = 0;
	uint32_t b_vol = 0, b_fat = 0, b_dir = 0, b_data = 0;	/* LBA */
	uint32_t n_vol = 0, n_rsv = 0, n_fat = 0, n_dir = 0;	/* Size */
	FATFS *fs = (FATFS *)NULL;
	DSTATUS stat = 0;


	/* Check mounted drive and clear work area */
	if (vol >= SYS_FS_VOLUME_NUMBER) return FR_INVALID_DRIVE;
...
#if FAT_FS_MAX_SS != 512					/* Get disk sector size */
	if (disk_ioctl(pdrv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK || SS(fs) > FAT_FS_MAX_SS)
		return FR_DISK_ERR;
#endif
...
{
	uint32_t i = 0, n = 0, sz_cyl = 0, tot_cyl = 0, b_cyl = 0, e_cyl = 0, p_cyl = 0;
	uint8_t s_hd = 0, e_hd = 0, *p = NULL, *buf = (uint8_t*)work;
	DSTATUS stat = 0;
	uint32_t sz_disk = 0, sz_part = 0, s_part = 0;
...
)
{
	...
	btw = 1;
#endif
	f_write(handle, s, btw, &bw);		/* Write the char to the file */
...
)
{
	...
	for (n = 0; *str; str++, n++) {
		if (f_putc(*str, handle) == EOF) return EOF;
	}
	return n;
}
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		case 'S' :					/* String */
			...
			chc = 0;
			if (!(f & 2)) {
				while (j++ < w) chc += (cc = f_putc(' ', handle));
			}
			chc += (cc = f_puts(p, handle));
			while (j++ < w) chc += (cc = f_putc(' ', handle));
			if (cc != EOF) cc = chc;
			continue;
		case 'C' :					/* Character */
			cc = f_putc((TCHAR)va_arg(arp, int), handle); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
		...
		j = i; d = (f & 1) ? '0' : ' ';
		chc = 0;
		while (!(f & 2) && j++ < w) chc += (cc = f_putc(d, handle));
		do chc += (cc = f_putc(s[--i], handle)); while(i);
		while (j++ < w) chc += (cc = f_putc(' ', handle));
		if (cc != EOF) cc = chc;
	}
