...
/* Function:
    ...
    Parameters:
      handle  - Directory handle obtained during directory open.
      stat    - Empty structure of type SYS_FS_FSTAT, where the properties of
                the open directory will be populated after the SYS_FS_DirRead
                function returns successfully. If LFN is used, then the
                "lfname" member of the SYS_FS_FSTAT structure should be
                initialized with the address of suitable buffer. Also, the
                "lfsize" should be initialized with the size of the buffer.
                Once the function returns, the buffer whose address is held in
                "lfname" will have the file name(long file name)
