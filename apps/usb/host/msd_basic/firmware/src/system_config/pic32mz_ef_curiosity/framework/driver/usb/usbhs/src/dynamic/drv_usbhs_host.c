    ...
    switch(hDriver->attachState)
    {
        case DRV_USBHS_HOST_ATTACH_STATE_CHECK_FOR_DEVICE_ATTACH:
            ...
            if(hDriver->deviceAttached)
            {
                ...
                hDriver->timerHandle = SYS_TMR_DelayMS(DRV_USBHS_HOST_ATTACH_DEBOUNCE_DURATION);
...
void _DRV_USBHS_HOST_ResetStateMachine(DRV_USBHS_OBJ * hDriver)
{
    ...
    switch(hDriver->resetState)
    {
        ...
        case DRV_USBHS_HOST_RESET_STATE_START:
             ...
             * timer */
            hDriver->timerHandle = SYS_TMR_DelayMS(DRV_USBHS_HOST_RESET_DURATION);
            if(hDriver->timerHandle != SYS_TMR_HANDLE_INVALID)
            {
...
)
{
    ...
    {
        /* Non control tranfer pipes are not stored as groups.  We deallocate
...
void _DRV_USBHS_HOST_Tasks_ISR(DRV_USBHS_OBJ * hDriver)
{
    ...
    else if(interruptStatus & USBHS_GENINT_DEVDISCONN)
    {
        ...
        {
            /* Ask the host layer to de-enumerate this device. The denumeration
    ...
    if(interruptStatus & USBHS_GENINT_VBUSERR)
    {
        ...
        /* This line of code will be replaced with a USBHS Peripheral Library
         * call in a future release of Harmony. */
        USBEOFRSTbits.SOFRST = 2;
    }
...
void DRV_USBHS_HOST_ROOT_HUB_OperationEnable(DRV_HANDLE handle, bool enable)
{
    ...
    else
    {
        if(true == enable)
        {
            ...
            PLIB_USBHS_InterruptEnableSet(pUSBDrvObj->usbID, (USBHS_GENINT_VBUSERR|USBHS_GENINT_DEVCONN|USBHS_GENINT_DEVDISCONN), 0x1, 0x0);
