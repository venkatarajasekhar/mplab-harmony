...
* (comparatively) file system on the drive, which will consume ~18kB of overhead
* for the file system.  If the total drive size is too small to fit the 
* file system, then Windows will give an error. This also means that formatting
* the drive will "shrink" the usable data storage area, since the default
* FAT12 file system implemented in the Files.c data tables is very
...
* for storing the configuration bits, see device data sheet). This can be 
 ...
 * size (ex: FAT12 file system), then the total FAT entries that can fit in a
 ...
 * Therefore, more FAT sectors are needed if creating an MSD volume bigger than
 * this.
 *****************************************************************************/
 ...
 **********************************************************/
    ...
    #error "The current implementation of internal flash FS only supports a media sector size of 512.  Please modify your selected value in the FSconfig.h file."
#endif
