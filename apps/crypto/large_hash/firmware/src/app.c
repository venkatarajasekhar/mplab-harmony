    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_DISPLAY_RESULTS:
            ...
            else
            {
                ...
                appData.wallTime = _CP0_GET_COUNT() + SYS_CLK_BUS_PERIPHERAL_7 / 190;
#else
                appData.wallTime = _CP0_GET_COUNT() + SYS_CLK_FREQ / 190;
#endif
...
//            if (appData.wrComplete)
            if (appData.wallTime <= _CP0_GET_COUNT())
                appData.state = APP_STATE_DISPLAY_RESULTS;
            break;
