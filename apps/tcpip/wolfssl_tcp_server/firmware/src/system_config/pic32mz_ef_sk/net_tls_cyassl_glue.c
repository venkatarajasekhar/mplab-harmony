...
int NET_TLS_CG_Datagram_Receive_cb(void *sslin, char *buf, int sz, void *ctx)
{
    int fd = *(int *)ctx;
    uint16_t bufferSize;
    bufferSize = (*spTransData->fpDataGramReadyToRead)((uintptr_t)fd);
    if (bufferSize == 0)
    {
        return CYASSL_CBIO_ERR_WANT_READ;
    }
    bufferSize = (*spTransData->fpDataGramRead)((uintptr_t)fd, (uint8_t*)buf, sz);
    return bufferSize;
}
...
int NET_TLS_CG_Datagram_Send_cb(void *sslin, char *buf, int sz, void *ctx)
{
    int fd = *(int *)ctx;
    uint16_t bufferSize;
    bufferSize = (*spTransData->fpDataGramReadyToWrite)((uintptr_t)fd);
    if (bufferSize == 0)
    {
        return CYASSL_CBIO_ERR_WANT_WRITE;
    }

    bufferSize =  (*spTransData->fpDataGramWrite)((uintptr_t)fd, (uint8_t*)buf, (uint16_t)sz);
    return bufferSize;
}
...
bool NET_TLS_CyaSSLStreamServerCreate(uintptr_t transHandle, int8_t certIndex, void * providerData)
{
    ...
    pData->ctx = CyaSSL_CTX_new(CyaSSLv23_server_method());
    if (pData->ctx == 0)
    {
        return false;
    }
    ...
    if (CyaSSL_CTX_use_certificate_buffer(pData->ctx,
            (uint8_t*)serverCert,
            sizeof(serverCert),
    ...
    if (CyaSSL_CTX_use_PrivateKey_buffer(pData->ctx,
            (uint8_t*)serverCert,
            sizeof(serverCert),
            SSL_FILETYPE_PEM) != SSL_SUCCESS)
    {
...
NET_TLS_SessionStatus NET_TLS_CyaSSLStreamServerAccept(void * providerData)
{
    ...
    int result = CyaSSL_accept(pData->ssl);
    switch (result)
    {
        case SSL_SUCCESS:
        ...
        default:
        {
            ...
            switch (error)
            {
                ...
                case SSL_ERROR_WANT_WRITE:
                    return NET_TLS_SS_SERVER_NEGOTIATING;
                default:
                    return NET_TLS_SS_FAILED;
            }
        }
