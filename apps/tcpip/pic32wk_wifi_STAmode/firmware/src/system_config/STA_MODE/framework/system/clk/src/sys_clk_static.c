...
void SYS_CLK_Initialize( const SYS_CLK_INIT const * clkInit )
{
	...
	PLIB_OSC_SysPLLInputDivisorSet(OSC_ID_0, 1 - 1);
	PLIB_OSC_SysPLLMultiplierSelect(OSC_ID_0, 24);
	PLIB_OSC_SysPLLOutputDivisorSet(OSC_ID_0, OSC_SYSPLL_OUT_DIV_8);
	PLIB_OSC_SysPLLFrequencyRangeSet(OSC_ID_0, OSC_SYSPLL_FREQ_RANGE_25M_TO_40M);
	
	/* Configure UPLL */
	PLIB_OSC_PLLBypassDisable(OSC_ID_0, OSC_PLL_USB);
	PLIB_OSC_UPLLInputDivisorSet(OSC_ID_0, OSC_UPLL_IN_DIV_2);
	PLIB_OSC_UPLLMultiplierSelect(OSC_ID_0, 128);
	PLIB_OSC_UPLLOutputDivisorSet(OSC_ID_0, OSC_UPLL_OUT_DIV_1);
	PLIB_OSC_UPLLFrequencyRangeSet(OSC_ID_0, OSC_UPLL_FREQ_RANGE_10M_TO_16M);	
	...
	PLIB_OSC_BTPLLInputDivisorSet(OSC_ID_0, 1 - 1);
	PLIB_OSC_BTPLLMultiplierSelect(OSC_ID_0, 128);
	PLIB_OSC_BTPLLOutputDivisorSet(OSC_ID_0, OSC_BTPLL_OUT_DIV_1);
	PLIB_OSC_BTPLLFrequencyRangeSet(OSC_ID_0, OSC_BTPLL_FREQ_RANGE_10M_TO_16M);
	...
	if(PLIB_OSC_CurrentSysClockGet(OSC_ID_0) != OSC_PRIMARY_WITH_PLL)
	{
