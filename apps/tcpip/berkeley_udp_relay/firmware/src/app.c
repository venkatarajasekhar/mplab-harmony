            ...
            for (i = 0; i < nNets; i++)
            {
                ...
                if(dwLastIP[i].Val != ipAddr.Val)
                {
                    ...
                    if (ipAddr.v[0] != 0 && ipAddr.v[0] != 169) // Wait for a Valid IP
                    {
                        ...
                        SYS_CONSOLE_MESSAGE("Waiting for start command:\r\n");
                        SYS_CONSOLE_MESSAGE("\tcurrent - to get the current configuration:\r\n");
                        SYS_CONSOLE_MESSAGE("\tstart - to start the packet relay service:\r\n");
                    }
                }
            }
            break;
