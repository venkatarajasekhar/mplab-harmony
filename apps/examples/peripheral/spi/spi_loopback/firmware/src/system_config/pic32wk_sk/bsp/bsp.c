/*******************************************************************************
  ...
  Summary:
    Board Support Package implementation for PIC32WK Starter Kit with Module.
...
void BSP_USBVBUSSwitchStateSet(BSP_USB_VBUS_SWITCH_STATE state)
{
    ...
    PLIB_PORTS_PinWrite( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_12, state );
}
...
void BSP_USBVBUSPowerEnable(uint8_t port, bool enable)
{
    ...
    PLIB_PORTS_PinWrite( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_12, enable );
}
...
void BSP_Initialize(void )
{
    ...
    BSP_LEDOff(LED_SUCCESS);
    BSP_LEDOff(LED_FAILURE);
}
