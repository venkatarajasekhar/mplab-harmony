...
void APP_Tasks ( void )
{
//   const DRV_IO_INTENT Intent = DRV_IO_INTENT_READWRITE | DRV_IO_INTENT_NONBLOCKING;
   unsigned int x;
   switch(appData.state)
   {
      ...
      case APP_STATE_NVM_FILL_DATABUF_AND_ERASE_STATE:
        ...
        DRV_FLASH_ErasePage(appData.flashHandle, APP_PROGRAM_FLASH_BASE_ADDRESS_VALUE);
      ...
      case APP_STATE_NVM_WRITE_START:
        ...
        DRV_FLASH_WriteRow(appData.flashHandle, APP_PROGRAM_FLASH_BASE_ADDRESS_VALUE, databuff);
        //DRV_FLASH_WriteQuadWord(appData.flashHandle, APP_PROGRAM_FLASH_BASE_ADDRESS, databuff);
        //DRV_FLASH_WriteWord(appData.flashHandle, APP_PROGRAM_FLASH_BASE_ADDRESS, databuff[0]);
      ...
      case APP_STATE_NVM_WRITE_COMPLETION_CHECK_AND_VERIFY_CHECK:
        if(!DRV_FLASH_IsBusy(appData.flashHandle))
        {
            ...
            if (!memcmp(databuff, (void *)KVA0_TO_KVA1(APP_PROGRAM_FLASH_BASE_ADDRESS), sizeof(databuff)))
            {
