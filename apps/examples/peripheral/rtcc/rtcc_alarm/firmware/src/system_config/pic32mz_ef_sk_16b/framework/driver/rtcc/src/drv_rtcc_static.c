...
void DRV_RTCC_Initialize(void)
{
    ...
    PLIB_RTCC_RTCTimeSet(RTCC_ID_0, 0x05595500); /* Set RTCC time */
    ...
    PLIB_RTCC_AlarmTimeSet(RTCC_ID_0, 0x06000000); /* Set RTCC alarm time */
    PLIB_RTCC_AlarmDateSet(RTCC_ID_0, 0x00010100); /* Set RTCC alarm date */
    PLIB_RTCC_AlarmMaskModeSelect(RTCC_ID_0, RTCC_ALARM_ONCE_A_DAY); /* Set RTCC alarm mode */
    PLIB_RTCC_OutputSelect(RTCC_ID_0, RTCC_OUTPUT_ALARM_PULSE); /* Select ouput to be shown on RTCC */
    PLIB_RTCC_ClockOutputEnable(RTCC_ID_0); /* Enable RTCC otput */
    
    PLIB_RTCC_ClockSourceSelect(RTCC_ID_0,RTCC_CLOCK_SOURCE_LPRC); /* Set RTCC clock source (LPRC/SOSC) */

    /* Setup RTCC Interrupt */
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_RTCC); /* Set interrupt source */
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_RTCC, INT_PRIORITY_LEVEL1); /* Set interrupt priority */
...
void DRV_RTCC_ClockOutput(void)
{
    ...
    PLIB_RTCC_OutputSelect(RTCC_ID_0, RTCC_OUTPUT_ALARM_PULSE);
}
