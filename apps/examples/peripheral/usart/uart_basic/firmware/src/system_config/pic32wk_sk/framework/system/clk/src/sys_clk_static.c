...
void SYS_CLK_Initialize( const SYS_CLK_INIT const * clkInit )
{
	...
	PLIB_OSC_UPLLOutputDivisorSet(OSC_ID_0, OSC_UPLL_OUT_DIV_10);
	PLIB_OSC_UPLLFrequencyRangeSet(OSC_ID_0, OSC_UPLL_FREQ_RANGE_25M_TO_40M);	
	PLIB_OSC_UsbClockSourceSelect (OSC_ID_0, SYS_OSC_USBCLK_PRIMARY);

	/* Configure BTPLL */
	PLIB_OSC_PLLBypassDisable(OSC_ID_0, OSC_PLL_BT);
	PLIB_OSC_BTPLLInputClockSourceSet(OSC_ID_0, OSC_BTPLL_IN_CLK_SOURCE_PRIMARY);
	PLIB_OSC_BTPLLInputDivisorSet(OSC_ID_0, 2 - 1);
	PLIB_OSC_BTPLLMultiplierSelect(OSC_ID_0, 39);
	PLIB_OSC_BTPLLOutputDivisorSet(OSC_ID_0, OSC_BTPLL_OUT_DIV_30);
	PLIB_OSC_BTPLLFrequencyRangeSet(OSC_ID_0, OSC_BTPLL_FREQ_RANGE_16M_TO_25M);
