...
{
    /* Initialize I2C0 */
    PLIB_I2C_BaudRateSet(I2C_ID_2, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_1), 50000);
    ...
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_I2C2, INT_PRIORITY_LEVEL4);
...
{ 
    ...
    if (i2cBufferData != NULL)
    {
        ...
        if (i2cBufferData->i2cMode == DRV_I2C_MODE_MASTER)
        {
            ...
            if ( ((!(PLIB_I2C_StartWasDetected(I2C_ID_2))) && (!(PLIB_I2C_StopWasDetected(I2C_ID_2)))) || (PLIB_I2C_StopWasDetected(I2C_ID_2)) )
            {
                if ( (PLIB_I2C_BusIsIdle(I2C_ID_2)) && (!(PLIB_INT_SourceFlagGet(INT_ID_0, INT_SOURCE_I2C_2_MASTER))) )
                {
                        i2c0State = DRV_I2C_TASK_SEND_DEVICE_ADDRESS;
    ...
    switch (i2c0State)
    {
        case DRV_I2C_TASK_SEND_DEVICE_ADDRESS:
        {
            if (i2c0Obj.i2cMode == DRV_I2C_MODE_MASTER)
            {
                ...
                if (PLIB_I2C_StartWasDetected(I2C_ID_2))
                {
                    if ( (lBufferObj0->i2coperation == DRV_I2C_OP_WRITE) || (lBufferObj0->i2coperation == DRV_I2C_OP_WRITE_READ) )
                    {
                        ...
                        PLIB_I2C_TransmitterByteSend( I2C_ID_2, lBufferObj0->slaveaddresshighbyte );
                    }
                    else
                    {
                        lBufferObj0->bufferstatus    = DRV_I2C_BUFFER_SLAVE_READ_REQUESTED;
                        i2c0State = DRV_I2C_TASK_SET_RCEN_ONLY;
                        PLIB_I2C_TransmitterByteSend(I2C_ID_2, lBufferObj0->slaveaddresshighbyte | DRV_I2C_OP_READ );
                    }
                }
            }
            else if (i2c0Obj.i2cMode == DRV_I2C_MODE_SLAVE)
            {
                ...
                else if ( (PLIB_I2C_SlaveReadIsRequested(I2C_ID_2)) && (PLIB_I2C_SlaveAddressIsDetected(I2C_ID_2)) )
                {

                    PLIB_I2C_ReceivedByteGet (I2C_ID_2);
                    PLIB_I2C_SlaveClockHold (I2C_ID_2);
                    ...
                    // transmit first byte
                    PLIB_I2C_TransmitterByteSend(I2C_ID_2, *lBufferObj0->txBuffer++);
                ...
                else if ( (!PLIB_I2C_SlaveReadIsRequested(I2C_ID_2)) && PLIB_I2C_SlaveDataIsDetected(I2C_ID_2))
                {
                    ...
                    {
                        *lBufferObj0->rxBuffer++ = PLIB_I2C_ReceivedByteGet(I2C_ID_2);
                        lBufferObj0->transfersize++;
                ...
                else if (PLIB_I2C_SlaveReadIsRequested(I2C_ID_2) && PLIB_I2C_SlaveDataIsDetected(I2C_ID_2))
                {
                    ...
                    else if ( !PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_2) )
                    {
                        PLIB_I2C_SlaveClockRelease (I2C_ID_2);
        ...
        case DRV_I2C_SEND_DEVICE_ADDRESS_BYTE_2:
        {
            ...
            {
                PLIB_I2C_TransmitterByteSend(I2C_ID_2,lBufferObj0->slaveaddresslowbyte>>1) ;
                i2c0State = DRV_I2C_TASK_SET_RCEN_ONLY;
            }
            else
            {                         
        ...
        case DRV_I2C_SEND_RANDOM_READ_DEVICE_ADDRESS:
        {
            ...
            PLIB_I2C_TransmitterByteSend( I2C_ID_2,tempDeviceAddress );
            i2c0State = DRV_I2C_TASK_SET_RCEN_ONLY;
            break;
        }
        ...
        case    DRV_I2C_TASK_PROCESS_WRITE_ONLY:
        {
            if (lBufferObj0->transfersize)
            {
                if ( PLIB_I2C_TransmitterByteHasCompleted (I2C_ID_2) )
                {
                    if ( PLIB_I2C_TransmitterByteWasAcknowledged (I2C_ID_2) ||
            ...
            */
            ...
            if ( (PLIB_I2C_TransmitterByteWasAcknowledged (I2C_ID_2)) ||
                    ( PLIB_I2C_TransmitterByteHasCompleted (I2C_ID_2)  &&
                        (!PLIB_I2C_TransmitterByteWasAcknowledged (I2C_ID_2)) ) )
            {
        ...
        {
            if (PLIB_I2C_StopWasDetected(I2C_ID_2))
            {
                i2c0State = DRV_I2C_TASK_SEND_DEVICE_ADDRESS;
                if (lBufferObj0->transfersize)
                {
...
/* Master Object variables*/
DRV_I2C_DATA_OBJECT_TASK                    i2c1State;
DRV_STATIC_I2C_OBJECT                       i2c1Obj;
DRV_STATIC_I2C_CLIENT_OBJ                   i2c1ClientObj;

/* Variables to maintain Queue State */
volatile uint16_t QueueIn_1     = 0;
volatile uint16_t QueueOut_1    = 0;
volatile uint16_t QueueDelta_1  = 0;

static DRV_STATIC_I2C_OBJECT sDrvI2CBufferObj_1[DRV_I2C_NUM_OF_BUFFER_OBJECTS];

DRV_STATIC_I2C_OBJECT* lBufferObj1;

// *****************************************************************************
// Section: Queue 1 Initialize Queue index variables
// *****************************************************************************
void QueueInitialize_1(void)
{
    QueueIn_0 =  0;
    QueueOut_0 = 0;
}
...
// *****************************************************************************
// Section: Queue 1 Find a slot in queue
// *****************************************************************************

DRV_STATIC_I2C_OBJECT*  _DRV_I2C_QueueSlotGet_1(void)
{
    ...
    if (QueueIn_1 >= QueueOut_1)
    {
        numberOfSlotsAvailable = (DRV_I2C_NUM_OF_BUFFER_OBJECTS-(QueueIn_1 - QueueOut_1) -1);
    }
    else 
    {
        numberOfSlotsAvailable = ((QueueOut_1 - QueueIn_1) -1);      
    }
    
    if (numberOfSlotsAvailable > 0)
    {
        lQueueObj = &sDrvI2CBufferObj_1[QueueIn_1];
        
        QueueIn_1++;
        
        if (QueueIn_1 >= (DRV_I2C_NUM_OF_BUFFER_OBJECTS))
        {
            QueueIn_1 = 0;
        }
...
bool IsQueueEmpty_1(void)
{
    if ( (QueueOut_1 == QueueIn_1) )
        return true;
...
DRV_STATIC_I2C_OBJECT* _DRV_I2C_QueuePop_1(void)
{
    uint8_t temp;
    ...
    lQueuePop = &sDrvI2CBufferObj_1[temp]; 
    
    
    if (i2c1Obj.i2cMode == DRV_I2C_MODE_SLAVE)
    {    
        if ( QueueOut_1 >= (DRV_I2C_NUM_OF_BUFFER_OBJECTS-1) )
            QueueOut_1 = 0;
        else
            QueueOut_1++;                  
    }
...
void DRV_I2C1_Initialize(void)
{
    ...
    PLIB_I2C_SlaveAddress7BitSet(I2C_ID_4, 0x60>>1);
    PLIB_I2C_SlaveMask7BitSet(I2C_ID_4, 0x00);
    PLIB_I2C_GeneralCallDisable(I2C_ID_4);
    PLIB_I2C_ReservedAddressProtectEnable(I2C_ID_4);
    PLIB_I2C_SlaveClockStretchingEnable(I2C_ID_4);

    /*include callback function, see commented out line below for example */
    /* refer at top of the file for example   */
    i2c1Obj.operationStarting = &slaveRxorTXNotification;
    
    i2c1Obj.i2cMode         = DRV_I2C_MODE_SLAVE;
    i2c1Obj.transfersize    = 0;
    ...
    PLIB_INT_SourceFlagClear(INT_ID_0, INT_SOURCE_I2C_4_ERROR);
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_I2C_4_ERROR);
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_I2C4, INT_PRIORITY_LEVEL5);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_I2C4, INT_SUBPRIORITY_LEVEL0);
...
// *****************************************************************************
// Section: I2C Instance 1 Function to read a buffer (Master/Slave)
// *****************************************************************************

DRV_I2C_BUFFER_HANDLE DRV_I2C1_Receive (uint16_t deviceaddress,
                                    void *rxBuffer,
                                    size_t size,
                                    void * context)
{ 
    ...
    i2cBufferData = _DRV_I2C_QueueSlotGet_1();
...
// *****************************************************************************
// Section: I2C Instance 1 Function to write a buffer (Master/Slave)
// *****************************************************************************

DRV_I2C_BUFFER_HANDLE DRV_I2C1_Transmit (uint16_t deviceaddress,
                                    void *txBuffer,
                                    size_t size,
                                    void * context)
{
...
DRV_I2C_BUFFER_HANDLE DRV_I2C1_TransmitForced (uint16_t deviceaddress,
                                                             void *txBuffer,
                                                             size_t size,
                                                             void * context)
{
...
DRV_I2C_BUFFER_HANDLE DRV_I2C1_TransmitThenReceive ( uint16_t deviceaddress,
                                    void *txBuffer,
                                    size_t wsize,
                                    void *rxBuffer,
                                    size_t rsize,
...
void DRV_I2C1_Tasks(void)
{
    ...
    switch (i2c1State)
    {
        case DRV_I2C_TASK_SEND_DEVICE_ADDRESS:
        {
            if (i2c1Obj.i2cMode == DRV_I2C_MODE_MASTER)
            {
                lBufferObj1 = _DRV_I2C_QueuePop_1();

                if (PLIB_I2C_StartWasDetected(I2C_ID_4))
                {
                    if ( (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE) || (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE_READ) )
                    {
                        if (lBufferObj1->slaveaddresslowbyte)
                        {
                            i2c1State = DRV_I2C_SEND_DEVICE_ADDRESS_BYTE_2;
                        }
                        else
                        {
                            lBufferObj1->bufferstatus    = DRV_I2C_BUFFER_SLAVE_WRITE_REQUESTED;
                            i2c1State = DRV_I2C_TASK_PROCESS_WRITE_ONLY;
                        }

                        PLIB_I2C_TransmitterByteSend( I2C_ID_4, lBufferObj1->slaveaddresshighbyte );
                    }
                    else
                    {
                        lBufferObj1->bufferstatus    = DRV_I2C_BUFFER_SLAVE_READ_REQUESTED;
                        i2c1State = DRV_I2C_TASK_SET_RCEN_ONLY;
                        PLIB_I2C_TransmitterByteSend(I2C_ID_4, lBufferObj1->slaveaddresshighbyte | DRV_I2C_OP_READ );
                    }
                }
            }
            else if (i2c1Obj.i2cMode == DRV_I2C_MODE_SLAVE)
            {
                i2c1State = DRV_I2C_TASK_SEND_DEVICE_ADDRESS;
                /* Slave is accepting data from Master */
                if ( (!PLIB_I2C_SlaveReadIsRequested(I2C_ID_4)) && (PLIB_I2C_SlaveAddressIsDetected(I2C_ID_4)) )
                {
                    i2c1Obj.operationStarting(DRV_I2C_BUFFER_SLAVE_READ_REQUESTED,0x00);

                    lBufferObj1 = _DRV_I2C_QueuePop_1();

                    /* do a dummy read so that I2CxRCV is cleared and not clobbered with data */
                    PLIB_I2C_ReceivedByteGet (I2C_ID_4);
                    PLIB_I2C_SlaveClockRelease (I2C_ID_4);
                }
                /*  Slave is sending data back to the Master    */
                else if ( (PLIB_I2C_SlaveReadIsRequested(I2C_ID_4)) && (PLIB_I2C_SlaveAddressIsDetected(I2C_ID_4)) )
                {

                    PLIB_I2C_ReceivedByteGet (I2C_ID_4);
                    PLIB_I2C_SlaveClockHold (I2C_ID_4);
                    
                    i2c1Obj.operationStarting(DRV_I2C_BUFFER_SLAVE_WRITE_REQUESTED,0x00);

                    lBufferObj1 = _DRV_I2C_QueuePop_1();

                    lBufferObj1->transfersize = 0;

                    // transmit first byte
                    PLIB_I2C_TransmitterByteSend(I2C_ID_4, *lBufferObj1->txBuffer++);
                    lBufferObj1->transfersize++;
                    PLIB_I2C_SlaveClockRelease (I2C_ID_4);
                }
                else if ( (!PLIB_I2C_SlaveReadIsRequested(I2C_ID_4)) && PLIB_I2C_SlaveDataIsDetected(I2C_ID_4))
                {
                    /*        Master sends data to the slave        */ 
                    PLIB_I2C_SlaveClockRelease (I2C_ID_4);

                    if (lBufferObj1->transfersize < 255)
                    {
                        *lBufferObj1->rxBuffer++ = PLIB_I2C_ReceivedByteGet(I2C_ID_4);
                        lBufferObj1->transfersize++;
                        lBufferObj1->bufferstatus = DRV_I2C_BUFFER_SLAVE_READ_BYTE;
                        if ( i2c1ClientObj.callback != NULL )
                        {
                            /* Give an indication to the higher layer upon successful operation */
                            i2c1ClientObj.callback(DRV_I2C_BUFFER_SLAVE_READ_BYTE, 
                                        (DRV_I2C_BUFFER_HANDLE) lBufferObj1, 0x00);
                        }                       
                    }
                    else
                    {
                        lBufferObj1->bufferstatus = DRV_I2C_BUFFER_EVENT_ERROR;                                 
                        PLIB_I2C_ReceivedByteGet(I2C_ID_4);
                        if ( i2c1ClientObj.callback != NULL )
                        {
                            /* Give an indication to the higher layer upon when error happens */
                            i2c1ClientObj.callback(DRV_I2C_BUFFER_EVENT_ERROR, 
                                        (DRV_I2C_BUFFER_HANDLE) lBufferObj1, 0x00);
                        }  
                    }
                }
                else if (PLIB_I2C_SlaveReadIsRequested(I2C_ID_4) && PLIB_I2C_SlaveDataIsDetected(I2C_ID_4))
                {
                    ...
                    if ( PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_4) )
                    {
                        if ( lBufferObj1->transfersize < 255 )
                        {
                            ...
                            PLIB_I2C_TransmitterByteSend (I2C_ID_4, *lBufferObj1->txBuffer++ );
                            lBufferObj1->transfersize++;
                            lBufferObj1->bufferstatus = DRV_I2C_BUFFER_SLAVE_WRITE_BYTE;
                            if ( i2c1ClientObj.callback != NULL )
                            {
                                /* Give an indication to the higher layer upon successful transmission */
                                i2c1ClientObj.callback(DRV_I2C_BUFFER_SLAVE_WRITE_BYTE, 
                                            (DRV_I2C_BUFFER_HANDLE) lBufferObj1, 0x00);
                    }
                        }
                        else
                        {
                            lBufferObj1->bufferstatus = DRV_I2C_BUFFER_EVENT_ERROR;
                            PLIB_I2C_TransmitterByteSend(I2C_ID_4, 0);
                            if ( i2c1ClientObj.callback != NULL )
                            {
                                /* Give an indication to the higher layer upon when error happens */
                                i2c1ClientObj.callback(DRV_I2C_BUFFER_EVENT_ERROR, 
                    ...
                    else if ( !PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_4) )
                    {
                        PLIB_I2C_SlaveClockRelease (I2C_ID_4);
                        //TODO callback implementation
                        lBufferObj1->bufferstatus = DRV_I2C_BUFFER_EVENT_COMPLETE; 
                    }
                }
            }
            break;
        ...
        case DRV_I2C_SEND_DEVICE_ADDRESS_BYTE_2:
        {
            if ( lBufferObj1->i2coperation == DRV_I2C_OP_READ )
            {
                PLIB_I2C_TransmitterByteSend(I2C_ID_4,lBufferObj1->slaveaddresslowbyte>>1) ;
                i2c1State = DRV_I2C_TASK_SET_RCEN_ONLY;
            }
            else
            {                         
        ...
        case DRV_I2C_SEND_RANDOM_READ_DEVICE_ADDRESS:
        {
            tempDeviceAddress = (lBufferObj1->slaveaddresshighbyte);
            tempDeviceAddress |= 0x01;
            PLIB_I2C_TransmitterByteSend( I2C_ID_4,tempDeviceAddress );
            i2c0State = DRV_I2C_TASK_SET_RCEN_ONLY;
            break;
        }
        ...
        {
            /* Read I2CxRCV register for received data */
            if (lBufferObj1->transfersize)
            {
                if ( PLIB_I2C_ReceivedByteIsAvailable (I2C_ID_4) )
                {
                    /* If all but one reception is complete */
                    if ( lBufferObj1->transfersize > 1 )
                    {
                        *lBufferObj1->rxBuffer++ = PLIB_I2C_ReceivedByteGet (I2C_ID_4);
                        lBufferObj1->transfersize--;
                        lBufferObj1->actualtransfersize++;
                        i2c1State  = DRV_I2C_TASK_SET_RCEN_ONLY;
        ...
        case    DRV_I2C_TASK_PROCESS_WRITE_ONLY:
        {
            if (lBufferObj1->transfersize)
            {
                if ( PLIB_I2C_TransmitterByteHasCompleted (I2C_ID_4) )
                {
                    if ( PLIB_I2C_TransmitterByteWasAcknowledged (I2C_ID_4) ||
                        (lBufferObj1->transmitForced)  )
                    {
                        if (lBufferObj1->transfersize > 1)
                            i2c1State = DRV_I2C_TASK_PROCESS_WRITE_ONLY;
                        else
                    ...
                    {
                        i2c1State = DRV_I2C_TASK_PROCESS_STOP;
        ...
        case    DRV_I2C_BUS_SILENT:
        {
            ...
            */
            ...
            if ( (PLIB_I2C_TransmitterByteWasAcknowledged (I2C_ID_4)) ||
                    ( PLIB_I2C_TransmitterByteHasCompleted (I2C_ID_4)  &&
                        (!PLIB_I2C_TransmitterByteWasAcknowledged (I2C_ID_4)) ) )
            {
                if (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE_READ)
                {
                    lBufferObj1->transfersize   = lBufferObj1->readtransfersize;
                    i2c1State                   = DRV_I2C_SEND_RANDOM_READ_DEVICE_ADDRESS;
                    lBufferObj1->i2coperation   = DRV_I2C_OP_READ;
        ...
        case DRV_I2C_TASK_SET_RCEN_ONLY:
		{
            if ( PLIB_I2C_TransmitterByteHasCompleted(I2C_ID_4) )
            {
                ...
                {
                    /* if I2CxSTAT.ACKEN bit is 0, the Acknowledge sequence has been completed */
                    if (PLIB_I2C_ReceiverByteAcknowledgeHasCompleted(I2C_ID_4))
                    {
                        PLIB_I2C_MasterReceiverClock1Byte (I2C_ID_4);
                        i2c1State = DRV_I2C_TASK_PROCESS_READ_ONLY;
                    }
                }
                else
                {
        ...
        case    DRV_I2C_TASK_PROCESS_STOP:
        {
            if (PLIB_I2C_StopWasDetected(I2C_ID_4))
            {
                i2c1State = DRV_I2C_TASK_SEND_DEVICE_ADDRESS;
                if (lBufferObj1->transfersize)
                {
                ...
                {
                    /* Give an indication to the higher layer upon successful transmission */
                    i2c1ClientObj.callback(lBufferObj1->bufferstatus, 
                                                        (DRV_I2C_BUFFER_HANDLE) lBufferObj1, 0x00);
                }
            }
            else
    ...
    if (i2c1Obj.i2cMode == DRV_I2C_MODE_MASTER)
    {
        if ( (lBufferObj1->bufferstatus == DRV_I2C_BUFFER_EVENT_COMPLETE) || (lBufferObj1->bufferstatus == DRV_I2C_BUFFER_EVENT_ERROR) )
        {
...
uint32_t DRV_I2C1_BytesTransferred ( DRV_I2C_BUFFER_HANDLE bufferHandle )
{
    /* returns the number of bytes transferred in an I2C transaction */
    return ((DRV_STATIC_I2C_OBJECT*)bufferHandle)->actualtransfersize;
}
...
void DRV_I2C1_BufferEventHandlerSet (const DRV_I2C_BUFFER_EVENT_HANDLER eventHandler,
                                                   const uintptr_t context )
{
    i2c1ClientObj.callback = eventHandler;

    i2c1ClientObj.context = context;
}   /* DRV_I2C_BufferEventHandlerSet */
