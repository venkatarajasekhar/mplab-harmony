...
{
    /* Initialize I2C0 */
    PLIB_I2C_BaudRateSet(I2C_ID_2, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 100000);
    ...
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_I2C2_MASTER, INT_PRIORITY_LEVEL3);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_I2C2_MASTER, INT_SUBPRIORITY_LEVEL0);

    /* Initialize fault interrupt */
    PLIB_INT_SourceFlagClear(INT_ID_0, INT_SOURCE_I2C_2_BUS);
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_I2C_2_BUS);
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_I2C2_BUS, INT_PRIORITY_LEVEL1);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_I2C2_BUS, INT_SUBPRIORITY_LEVEL0);
...
void DRV_I2C1_Initialize(void)
{
    ...
    PLIB_I2C_SlaveAddress7BitSet(I2C_ID_1, 0x60>>1);
    ...
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_I2C1_SLAVE, INT_PRIORITY_LEVEL5);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_I2C1_SLAVE, INT_SUBPRIORITY_LEVEL0);

    /* Initialize fault interrupt */
    PLIB_INT_SourceFlagClear(INT_ID_0, INT_SOURCE_I2C_1_BUS);
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_I2C_1_BUS);
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_I2C1_BUS, INT_PRIORITY_LEVEL1);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_I2C1_BUS, INT_SUBPRIORITY_LEVEL0);
...
void DRV_I2C1_Tasks(void)
{
    ...
    switch (i2c1State)
    {
        case DRV_I2C_TASK_SEND_DEVICE_ADDRESS:
        {
            if (i2c1Obj.i2cMode == DRV_I2C_MODE_MASTER)
            {
                ...
                if (PLIB_I2C_StartWasDetected(I2C_ID_1))
                {
                    if ( (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE) || (lBufferObj1->i2coperation == DRV_I2C_OP_WRITE_READ) )
                    {
                        ...
                        PLIB_I2C_TransmitterByteSend( I2C_ID_1, lBufferObj1->slaveaddresshighbyte );
                    }
                    else
                    {
                        lBufferObj1->bufferstatus    = DRV_I2C_BUFFER_SLAVE_READ_REQUESTED;
                        i2c1State = DRV_I2C_TASK_SET_RCEN_ONLY;
                        PLIB_I2C_TransmitterByteSend(I2C_ID_1, lBufferObj1->slaveaddresshighbyte | DRV_I2C_OP_READ );
                    }
                }
            }
            else if (i2c1Obj.i2cMode == DRV_I2C_MODE_SLAVE)
            {
                ...
                else if ( (PLIB_I2C_SlaveReadIsRequested(I2C_ID_1)) && (PLIB_I2C_SlaveAddressIsDetected(I2C_ID_1)) )
                {
                    ...
                    // transmit first byte
                    PLIB_I2C_TransmitterByteSend(I2C_ID_1, *lBufferObj1->txBuffer++);
                ...
                else if ( (!PLIB_I2C_SlaveReadIsRequested(I2C_ID_1)) && PLIB_I2C_SlaveDataIsDetected(I2C_ID_1))
                {
                    ...
                    {
                        *lBufferObj1->rxBuffer++ = PLIB_I2C_ReceivedByteGet(I2C_ID_1);
                        lBufferObj1->transfersize++;
        ...
        case DRV_I2C_SEND_DEVICE_ADDRESS_BYTE_2:
        {
            ...
            {
                PLIB_I2C_TransmitterByteSend(I2C_ID_1,lBufferObj1->slaveaddresslowbyte>>1) ;
                i2c1State = DRV_I2C_TASK_SET_RCEN_ONLY;
            }
            else
            {                         
