...
void DRV_SQI_Initialize(void)
{
    ...
    PLIB_SQI_ClockDividerSet(DRV_SQI_ID_0, CLK_DIV_1);
...
void DRV_SQI_PIO_PageWrite (uint32_t flashAddress, uint8_t* writeBuffer)
{
    ...
    _DRV_SQI_StartCoreTimer(0);
    _DRV_SQI_CoreTimer_Delay(18000);
}
...
void DRV_SQI_XIP_Read (uint32_t flashAddress, uint32_t readSize, uint8_t* readBuffer)
{
    uint32_t readLoop;
    uint32_t * sqiXIPAddr;
	uint32_t * intReadBuffer;

    // Address decoding
    sqiXIPAddr = (uint32_t *) (XIP_UNCACHED_ADDR_MASK | flashAddress);
	
	intReadBuffer = (uint32_t *) readBuffer;

    // Configure XIP Control 1 SFR (quad lane fast read)
    PLIB_SQI_XIPControlWord1Set(DRV_SQI_ID_0,DRV_DUMMY_BYTES_3,DRV_ADDR_BYTES_3,
                                FLASH_FAST_READ,DRV_SQI_LANE_QUAD,
                                DRV_SQI_LANE_QUAD,DRV_SQI_LANE_QUAD,
                                DRV_SQI_LANE_QUAD,DRV_SQI_LANE_QUAD);

    // Configure XIP Control 2 SFR (chip select 1)
    PLIB_SQI_XIPControlWord2Set(DRV_SQI_ID_0,DRV_SQI_CS_1,DRV_MODE_BYTES_0,0);

    // Set to XIP mode
    PLIB_SQI_TransferModeSet(DRV_SQI_ID_0, DRV_SQI_XFER_MODE_XIP);

    // Access the SQI flash through memory mapped address
    for (readLoop = 0; readLoop < readSize; readLoop++){
        *intReadBuffer++ = *sqiXIPAddr++;
    }
