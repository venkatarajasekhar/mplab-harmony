...
uint8_t __attribute__((coherent))   writeBuffer[FLASH_PAGE_SIZE];
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_FLASH_ID_READ:
        {
            ...
            sqiStatus = DRV_SQI_Flash_ID_Check();
        ...
        {
            for (writeReadLoop = 0; writeReadLoop < FLASH_PAGE_SIZE; writeReadLoop ++)
                writeBuffer[writeReadLoop] = writeReadLoop;
            
            DRV_SQI_PIO_PageWrite(FLASH_PAGE_ADDR, writeBuffer);
        ...
        {
            /* Get the ID read status */
            DRV_SQI_DMA_Read(FLASH_PAGE_ADDR, FLASH_PAGE_SIZE, readBuffer);
            
            sqiStatus = memcmp(writeBuffer, readBuffer, FLASH_PAGE_SIZE);
