...
void DRV_SQI_Flash_Erase (void)
{
    ...
    PLIB_SQI_ControlWordSet(DRV_SQI_ID_0, 0, DRV_SQI_CS_1, DRV_SQI_LANE_QUAD, DRV_SQI_CMD_TRANSMIT, 12);    /* BLKUP bits */
...
void DRV_SQI_PIO_PageRead(uint32_t flashAddress, uint32_t* readBuffer)
{
    ...
    PLIB_SQI_ControlWordSet(DRV_SQI_ID_0,0,DRV_SQI_CS_1,DRV_SQI_LANE_QUAD,DRV_SQI_CMD_TRANSMIT,1);
    ...
    PLIB_SQI_TransmitData(DRV_SQI_ID_0, sqiCmdBuffer[0] << 24 | FLASH_NOP << 16 | (FLASH_NOP << 8) | FLASH_NOP);

    /* Write address and dummy bytes {DUMMY, ADDRESS[3:0]} */
    PLIB_SQI_TransmitData(DRV_SQI_ID_0, (FLASH_NOP << 24) | (sqiCmdBuffer[3] << 16) | (sqiCmdBuffer[2] << 8) | (sqiCmdBuffer[1]));

    /* Setup receive buffer threshold */
    _DRV_SQI_BufferThrSet(0, 0, 28);
    ...
    {
        while ((PLIB_SQI_RxBufferThresholdGet(DRV_SQI_ID_0)& 0xFF) != 28);
