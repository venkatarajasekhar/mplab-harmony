...
typedef struct 
{
    uint32_t    stamp[APP_TIMESTAMP_NUM];
    uint8_t     index;
    uint8_t     limit;
} timestampsT;


static SYS_RTCC_ALARM_HANDLE appHandle;
...
static void APP_RTCC_TIMESTAMP_Task(void)
{
    static timestampsT timestamps;

    switch (appData.rtccTimeStateMachine)
    {
        default:
        case APP_RTCC_TIMESTAMP_START:
            /* register the callback with system RTCC */
            timestamps.limit = APP_TIMESTAMP_NUM;
            timestamps.index = 0;
            appHandle = SYS_RTCC_AlarmRegister(APP_RTCC_TIMESTAMP_Callback, 
                (uintptr_t)&timestamps);

            /* continue only if the handle is valid */
            if (appHandle != SYS_RTCC_ALARM_HANDLE_INVALID)
            {
                SYS_RTCC_AlarmEnable();
                appData.rtccTimeStateMachine =  APP_RTCC_TIMESTAMP_CHECK;
            }
            break;

        case APP_RTCC_TIMESTAMP_CHECK:
...
void APP_Initialize ( void )
{
    ...
    appData.rtccTimeStateMachine = APP_RTCC_TIMESTAMP_START;
