...
/*******************************************************************************
  Interface definition of the power system service.
  ...
  Summary:
    Interface definition of realtime Clock & Calendar.

  Description:
    This file defines the interface for realtime clock and calendar.
    The realtime clock allows events to be triggered by time and date.
*******************************************************************************/
...
/* SYS_RTCC_STATUS
  ...
  Remarks:
  	Status should be checked after a call to see if the call executed OK and
...
/* SYS_RTCC_INIT
  ...
  Summary:
    Stuff this full of things to initialize the RTCC.

  Description:
    Fill this structure to initialize the RTCC
*/
...
typedef	struct
{
    ...
    SYS_RTCC_BCD_TIME rtccTime;
    SYS_RTCC_BCD_DATE rtccDate;
    SYS_RTCC_BCD_TIME alarmTime;
    SYS_RTCC_BCD_DATE alarmDate;
    RTCC_ALARM_MASK_CONFIGURATION alarmMask;
    bool alarmRepeatForever;
    uint8_t alarmRepeatCount;

    /* enable output */
    bool outputEnabled;

    /* which output to use */
    RTCC_OUTPUT_SELECT outputSelect;
    
    /* Clock Selection */
    RTCC_CLOCK_SOURCE_SELECT clockSource;

    /* Interrupt selection */
    bool interruptUsed;
    INT_SOURCE interruptSource;
    INT_VECTOR interruptVector;
    INT_PRIORITY_LEVEL interruptPriority;
    INT_SUBPRIORITY_LEVEL interruptSubpriority;
...
/*
  ...
  Example:
    <code>
    sysObj.sysRtcc = SYS_RTCC_Initialize((const SYS_MODULE_INIT  * const)&Init);
    </code>
...
/* Function:
  ...
  Description:
    This function is used to maintain the system RTCC's internal state machine.
