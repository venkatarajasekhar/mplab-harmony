...
bool _DRV_TOUCH_MTCH6303_ClientBufferQueueObjectsRemove( void )
{
    ...
    interruptWasEnabled = SYS_INT_SourceDisable(INT_SOURCE_EXTERNAL_1);
        ...
        NULL               != bufferObj )
    {
        if( 0 == bufferObj->size )
        {
                ...
                hDriver->touchData.touch[0].y != 0 )
            {
                ...
                hDriver->mostRecentTouchX = (int16_t)DRV_TOUCH_MTCH6303_TouchInputMap(hDriver->touchData.touch[0].x, DISP_HOR_RESOLUTION);
                hDriver->mostRecentTouchY = (int16_t)DRV_TOUCH_MTCH6303_TouchInputMap(hDriver->touchData.touch[0].y, DISP_VER_RESOLUTION);
            }
            else
            {
