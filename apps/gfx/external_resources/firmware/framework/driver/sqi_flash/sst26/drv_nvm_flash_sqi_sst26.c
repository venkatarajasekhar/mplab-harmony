...
// include files
#include <string.h>
#include "driver/sqi_flash/sst26/drv_nvm_flash_sqi_sst26.h"

#ifdef SST26VF_B
   // #warning "SST26VF_B is defined. Please remove it if you are using SST26VFxxxA part"
#else
    #warning "Please define SST26VF_B in your compiler preprocessor options if you are using SST26VFxxxB part"
#endif
...
uint8_t SST26ReadByte(uint32_t address)
{
    ...
    PLIB_SQI_ControlWordSet(SQI_ID_0, 1, SQI_CS_1, SQI_LANE_QUAD, SQI_CMD_TRANSMIT, 1); // NOP
    PLIB_SQI_ControlWordSet(SQI_ID_0, 0, SQI_CS_1, SQI_LANE_QUAD, SQI_CMD_TRANSMIT, 1); // FAST_READ
    PLIB_SQI_ControlWordSet(SQI_ID_0, 0, SQI_CS_1, SQI_LANE_QUAD, SQI_CMD_TRANSMIT, 4); // address + dummy byte
#endif
...
#else
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF_FAST_READ << 24) | (SST26VF_NOP << 16) | (SST26VF_NOP << 8) | SST26VF_NOP); // Nop, Nop, Nop, FAST READ
    PLIB_SQI_TransmitData(SQI_ID_0, (DUMMY_BYTE << 24) | address); // Address + Dummy Byte
#endif
...
#else
    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF_RDSR << 24) | (SST26VF_NOP << 16) | (SST26VF_NOP << 8) | SST26VF_NOP);
#endif
...
#else
        // set the commands: FAST READ + ADDRESS + DUMMY BYTE
        sqiCmdBuffer[0] = (uint8_t) SST26VF_FAST_READ;
        sqiCmdBuffer[1] = (uint8_t) (flashAddress >> 16);
        sqiCmdBuffer[2] = (uint8_t) (flashAddress >> 8);
        sqiCmdBuffer[3] = (uint8_t) (flashAddress);
        sqiCmdBuffer[4] = (uint8_t) DUMMY_BYTE; /* dummy byte */
#endif
...
void SST26ChipErase(void)
{
    ...
    PLIB_SQI_ControlWordSet(SQI_ID_0, 0, SQI_CS_1, SQI_LANE_QUAD, SQI_CMD_TRANSMIT, 20);     /* block unprotect bits */

    PLIB_SQI_TransmitData(SQI_ID_0, (SST26VF_WBPR << 24) | (SST26VF_WEN << 16) | (SST26VF_NOP << 8) | SST26VF_NOP);
    PLIB_SQI_TransmitData(SQI_ID_0, DUMMY_4_BYTES);
    PLIB_SQI_TransmitData(SQI_ID_0, DUMMY_4_BYTES);
    PLIB_SQI_TransmitData(SQI_ID_0, DUMMY_4_BYTES);
    PLIB_SQI_TransmitData(SQI_ID_0, DUMMY_4_BYTES);
    PLIB_SQI_TransmitData(SQI_ID_0, DUMMY_4_BYTES);
