...
//              8 characters for the name, three for the extension, and 1 for the radix ('.')
...
//              an open file's FSFILE structure will be passed to any library function that will modify that file.
typedef struct
{
    ...
    char            name[FILE_NAME_SIZE_8P3];       // The short name of the file
		...
		unsigned short int utf16LFNlength;          // LFN length in terms of uint16_ts excluding the NULL uint16_t at the last.
	#endif
...
//              searches to be performed in the same directory for additional files that meet the specified criteria.
typedef struct
{
    ...
    unsigned long   timestamp;                      // The last modified time of the file that has been found (create time for directories)
		...
		unsigned short int utf16LFNfoundLength;     // LFN Found length in terms of uint16_ts including the NULL uint16_t at the last.
	#endif
...
/*******************************************************************
  ...
  Remarks:
    Only devices with a sector size of 512 uint8_ts are supported by the
    format function                      
  *******************************************************************/
...
/*********************************************************************************
  ...
  Description:
    ...
    cluster will be allocated to the file.  Finally, the new position and file size
    will be stored in the FSFILE object.  The parameters 'size' and 'n' indicate how 
    much data to write.  'Size' refers to the size of one object to write (in uint8_ts),
