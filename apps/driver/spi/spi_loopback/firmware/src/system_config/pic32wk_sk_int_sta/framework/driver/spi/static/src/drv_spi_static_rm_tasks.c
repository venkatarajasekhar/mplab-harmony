...
int32_t DRV_SPI0_MasterRMSend8BitISR( struct DRV_SPI_OBJ * pDrvObj )
{
...
int32_t DRV_SPI0_MasterRMReceive8BitISR( struct DRV_SPI_OBJ * pDrvObj )
{
    register DRV_SPI_JOB_OBJECT * currentJob = pDrvObj->currentJob;
...
int32_t DRV_SPI1_SlaveRMSend8BitISR( struct DRV_SPI_OBJ * pDrvObj )
{
    register DRV_SPI_JOB_OBJECT * currentJob = pDrvObj->currentJob;
    if (currentJob == NULL)
    {
    ...
    // Add in DMA stuff here
        ...
        PLIB_SPI_BufferWrite(SPI_ID_2, currentJob->txBuffer[currentJob->dataTxed]);
        currentJob->dataTxed++;
        currentJob->dataLeftToTx--;
...
int32_t DRV_SPI1_SlaveRMReceive8BitISR( struct DRV_SPI_OBJ * pDrvObj )
{
    register DRV_SPI_JOB_OBJECT * currentJob = pDrvObj->currentJob;
    ...
    // Put in DMA stuff here
        if (currentJob->dataLeftToRx != 0)
        {
            currentJob->rxBuffer[currentJob->dataRxed] = PLIB_SPI_BufferRead(SPI_ID_2);
            currentJob->dataRxed++;
        currentJob->dataLeftToRx--;
        }
        else
