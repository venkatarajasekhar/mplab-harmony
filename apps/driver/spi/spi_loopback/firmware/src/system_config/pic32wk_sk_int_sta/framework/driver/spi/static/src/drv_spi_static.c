...
void DRV_SPI0_Tasks ( void )
{
    ...
    DRV_SPI0_ISRMasterRM8BitTasks(&gDrvSPI0Obj);
}
...
SYS_MODULE_OBJ DRV_SPI1_Initialize(void)
{
    ...
    /* Baud rate selection */
    PLIB_SPI_BaudRateSet( SPI_ID_2 , SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_3), 1000000 );
...
{
    /* Call the respective task routine */
    DRV_SPI1_ISRSlaveRM8BitTasks(&gDrvSPI1Obj);
}
...
int32_t DRV_SPI1_ClientConfigure ( const DRV_SPI_CLIENT_DATA * cfgData  )
{
    ...
    if (cfgData->baudRate != 0)
    {
        ...
        PLIB_SPI_BaudRateSet (SPI_ID_2,
                              SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_3),
                              cfgData->baudRate);
    }
