...
int32_t DRV_SPI0_ISRMasterRM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        {
            DRV_SPI0_MasterRMSend8BitISR(dObj);
        }
        ...
        if (bytesLeft != 0)
        {
            DRV_SPI0_MasterRMReceive8BitISR(dObj);
...
int32_t DRV_SPI1_ISRSlaveRM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        {
            DRV_SPI1_SlaveRMReceive8BitISR(dObj);
        }
        if (currentJob->dataLeftToTx != 0)
        {
            DRV_SPI1_SlaveRMSend8BitISR(dObj);
        }
        else
        {
            dObj->txEnabled = false;
        ...
        if (dObj->currentJob != NULL)
        {
            ...
            /* Interrupts should immediately become active again if they're in a fired condition */
            if ((SYS_INT_SourceStatusGet(INT_SOURCE_SPI_2_RECEIVE)) || (SYS_INT_SourceStatusGet(INT_SOURCE_SPI_2_ERROR)))
            {
                /* Interrupt would fire again anyway so we should just go back to the start*/
                continueLoop = true;
                continue;
