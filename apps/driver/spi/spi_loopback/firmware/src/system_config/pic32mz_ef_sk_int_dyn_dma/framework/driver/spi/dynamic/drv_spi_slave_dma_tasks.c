...
#include <stdbool.h>
    ...
    void DRV_SPI_SlaveDMASendData8BitISR(SYS_DMA_TRANSFER_EVENT event, SYS_DMA_CHANNEL_HANDLE handle, uintptr_t contextHandle)
{
...
void DRV_SPI_ISRDMASlaveSendEventHandler8bit(SYS_DMA_TRANSFER_EVENT event, SYS_DMA_CHANNEL_HANDLE handle, uintptr_t contextHandle)
{
    ...
    switch(currentJob->txDMAProgressStage)
    {
        case DRV_SPI_DMA_DATA_INPROGRESS:
            DRV_SPI_SlaveDMASendData8BitISR(event, handle, contextHandle);
            break;
        default:
            break;
    }
}
void DRV_SPI_SlaveDMAReceiveDummy8BitISR(SYS_DMA_TRANSFER_EVENT event, SYS_DMA_CHANNEL_HANDLE handle, uintptr_t contextHandle)
{
    if (event != SYS_DMA_TRANSFER_EVENT_COMPLETE)
    {
        // Ignore for now
...
void DRV_SPI_SlaveDMAReceiveData8BitISR(SYS_DMA_TRANSFER_EVENT event, SYS_DMA_CHANNEL_HANDLE handle, uintptr_t contextHandle)
{
    if (event != SYS_DMA_TRANSFER_EVENT_COMPLETE)
    {
        // Ignore for now
    ...
    {
        DRV_SPI_SlaveDMAReceiveDummy8BitISR(event,  handle,  contextHandle);
    }
}
...
void DRV_SPI_ISRDMASlaveReceiveEventHandler8bit(SYS_DMA_TRANSFER_EVENT event, SYS_DMA_CHANNEL_HANDLE handle, uintptr_t contextHandle)
{
    ...
    {
        case DRV_SPI_DMA_DATA_INPROGRESS:
            DRV_SPI_SlaveDMAReceiveData8BitISR(event, handle, contextHandle);
            break;
