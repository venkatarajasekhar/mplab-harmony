...
void APP4_Tasks ( void )
{
    ...
    switch ( app4Data.state )
    {
        case APP4_STATE_INIT:
            ...
            {
                SYS_CONSOLE_MESSAGE("Cli1: Created UDP client commands!\r\n");
                app4Data.state = APP4_STATE_RUN;
            }
            else
            {
                SYS_CONSOLE_MESSAGE("Cli1: Failed to create UDP client commands!\r\n");
                return;
            }
...
static int appClientOpen(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv)
{
    ...
    if(appSkt == INVALID_SOCKET)
    {
        ...
        if(appSkt == INVALID_SOCKET)
        {
            (*pCmdIO->pCmdApi->msg)(pCmdIO->cmdIoParam, "Cli1: Failed to Open Client Socket\r\n");
        }
        else
        {
            if(appUdpTxSize != 0)
            ...
            (*pCmdIO->pCmdApi->print)(pCmdIO->cmdIoParam, "Cli1: Opened Client Socket %d to %s\r\n", appSkt, hostString ? appIpv4RemHost : "no host");
...
static int appClientMsg(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv)
{
    static const char* appMsg = "Message from Client Cli1: %d\r\n";
