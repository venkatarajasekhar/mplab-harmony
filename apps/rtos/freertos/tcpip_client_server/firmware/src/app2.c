...
void APP2_Tasks ( void )
{
    ...
    switch ( app2Data.state )
    {
        case APP2_STATE_INIT:
            ...
            {
                SYS_CONSOLE_MESSAGE("TCli1: Created TCP client commands!\r\n");
                app2Data.state = APP2_STATE_RUN;
            }
            else
            {
                SYS_CONSOLE_MESSAGE("TCli1: Failed to create TCP client commands!\r\n");
                return;
            }
        ...
        case APP2_STATE_RUN:
            ...
            if(TCPIP_TCP_WasReset(appSkt))
            {
                if(!appResetReported)
                {
                    ...
                    {
                        (*appDisplayCmdNode->pCmdApi->msg)(appDisplayIoParam, "TCli1: Socket Reset Reported!\r\n");
                    }
            ...
            else if(!appConnectReported)
            {
                if(appDisplayCmdNode)
                {
                    (*appDisplayCmdNode->pCmdApi->msg)(appDisplayIoParam, "TCli1: TCP Socket Connected!\r\n");
                }
...
static int appClientOpen(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv)
{
    ...
    if(appSkt == INVALID_SOCKET)
    {
        ...
        if(appSkt == INVALID_SOCKET)
        {
            (*pCmdIO->pCmdApi->msg)(pCmdIO->cmdIoParam, "TCli1: Failed to Open Client Socket\r\n");
        }
        else
        {
            (*pCmdIO->pCmdApi->print)(pCmdIO->cmdIoParam, "TCli1: Opened Client Socket %d to %s\r\n", appSkt, hostString ? appIpv4RemHost : "no host");
            if(TCPIP_TCP_WasReset(appSkt) == false)
            {
                (*pCmdIO->pCmdApi->msg)(pCmdIO->cmdIoParam, "TCli1: Socket Failed to report ini Disconnect...\r\n");
            }
...
static int appClientMsg(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv)
{
    static const char* appMsg = "Message from Client TCli1: %d\r\n";
