...
void APP1_Tasks ( void )
{
    ...
    switch(app1Data.state)
    {
        case APP1_STATE_INIT:
            ...
            {
                SYS_CONSOLE_MESSAGE("TSrv1: Created TCP server commands!\r\n");
            ...
            {
                SYS_CONSOLE_MESSAGE("TSrv1: Failed to create TCP server commands!\r\n");
                return;
            }
...
static int appServerOpen(SYS_CMD_DEVICE_NODE* pCmdIO, int argc, char** argv)
{
    if(appSkt == INVALID_SOCKET)
    {
        ...
        {
            (*pCmdIO->pCmdApi->print)(pCmdIO->cmdIoParam, "Srv: Opened Socket %d\r\n", appSkt);
