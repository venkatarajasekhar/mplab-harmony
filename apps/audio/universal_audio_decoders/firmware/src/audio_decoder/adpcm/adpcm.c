...
static ADPCMHEADER adpcm_header;
...
{
    return adpcm_header.format;

}
int ADPCM_HdrGetNumOfChan(void)
{
    return adpcm_header.numOfChan;
}
...
{
    return adpcm_header.samplesPerSec;
}

int ADPCM_HdrGetBlockAlign(void)
{
   return adpcm_header.blockAlign;
}

int ADPCM_HdrGetBitsPerSample(void)
{
    return adpcm_header.bitsPerSample;

}
int ADPCM_HdrGetBytesPerSec(void)
{
    return adpcm_header.bytesPerSec;

}
int ADPCM_HdrGetDataLen(void)
{
    return (int) adpcm_header.dataLen;
}

unsigned int ADPCM_HdrGetFileSize(void)
{
    return (unsigned int) adpcm_header.extralen;
}


void ADPCM_Initialize(uint8_t *input)
{
    memcpy(&adpcm_header, input, ADPCM_HEADER_SIZE);
...
//    if(adpcm_header.numOfChan == 1){
//        // convert to stereo
////        convertMono2Stereo(tempOutPtr, inSize*2);
//        *written = ADPCM_OUTPUT_BUFFER_SIZE;
//    }else{ //stereo channel
//        *written = ADPCM_OUTPUT_BUFFER_SIZE;
//    }
//	else
