    ...
    switch(appData.state)
    {
        ...
        case APP_SUBMIT_INITIAL_USB_READ_REQUEST:
        {
                     ...
                     APP_USB_SPEAKER_PLAYBACK_STEREO_16KHZ)
            {
                ...
                //for(i = 0;i < (APP_PLAYBACK_QUEUE_BUFFER/4 + 8);i++)
                for(i = 0;i < APP_PLAYBACK_QUEUE_BUFFER;i++)
                {
        ...
        {    
            ...
            {
                for (i = 0;
                     i < APP_PLAYBACK_QUEUE_BUFFER/QUEUE_USB_INIT_PART; i++)
                {
        ...
        case APP_SUBMIT_INITIAL_CODEC_READ_REQUEST:
        {
                    ...
                    APP_USB_SPEAKER_PLAYBACK_STEREO_16KHZ)
            {
                ...
                DRV_CODEC_BufferAddRead(appData.codecClientRead.handle, &appData.codecClientRead.writeBufHandle1,
                appData.codecClientRead.txbufferObject1, appData.codecClientRead.bufferSize); //int16 values
            
                DRV_CODEC_BufferAddRead(appData.codecClientRead.handle, &appData.codecClientRead.writeBufHandle2,
                appData.codecClientRead.txbufferObject2, appData.codecClientRead.bufferSize); //int16 values
        ...
        case DRV_CODEC_BUFFER_EVENT_COMPLETE:
        {
                 ...
                 !current->codecInUse)
            {
                ...
                DRV_CODEC_BufferAddWrite(appData.codecClientWrite.handle, 
                                        ...
                                        current->buffer, 
                                        appData.codecClientWrite.bufferSize);
//                DRV_CODEC_BufferAddWrite(appData.codecClientWrite.handle, &current->writeHandle,
//                    testBuffer1, 128);
...
static bool _APP_USBReadAllBufferReady()
{
    ...
    for (i=0; i<APP_PLAYBACK_QUEUE_BUFFER/QUEUE_USB_INIT_PART; i++)
    {
...
static uint8_t _APP_GetNextIdx(uint8_t index)
{
    return (index+1)%APP_PLAYBACK_QUEUE_BUFFER;
}
