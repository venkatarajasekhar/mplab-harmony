...
void SYS_PORTS_Initialize(void)
{
    ...
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_CN, INT_PRIORITY_LEVEL5);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_CN, INT_SUBPRIORITY_LEVEL0);

    /* PPS Input Remapping */

    /* PPS Output Remapping */
    /* KEEP just these three lines for now, MHC seems to insist on generating
     * multiple calls to ReMapOutput for each pin
     */
    PLIB_PORTS_RemapOutput(PORTS_ID_0, OUTPUT_FUNC_REFCLKO, OUTPUT_PIN_RPF8 );
    PLIB_PORTS_RemapOutput(PORTS_ID_0, OUTPUT_FUNC_SS1, OUTPUT_PIN_RPD9 );
    PLIB_PORTS_RemapOutput(PORTS_ID_0, OUTPUT_FUNC_SDO1, OUTPUT_PIN_RPD0 );
