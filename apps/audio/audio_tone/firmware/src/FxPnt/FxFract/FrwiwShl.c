 ...
 *
 ******************************************************************************/
Fract32 FrwiwShl(Fract32 a, int16_t b)
{
  Fract32 result;                   /* Value returned */
  Fract32 temp;    /* Temp var for Shifted Sign compare results */
  ...
  if (b > 0)
  {
    ...
    {
      result = (a < 0) ? (Fract32)MINFRACT32: (Fract32)MAXFRACT32;
    }
    ...
    {
      temp = (((Fract32)MSBBITFRACT32 >> b) & (Fract32)a);
      if ((a > 0) && (temp != 0))
      {
        result = (Fract32)MAXFRACT32;
      }
      if ((a < 0) && (temp != ((Fract32)MSBBITFRACT32 >> b)))
      {
        result = (Fract32)MINFRACT32;
      }
    }
  }
  ...
  else
  {
    if (b != MININT16)
    {
      result = FrwiwShr(a,(int16_t)(-b));
    }
    else
    {
      result = FrwiwShr(a,(int16_t)(MAXINT16));
    }
  }
  return (result);
}
