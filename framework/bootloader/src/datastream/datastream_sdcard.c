...
DRV_HANDLE sdCardHandle = DRV_HANDLE_INVALID;
...
/*******************************************************
 * SD Card MSD Layer Events - Application Event Handler
 *******************************************************/
void Bootloader_SDCardMSDEventHandler(SYS_FS_EVENT event, void * eventData, uintptr_t context)
{
    switch ( event)
    {
...
)
{
    ...
    * move on to the next state */
    SYS_FS_EventHandlerSet(Bootloader_SDCardMSDEventHandler, (uintptr_t)NULL);
}

DRV_HANDLE DATASTREAM_Open(const DRV_IO_INTENT ioIntent)
{
    return 0;
    //return DRV_SDCARD_Open(DRV_SDCARD_INDEX_0, ioIntent);
}

DRV_CLIENT_STATUS DATASTREAM_ClientStatus(DRV_HANDLE handle)
{
    return DRV_CLIENT_STATUS_READY;
    //return DRV_SDCARD_Status(handle);
}
...
{
   //No writing performed for SD Card
    return(0);
}

void DATASTREAM_Close(void)
{
    if (sdCardHandle != DRV_HANDLE_INVALID)
    {
#if defined(DRV_SDCARD_Close)
        DRV_SDCARD_Close(sdCardHandle);
#endif
#if defined(DRV_SDHC_Close)
        DRV_SDHC_Close(sdCardHandle);
#endif
    }
    ...
    SYS_INT_SourceDisable(DRV_SPI_TX_INT_SOURCE_IDX0);
    SYS_INT_SourceDisable(DRV_SPI_RX_INT_SOURCE_IDX0);
    SYS_INT_SourceDisable(DRV_SPI_ERROR_INT_SOURCE_IDX0);
    SYS_INT_VectorPrioritySet(DRV_SPI_TX_INT_VECTOR_IDX0, INT_DISABLE_INTERRUPT);
    SYS_INT_VectorSubprioritySet(DRV_SPI_TX_INT_VECTOR_IDX0, INT_SUBPRIORITY_LEVEL0);
    SYS_INT_VectorPrioritySet(DRV_SPI_RX_INT_VECTOR_IDX0, INT_DISABLE_INTERRUPT);
    SYS_INT_VectorSubprioritySet(DRV_SPI_RX_INT_VECTOR_IDX0, INT_SUBPRIORITY_LEVEL0);
    SYS_INT_VectorPrioritySet(DRV_DRV_SPI_ERROR_INT_VECTOR_IDX0, INT_DISABLE_INTERRUPT);
    SYS_INT_VectorSubprioritySet(DRV_DRV_SPI_ERROR_INT_VECTOR_IDX0, INT_SUBPRIORITY_LEVEL0);
