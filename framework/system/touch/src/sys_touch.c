...
void SYS_TOUCH_TouchMsgGet(SYS_MODULE_INDEX moduleIndex)
{
    ...
    int16_t           x, y;
    DRV_TOUCH_PEN_STATE pen = DRV_TOUCH_PEN_UP;
    ...
    if ( SYS_MSG_QueueStatus( moduleIndex, 0 ) == SYS_MSG_QUEUE_FULL )
    {
        return;
    }
    ...
    if ( dObj->driverInitData->drvTouchPenGet )
    {
            pen = dObj->driverInitData->drvTouchPenGet(0);
    }
