...
{
    /* what happened in that call to register */
    ...
    /* how to find out what the state is */
  ...
  uintptr_t context)
{
    ...
    if (callback == NULL)
    {
        ...
        SysRtccObject.context = NULL;
        SysRtccObject.status = SYS_RTCC_STATUS_OK;
        return SYS_RTCC_ALARM_HANDLE_INVALID;
    }

    /* save our callback and context in our local memory */
    ...
    return (SYS_RTCC_ALARM_HANDLE)&SysRtccObject;
}
...
/* 4bits each: [hour10][hour1][mins10][mins1][secs10][secs1][xxxx][xxxx] */
...
union BCDConversion
{
    ...
    struct
    {
        uint32_t padding:8;     /* add pad. compiler starts bitfields at lsb */
        uint32_t  secs1 :4;
        uint32_t  secs10:4;
        uint32_t  mins1 :4;
        uint32_t  mins10:4;
...
uint32_t TimeBCD2Seconds(SYS_RTCC_BCD_TIME TimeInBCD)
{
    ...
    union BCDConversion conversion;
...
{
    SYS_RTCC_INIT *sysRtccInit = (SYS_RTCC_INIT *)init;
    ...
    PLIB_RTCC_RTCTimeSet(RTCC_PLIB_ID, sysRtccInit->rtccTime); /* Set RTCC time */
    PLIB_RTCC_RTCDateSet(RTCC_PLIB_ID, sysRtccInit->rtccDate); /* Set RTCC date */
    
    PLIB_RTCC_AlarmDisable(RTCC_PLIB_ID); /* Disable alarm */
    while (PLIB_RTCC_AlarmSyncStatusGet(RTCC_PLIB_ID))  ; /* Wait for disable */
    PLIB_RTCC_AlarmTimeSet(RTCC_PLIB_ID, sysRtccInit->alarmTime);
    PLIB_RTCC_AlarmDateSet(RTCC_PLIB_ID, sysRtccInit->alarmDate);

    /* repeat forever or 0-255 times */
    if (sysRtccInit->alarmRepeatForever)
    {
    ...
    else
    {
        ...
        PLIB_RTCC_AlarmRepeatCountSet(RTCC_PLIB_ID, sysRtccInit->alarmRepeatCount);
    }

    /* enum here to select the alarm mask */
    PLIB_RTCC_AlarmMaskModeSelect(RTCC_PLIB_ID, sysRtccInit->alarmMask);
    
    /* Initialize the output */
    if (sysRtccInit->outputEnabled)
    {
         /* Select ouput to be placed on output pin */
        if (PLIB_RTCC_ExistsOutputSelect(RTCC_PLIB_ID))
        {
            PLIB_RTCC_OutputSelect(RTCC_PLIB_ID, sysRtccInit->outputSelect);
        }
    ...
    if (PLIB_RTCC_ExistsClockSelect(RTCC_PLIB_ID))
    {
        PLIB_RTCC_ClockSourceSelect(RTCC_PLIB_ID,sysRtccInit->clockSource);
    }

    /* Setup RTCC Interrupt */
    if (sysRtccInit->interruptUsed)
    {
        PLIB_INT_SourceEnable(INT_ID_0, sysRtccInit->interruptSource);
        PLIB_INT_VectorPrioritySet(INT_ID_0, sysRtccInit->interruptVector, sysRtccInit->interruptPriority);
        PLIB_INT_VectorSubPrioritySet(INT_ID_0,sysRtccInit->interruptVector, sysRtccInit->interruptSubpriority);
        SysRtccObject.interruptSource = sysRtccInit->interruptSource;
    }
    else
    {
