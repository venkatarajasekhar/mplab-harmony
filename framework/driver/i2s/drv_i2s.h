/*******************************************************************************
  ...
  Description:
    The I2S device driver provides a simple interface to manage the I2S
    module on Microchip microcontrollers.  This file provides the
...
/* Driver I2S Module Index
  ...
  Remarks:
    ...
    These values should be passed into the DRV_I2S_Initialize and
...
/* I2S Driver Write Error
  ...
  Description:
    This constant is returned by DRV_I2S_Write() function when an error
    occurs.
...
/* I2S Driver Read Error
  ...
  Description:
    This constant is returned by DRV_I2S_Read function when an error
    occurs.
...
/* I2S Driver Buffer Handle
  ...
  Description:
    ...
    buffer.  The buffer handle value returned from the "buffer add" function
...
/* I2S Usage Modes Enumeration
  ...
  Description:
    This enumeration identifies the whether the I2S module will be used as
    a master or slave.
...
/* I2S Driver Events
   ...
   Description
    ...
    DRV_I2S_BufferAddRead, DRV_I2S_BufferAddWrite or
    DRV_I2S_BufferAddWriteRead functions.
...
/* I2S Driver Buffer Event Handler Function
  ...
  Example:
   ...
   Remarks:
    ...
    If the event is DRV_I2S_BUFFER_EVENT_COMPLETE, this means that the data
    was transferred successfully.

    If the event is DRV_I2S_BUFFER_EVENT_ERROR, this means that the data was
    ...
    called to know the error. The DRV_I2S_BufferProcessedSizeGet function
    ...
    DRV_I2S_BufferAddWriteRead functions can be called in the event handler
    to add a buffer to the driver queue. These functions can only be called
    to add buffers to the driver whose event handler is running. For example,
    buffers cannot be added I2S2 driver in I2S1 driver event handler.

*/
typedef void (*DRV_I2S_BUFFER_EVENT_HANDLER) (DRV_I2S_BUFFER_EVENT event,
...
Example:
  ...
  Remarks:
    ...
    unless DRV_I2S_Deinitialize is called to deinitialize the driver
    ...
    /'dmaChannelReceive' must be set to valid DMA channel.
    When DMA mode of operation is enabled, the normal mode(Usual TX and RX)
    operation is inhibited.
    When 'dmaChannelTransmit'/'dmaChannelReceive' are set to valid channel
    numbers the related DMA interrupt source parameters 'dmaInterruptTransmitSource'/
    'dmaInterruptReceiveSource' must be set with appropriate DMA channel interrupt source.
