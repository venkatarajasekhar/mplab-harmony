...
#ifndef _DRV_I2S_DMA_LOCAL_H
#define _DRV_I2S_DMA_LOCAL_H
...
// *****************************************************************************
/* I2S DMA transfer abort

  Summary:
    I2S DMA transfer abort

  Description:
    This constant indicates that the I2S DMA transfer is aborted.

  Remarks:
    None.
*/
#define DRV_I2S_DMA_TRANSFER_ABORT /*DOM-IGNORE-BEGIN*/((uint32_t)(-1))/*DOM-IGNORE-END*/
...
/* I2S Driver task buffer operations mode
  ...
  Description
    This enumeration lists the different modes that SPI task routine can have.
...
typedef struct _DRV_I2S_OBJ_STRUCT
{
   ...
   /* This is the transmit buffer current queue size. It
    ...
    /* DMA write chaining channel handle */
    SYS_DMA_CHANNEL_HANDLE  dmaChaningChannelHandleWrite;

    /* DMA read channel handle */
    SYS_DMA_CHANNEL_HANDLE  dmaChannelHandleRead;
    /* DMA read chaining channel handle */
    SYS_DMA_CHANNEL_HANDLE  dmaChaningChannelHandleRead;

    /* This is the I2S transmit DMA channel interrupt.*/
    INT_SOURCE dmaInterruptTransmitSource;
    /* This is the I2S transmit DMA Chaining channel interrupt.*/
    INT_SOURCE dmaInterruptTransmitChainingSource;

    /* This is the I2S receive DMA channel interrupt. */
    INT_SOURCE dmaInterruptReceiveSource;
    /* This is the I2S receive DMA Chaining channel interrupt. */
    INT_SOURCE dmaInterruptReceiveChainingSource;
    ...
    uint32_t readQueuedCount;
    uint32_t writeQueuedCount;
    uint32_t writeReadQueuedCount;
    bool txChannelPingInUse;
    bool txChannelPongInUse;
    bool rxChannelPingInUse;
    bool rxChannelPongInUse;    

    /* SPI module source/destination size */
    size_t srcDestSize;
