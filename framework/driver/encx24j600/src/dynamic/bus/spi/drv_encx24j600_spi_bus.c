...
static void DRV_ENCX24J600_SPI_OperationStarting (DRV_SPI_BUFFER_EVENT event, DRV_SPI_BUFFER_HANDLE bufferHandle, void * context )
{
    struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance = (struct _DRV_ENCX24J600_DriverInfo *)context;
...
static void DRV_ENCX24J600_SPI_OperationEnded (DRV_SPI_BUFFER_EVENT event, DRV_SPI_BUFFER_HANDLE bufferHandle, void * context )
{
    struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance = (struct _DRV_ENCX24J600_DriverInfo *)context;
...
static void DRV_ENCX24J600_SPI_OperationComplete (DRV_SPI_BUFFER_EVENT event, DRV_SPI_BUFFER_HANDLE bufferHandle, void * context )
{
...
int32_t DRV_ENCX24J600_SPI_OpenInterface(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance )
{
    ...
    DRV_SPI_CLIENT_DATA spiClientInfo= {
        .baudRate = pDrvInstance->drvCfg.spiBps,
        .operationStarting = DRV_ENCX24J600_SPI_OperationStarting,
        .operationEnded = DRV_ENCX24J600_SPI_OperationEnded,
    };
    DRV_SPI_ClientConfigure(pBusInfo->clientHandle, &spiClientInfo);
    PLIB_PORTS_PinDirectionOutputSet(pDrvInstance->drvCfg.spiSSPortModule,
                 pDrvInstance->drvCfg.spiSSPortChannel,
                 pDrvInstance->drvCfg.spiSSPortPin);
    PLIB_PORTS_PinSet(pDrvInstance->drvCfg.spiSSPortModule,
                 pDrvInstance->drvCfg.spiSSPortChannel,
                 pDrvInstance->drvCfg.spiSSPortPin);
    return 0;
}
...
{
    DRV_SPI_BUFFER_EVENT res = DRV_SPI_BufferStatus(handle);
    switch (res)
    {
...
uintptr_t DRV_ENCX24J600_SPI_SfrWrite(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_SFR_MAP  reg, DRV_ENCX24J600_RegUnion  value,  uint8_t  opIndex )
{
    ...
    if (bankSelectResult == 4)
    {
        ...
        pBusInfo->bufferHandles[opIndex] = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &(pBusInfo->commandWrBuffers[opIndex]), 4, NULL, pDrvInstance);
    ...
    pBusInfo->bufferHandles[opIndex] = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &(pBusInfo->commandWrBuffers[opIndex][1]), 3, NULL, pDrvInstance);
    return pBusInfo->bufferHandles[opIndex];
}
...
uintptr_t DRV_ENCX24J600_SPI_SfrReadStart(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_SFR_MAP  reg, uint8_t  opIndex )
{
    ...
    if (bankSelectResult == 4)
    {
        ...
        pBusInfo->commandWrBuffers[opIndex][1] = sfr;
        pBusInfo->bufferHandles[opIndex] = DRV_SPI_BufferAddWriteRead(pBusInfo->clientHandle, &(pBusInfo->commandWrBuffers[opIndex]), 4, &(pBusInfo->commandRdBuffers[opIndex]), 4, NULL, pDrvInstance);
        return pBusInfo->bufferHandles[opIndex];
    }
    ...
    pBusInfo->bufferHandles[opIndex] = DRV_SPI_BufferAddWriteRead(pBusInfo->clientHandle, &(pBusInfo->commandWrBuffers[opIndex][1]), 3, &(pBusInfo->commandRdBuffers[opIndex][1]), 3, NULL, pDrvInstance);
    return pBusInfo->bufferHandles[opIndex];
}
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &rstCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &erxCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &drxCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &rptCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &dpcCmd, 1, NULL, pDrvInstance);
 }
// *****************************************************************************
/* Enable Interrupts
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &eiCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &diCmd, 1, NULL, pDrvInstance);
 }
// *****************************************************************************
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &dfCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 uintptr_t DRV_ENCX24J600_SPI_FlowCtrlSingle(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance )
  {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &sfCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &mfCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &cfCmd, 1, NULL, pDrvInstance);
 }
...
*/
 ...
 {
     //DRV_ENCX24J600_spiBusData * pBusInfo = (DRV_ENCX24J600_spiBusData *)pDrvInstance->busData;
     DRV_ENCX24J600_RegUnion phyReg;
     phyReg.value = 0;
...
*/
 uintptr_t DRV_ENCX24J600_SPI_WritePointer(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_POINTER  reg, uint16_t  value, uint8_t  opIndex )
 {
     ...
     pBusInfo->bufferHandles[opIndex] = DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &(pBusInfo->commandWrBuffers[opIndex]), 3, NULL, pDrvInstance);
     return pBusInfo->bufferHandles[opIndex];
...
*/
 uintptr_t DRV_ENCX24J600_SPI_ReadPointerStart(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_POINTER  reg, uint8_t  opIndex )
 {
     ...
     pBusInfo->bufferHandles[opIndex] = DRV_SPI_BufferAddWriteRead(pBusInfo->clientHandle, &(pBusInfo->commandWrBuffers[opIndex]), 3, &(pBusInfo->commandRdBuffers[opIndex]), 3, NULL, pDrvInstance);
     return pBusInfo->bufferHandles[opIndex];
...
*/
 uintptr_t DRV_ENCX24J600_SPI_WriteData(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_POINTER  reg, uint8_t *  buffer, uint16_t  dataSize)
 {
     ...
     return DRV_SPI_BufferAddWrite(pBusInfo->clientHandle, &(buffer[-1]), dataSize+1, DRV_ENCX24J600_SPI_OperationComplete, pDrvInstance);
 }
...
*/
 ...
 uintptr_t DRV_ENCX24J600_SPI_ReadData(struct _DRV_ENCX24J600_DriverInfo *  pDrvInstance, DRV_ENCX24J600_POINTER  reg, uint8_t *  buffer, uint16_t  dataSize)
 {
     ...
     return DRV_SPI_BufferAddWriteRead(pBusInfo->clientHandle, &rddCmd, 1, &(buffer[-1]), dataSize+1, DRV_ENCX24J600_SPI_OperationComplete, pDrvInstance);
