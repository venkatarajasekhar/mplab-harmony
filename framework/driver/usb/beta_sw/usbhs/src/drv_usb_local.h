...
#include "driver/usb/beta_sw/usbhs/src/drv_usb_variant_mapping.h"
#include "osal/osal.h"
...
#define DRV_USB_MAX_DMA_CHANNELS                8
...
typedef enum
{
    ...
    DRV_USB_TASK_STATE_STARTING_DELAY = 1,

    /* Driver has started the delay and is waiting for complete */
    DRV_USB_TASK_STATE_WAITING_FOR_DELAY_COMPLETE,

    /* Driver has complete initialization and can be opened */
    DRV_USB_TASK_STATE_RUNNING

} DRV_USB_TASK_STATE;

typedef struct _DRV_USB_DMA_POOL_STRUCT
{
    bool inUse;
    bool endpointDir;
    uint8_t iEndpoint;
...
typedef struct _DRV_USB_OBJ_STRUCT
{
    ...
    DRV_USB_DMA_POOL gDrvUSBDMAPool[DRV_USB_MAX_DMA_CHANNELS + 1];
    ...
    _DRV_USB_FOR_HOST(unsigned int, gDrvUSBFifoTable[5]);
    _DRV_USB_FOR_HOST(unsigned int, gDrvUSBFifoTableSecondary[5]);
} DRV_USB_OBJ;
