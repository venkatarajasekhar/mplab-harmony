...
/* SRAM Driver Invalid Command Handle.
  ...
  Description:
    ...
    returned by read/write/erase routines when the command request was not
    accepted.
...
typedef struct
{
    /* SRAM Media start address. The driver treats this address as 
     * block 0 address for read and write operations.
     * */
...
/* Function:
  ...
  Summary:
    Handles the read/write requests queued to the driver.
  
  Description:
    This routine is used to handle the read/write requests queued to the
    driver.
...
/* Function:
  ...
  Description:
    ...
    data from the memory. The function returns with a valid buffer handle in
    ...
    driver will issue a DRV_SRAM_EVENT_COMMAND_COMPLETE event if the buffer was
    processed successfully or DRV_SRAM_EVENT_COMMAND_ERROR event if the buffer
  ...
  Parameters:
    ...
    blockStart    - Start block address in SRAM memory from where the read
                    should begin.
  ...
  Example:
    ...
    uint32_t blockStart = SRAM_BASE_ADDRESS_TO_READ_FROM;
    uint32_t nBlock = 2;
    DRV_SRAM_COMMAND_HANDLE commandHandle;
    MY_APP_OBJ myAppObj;    
    ...
    void APP_SRAMEventHandler(DRV_SRAM_EVENT event, 
            DRV_SRAM_COMMAND_HANDLE commandHandle, uintptr_t contextHandle)
    {
...
/* Function:
  ...
  Description:
    ...
    returns DRV_SRAM_COMMAND_HANDLE_INVALID in the commandHandle argument under
  ...
  Parameters:
    ...
    blockStart    - Start block address of SRAM memory where the write should
                    begin. This address should be aligned on a block boundary.
  ...
  Example:
    ...
    // address should be block aligned.
    uint32_t blockStart = SRAM_BASE_ADDRESS_TO_WRITE_TO;
    uint32_t nBlock = 2;
    DRV_SRAM_COMMAND_HANDLE commandHandle;
    MY_APP_OBJ myAppObj;    
...
/* Function:
  ...
  Description:
    ...
    on. The function may return DRV_SRAM_COMMAND_HANDLE_INVALID in a case where
  ...
  Returns:
    A DRV_SRAM_COMMAND_STATUS value describing the current status of the command.
    Returns DRV_SRAM_COMMAND_HANDLE_INVALID if the client handle or the command
    handle is not valid.
...
/* Function:
  ...
  Precondition:
    The DRV_SRAM_Initialize() routine must have been called for the
    specified SRAM driver instance.
...
/* Function:
  ...
  Example:
    ...
    void APP_SRAMEventHandler(DRV_SRAM_EVENT event, 
            DRV_SRAM_COMMAND_HANDLE handle, uintptr_t context)
    {
...
/* Function:
  ...
  Example:
    ...
    // The SRAM media is always attached and so the below
    // always returns true.
