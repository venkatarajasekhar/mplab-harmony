...
)
{
    ...
    /* Validate the handle */
    if ((handle == NULL) || (handle == DRV_HANDLE_INVALID))
    {
        return NULL;
    }
...
)
{
    ...
    switch (dObj->subState)
    {
        ...
        default:
            {
                DRV_SQI_TransferElement *xferElement = dObj->transferElement;
                uint8_t *cmdParams = dObj->cmdParams;

                /* Reset Enable Command */
                cmdParams[0] = DRV_SST26_CMD_FLASH_RESET_ENABLE;
                xferElement[0].data = &cmdParams[0];
                xferElement[0].length = 1;
                xferElement[0].flag = (DRV_SQI_FLAG_MODE_SINGLE_LANE | DRV_SQI_FLAG_DEASSERT_CS);

                DRV_SQI_TransferData (dObj->sqiHandle, &dObj->cmdHandle, dObj->sqiDevice, xferElement, 1);
        ...
        case DRV_SST26_SUBTASK_RESET_ENABLE_DELAY:
            {
                if (SYS_TMR_DelayStatusGet(dObj->tmrHandle))
                {
                    ...
                    /* Reset Command */
                    cmdParams[0] = DRV_SST26_CMD_FLASH_RESET;
                    xferElement[0].data = &cmdParams[0];
                    xferElement[0].length = 1;
                    xferElement[0].flag = (DRV_SQI_FLAG_MODE_SINGLE_LANE | DRV_SQI_FLAG_DEASSERT_CS);
        ...
        default:
            {
                ...
                cmdParams[0] = DRV_SST26_CMD_ENABLE_QUAD_IO;
        ...
        default:
            {
                ...
                cmdParams[0] = DRV_SST26_CMD_WRITE_ENABLE;
                cmdParams[1] = DRV_SST26_CMD_UNPROTECT_GLOBAL;

                xferElement[0].data = &cmdParams[0];
                xferElement[0].length = 1;
                xferElement[0].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DEASSERT_CS);

                xferElement[1].data = &cmdParams[1];
                xferElement[1].length = 1;
                xferElement[1].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DEASSERT_CS);

                DRV_SQI_TransferData (dObj->sqiHandle, &dObj->cmdHandle, dObj->sqiDevice, xferElement, 2);
                if (dObj->cmdHandle == DRV_SST26_COMMAND_HANDLE_INVALID)
                {
                    /* Failed to queue the request. */
                    cmdStatus = DRV_SQI_COMMAND_ERROR_UNKNOWN;
        ...
        default:
            {
                ...
                cmdParams[0] = DRV_SST26_CMD_QUAD_JEDEC_ID_READ;
                xferElement[0].data = &cmdParams[0];
                xferElement[0].length = 2;
                xferElement[0].flag = DRV_SQI_FLAG_MODE_QUAD_LANE;

                /* Set up the buffer into which the ID is to be read. */
                xferElement[1].data = &dObj->flashId[0];
                xferElement[1].length = 3;
                xferElement[1].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DIR_READ | DRV_SQI_FLAG_DEASSERT_CS);
        ...
        default:
            {
                ...
                cmdParams[0] = DRV_SST26_CMD_HIGH_SPEED_READ;
                ...
                xferElement[0].data = &cmdParams[0];
                xferElement[0].length = 7;
                xferElement[0].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE);

                /* Read buffer Transfer Element */
                xferElement[1].data = data;
                xferElement[1].length = nBlocks;
                xferElement[1].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DIR_READ | DRV_SQI_FLAG_DEASSERT_CS);
        ...
        default:
            {
                ...
                cmdParams[0] = DRV_SST26_CMD_READ_STATUS_REG;
                /* Dummy Byte */
                cmdParams[1] = 0x00;
                ...
                /* Read Status data */
                xferElement[1].data = dObj->statusReg;
                xferElement[1].length = 1;
                xferElement[1].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DIR_READ | DRV_SQI_FLAG_DEASSERT_CS);
        ...
        default:
            {
                ...
                /* Convert from page to byte address */
                address <<= 0x08;
                cmdParams[0] = DRV_SST26_CMD_WRITE_ENABLE;
                cmdParams[1] = DRV_SST26_CMD_PAGE_PROGRAM;
                cmdParams[2] = (address >> 16) & 0xFF;
                cmdParams[3] = (address >> 8) & 0xFF;
                cmdParams[4] = address & 0xFF;
                ...
                /* QUAD Page Program command and address */
                xferElement[1].data = &cmdParams[1];
                xferElement[1].length = 4;
                xferElement[1].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE);

                /* Page Program Data */
                xferElement[2].data = data;
                xferElement[2].length = 256;
                xferElement[2].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DEASSERT_CS);

                
                DRV_SQI_TransferData (dObj->sqiHandle, &dObj->cmdHandle, dObj->sqiDevice, xferElement, 3);
                if (dObj->cmdHandle == DRV_SQI_COMMAND_HANDLE_INVALID)
                {
                    cmdStatus = DRV_SQI_COMMAND_ERROR_UNKNOWN;
                }
        ...
        default:
            {
                ...
                cmdParams[0] = DRV_SST26_CMD_WRITE_ENABLE;
                cmdParams[1] = DRV_SST26_CMD_SECTOR_ERASE;
                cmdParams[2] = (blockAddress >> 16) & 0xFF;
                cmdParams[3] = (blockAddress >> 8) & 0xFF;
                cmdParams[4] = blockAddress & 0xFF;
                ...
                /* Sector Erase command and address */
                xferElement[1].data = &cmdParams[1];
                xferElement[1].length = 4;
                xferElement[1].flag = (DRV_SQI_FLAG_MODE_QUAD_LANE | DRV_SQI_FLAG_DEASSERT_CS);
...
)
{
    ...
    switch (dObj->state)
    {
        case DRV_SST26_OPEN_SQI:
            {
                /* TODO: There is only one instance of the SQI. */
