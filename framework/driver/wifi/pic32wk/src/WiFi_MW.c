...
// Command received from UART or external host can be
bool wifi_command_process(int argc,char *argv[])
{
    ...
    static bool user_regdom_enable = 0;
#endif
...
#ifdef WLAN_REGDOMAIN_DEBUG     
    } else if((argc == 2)&& (!strcmp((char*)argv[1],"openregdom"))) {
       WIFI_IPF_regdomconfig_memalloc();
       WIFI_IPF_calregion_unprotect();
       user_regdom_enable = 1;
...
#ifdef WLAN_REGDOMAIN_DEBUG
     } else if((argc == 2)&& (!strcmp((char*)argv[1],"closeregdom"))) {    
        WIFI_IPF_regdomconfig_memfree();
        WIFI_IPF_calregion_protect();
		user_regdom_enable = 0;
#endif 		
    }  
    else if((argc >= 4)&&(user_config_enable) && (!strcmp((char*)argv[1],"set")))
    {
		...
		else if(!strcmp("wid",argv[2]))
		{
            ...
            ret = DRV_WIFI_Write(wid,strtoul( argv[4], 0, 10));
            else 
            ret = DRV_WIFI_Write(wid,(unsigned int)argv[4]);    
            return 0;
		}
        else
            printf("\nWrong input values");
    ...
    else if((argc >= 3)&& (user_config_enable) && (!strcmp((char*)argv[1],"get")))
    {
       ...
       else if((argc >= 3)&& (user_config_enable || user_regdom_enable) && (!strcmp((char*)argv[1],"save")))
#else
       else if((argc >= 3)&& (user_config_enable ) && (!strcmp((char*)argv[1],"save")))
#endif 
    {
                /*Save the configuration to FLASH*/
    ...
    else if((argc >= 3)&& (user_config_enable) && (!strcmp((char*)argv[1],"apply")))
    {
        ...
        if(g_wifiConfigData->ssid != NULL)
        {
            ...
            ret = DRV_WIFI_Write(DRV_WIFI_WID_SSID,(unsigned int)g_wifiConfigData->ssid);
        }
        if(g_wifiConfigData->SecurityKeyLength != 0)
        {
            ...
            ret = DRV_WIFI_Write(DRV_WIFI_WID_11I_PSK,(unsigned int)g_wifiConfigData->key);
            ret = DRV_WIFI_Write(DRV_WIFI_WID_11I_MODE,(unsigned int) g_wifiConfigData->cipher);    
            ret = DRV_WIFI_Write(DRV_WIFI_WID_AUTH_TYPE, 0);
            ret = DRV_WIFI_Write(DRV_WIFI_WID_11G_OPERATING_MODE, 2);
            ret = DRV_WIFI_Write(DRV_WIFI_WID_ACK_POLICY, 0);
            ret = DRV_WIFI_Write(DRV_WIFI_WID_11N_ENABLE, 1);            
        }
        if(ret == 0)
        {
        ...
        ret = DRV_WIFI_Write(DRV_WIFI_WID_START_SCAN_REQ,1);
    }
    ...
    else if((argc >=3) && (!strcmp((char*)argv[1],"ccath")))
    {
         if((argv[2]) && (argv[3]) && (argv[4]) && (!strcmp((char*)argv[2],"set")))
        {
            ...
            DRV_WIFI_Write(DRV_WIFI_WID_CCA_THRESHOLD,(((busy_th&0x01FF)<<15)|(clear_th & 0x01FF)));
    ...
    else if((argc ==3) && (!strcmp((char*)argv[1],"--dgchigh")))
    {
        ...
        DRV_WIFI_Write(DRV_WIFI_WID_DGC_RSSI_TH_HIGH,val);
     }
    else if((argc ==3) && (!strcmp((char*)argv[1],"--dgclow")))
    {
        int val = strtoul( argv[2], 0, 10);
        DRV_WIFI_Write(DRV_WIFI_WID_DGC_RSSI_TH_LOW,val);
    }
    ...
    else if((argc >=3) && (!strcmp((char*)argv[1],"--saddr")))
    {
        ...
        DRV_WIFI_Write(DRV_WIFI_WID_MAC_ADDR,(unsigned int)mac_t);
       
    }
#ifdef WLAN_REGDOMAIN_DEBUG    
       else if ((argc >= 3) && (user_regdom_enable) && (!strcmp((char*)argv[1],"pow"))) {
        char *parg=NULL;
