/*******************************************************************************
  ...
  Summary:
   Microchip Wi-Fi Driver Interface File.

  Description:
    Contains all data types and function prototypes for interfacing to the Microchip Wi-Fi driver..
*******************************************************************************/
...
*******************************************************************************/
//#ifdef IMOLA_MCHPCOMMAND
#ifndef _DRV_MCHP_WLAN_H
#define _DRV_MCHP_WLAN_H
...
/*
	Summary:
		WID for encryption type.
	Description:
		DRV_WIFI_WID_11I_MODE indicates the encryption type. 
		The valid values in AP/STA mode are shown,
		...
		WPA – AES (PSK)			0x29
		WPA – TKIP (PSK)		0x49
		WPA – AES + TKIP (PSK)	0x69
		WPA2 – AES (PSK)		0x31
		WPA2 – TKIP (PSK)		0x51
		WPA2 – AES + TKIP (PSK)	0x71

*/
#define DRV_WIFI_WID_11I_MODE  						0x000C

// *****************************************************************************
/*
	Summary:
		WID for authentication type.
	Description:
		DRV_WIFI_WID_AUTH_TYPE indicates the authentication type to be used when security feature is enabled. 
		The possible values are shown below,
	
		Authentication Type 									Value
		Open authentication for WEP/ Pre-shared key for WPA		1
		Shared Key Authentication for WEP						2
		IEEE 802.1x Authentication for WPA						5
*/
#define DRV_WIFI_WID_AUTH_TYPE 						0x000D
...
/*
	Function:
		DRV_WIFI_Write(short wid,unsigned int data)
		
	Summary:
		Provide WLAN MAC driver write API.
		<p><b>Implementation:</b> Static</p>
	
	Description:
		This function provide interface to write WLAN MAC WID parameter.
	
	Precondition:
		TCP/IP stack should be initialized.
	
	Parameters:
		wid  - WID setting need to apply on WLAN MAC  
		data - Actual data which need to set on WLAN MAC
	
	Returns: 
		0 - on able to apply setting on WLAN MAC.
		1 - Failed to apply setting on WLAN MAC.
	
	Remarks:
		None.

 */
bool DRV_WIFI_Write(short wid,unsigned int data);
// *****************************************************************************
/*
	Function:
		bool DRV_WIFI_MultiWrite(unsigned char wid[], int nWids)
	...
	Description:
		This function provide interface to write Multiple WLAN MAC WID parameter.

	Precondition:
		TCP/IP stack should be initialized.	
	
	Parameters:
		wid  - arry of WID setting need to apply on WLAN MAC  
		nWids - Number of Wid		
...
/* 
	Function:
		int DRV_WIFI_Read(short wid,void *data)
		
	Summary:
		Provide WLAN MAC driver read API.
		<p><b>Implementation:</b> Static</p>
		
	Description:
		This function provide interface for reading WLAN MAC WID parameter.
	
	Precondition:
		TCP/IP stack should be initialized.
	
	Parameters: 
		1)wid: WID parameter need to read from WLAN MAC.
		2)data : data buffer Where read wid parameter will be copied by WLAN MAC.

	Returns: 
		This function return Length data copied by WLAN MAC. 
