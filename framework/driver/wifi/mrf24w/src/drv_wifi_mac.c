...
static bool isRxFifoFull(WF_RX_FIFO * p_fifo)
{
        ...
        case WAIT_FOR_DATA_RX_MOUNT:
            ...
            if (isRawMoveComplete(RAW_DATA_RX_ID, &status, &byteCount))
            {
                ...
                if (g_RxDataLock)
                {
                    ...
                    {
                        const uint32_t dumpBuffSize = 512;
                        uint32_t totalReadLen = 0, currReadLen = 0;
                        uint8_t *dumpBuffer = (uint8_t *)TCPIP_STACK_MALLOC_FUNC(dumpBuffSize);

                        //SYS_CONSOLE_PRINT("byteCount = %d\r\n",byteCount);
                        ethPacketLength = byteCount - ETH_HEADER_START_OFFSET;
                        while (ethPacketLength > 0)
                        {
                            if (ethPacketLength > dumpBuffSize)
                                currReadLen = dumpBuffSize;
                            else
                                currReadLen = ethPacketLength;

                            RawRead(RAW_DATA_RX_ID, ETH_HEADER_START_OFFSET + totalReadLen, currReadLen, dumpBuffer);
                            ethPacketLength -= currReadLen;
                            totalReadLen += currReadLen;
                        }
                        TCPIP_STACK_FREE_FUNC(dumpBuffer);
                    }
                }
...
TCPIP_MAC_RES MRF24W_TxPacketSend(TCPIP_MAC_PACKET *p_packet)
{
    ...
    {
        SYS_CONSOLE_MESSAGE("MRF24W is in Hibernate mode, dropped the Tx packet\r\n");
