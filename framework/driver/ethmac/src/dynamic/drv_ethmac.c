...
{
    TCPIP_MODULE_MAC_PIC32INT,
    "PIC32INT",
    DRV_ETHMAC_PIC32MACInitialize,
#if (TCPIP_STACK_MAC_DOWN_OPERATION != 0)
    DRV_ETHMAC_PIC32MACDeinitialize,
    DRV_ETHMAC_PIC32MACReinitialize,
#else
    0,
    0,
#endif  // (TCPIP_STACK_DOWN_OPERATION != 0)
    DRV_ETHMAC_PIC32MACStatus,
    DRV_ETHMAC_PIC32MACTasks,
    DRV_ETHMAC_PIC32MACOpen,
    DRV_ETHMAC_PIC32MACClose,
    DRV_ETHMAC_PIC32MACLinkCheck,
    DRV_ETHMAC_PIC32MACRxFilterHashTableEntrySet,
    DRV_ETHMAC_PIC32MACPowerMode,
    DRV_ETHMAC_PIC32MACPacketTx,
    DRV_ETHMAC_PIC32MACPacketRx,
    DRV_ETHMAC_PIC32MACProcess,
    DRV_ETHMAC_PIC32MACStatisticsGet,
    DRV_ETHMAC_PIC32MACParametersGet,
    DRV_ETHMAC_PIC32MACRegisterStatisticsGet,
    DRV_ETHMAC_PIC32MACConfigGet,
    DRV_ETHMAC_PIC32MACEventMaskSet,
    DRV_ETHMAC_PIC32MACEventAcknowledge,
    DRV_ETHMAC_PIC32MACEventPendingGet,
};
...
SYS_MODULE_OBJ DRV_ETHMAC_PIC32MACInitialize(const SYS_MODULE_INDEX index, const SYS_MODULE_INIT * const init)
{
    ...
    if(initData->pPhyObject == 0 || initData->pPhyBase == 0)
    {
	...
	while(1)
	{
        ...
        PLIB_ETH_MACSetMaxFrame(ethId, TCPIP_EMAC_RX_MAX_FRAME);


        sPhyInit.moduleInit.value = 0;
        sPhyInit.ethphyId = ethId;
        sPhyInit.pPhyObject  = pMacD->mData.macConfig.pPhyObject;

        pPhyBase =  pMacD->mData.macConfig.pPhyBase;

        hPhySysObject = pPhyBase->DRV_ETHPHY_Initialize(phyIx, (SYS_MODULE_INIT *)&sPhyInit);
        ...
        phySetup.phyAddress = pMacD->mData.macConfig.phyAddress;
        phySetup.openFlags = pMacD->mData.macConfig.ethFlags; 
        phySetup.configFlags = pMacD->mData.macConfig.phyFlags;
...
TCPIP_MAC_PACKET* DRV_ETHMAC_PIC32MACPacketRx (DRV_HANDLE hMac, TCPIP_MAC_RES* pRes, const TCPIP_MAC_PACKET_RX_STAT** ppPktStat)
{
	const DRV_ETHMAC_PKT_STAT_RX    *pRxPktStat;
...
{
    DRV_ETHPHY_CLIENT_STATUS phyStat;
    DRV_ETHPHY_CONFIG_FLAGS phyCfgFlags;
    DRV_ETHPHY_RESULT phyRes;
    const DRV_ETHPHY_OBJECT_BASE* pPhyBase;   
    TCPIP_ETH_PAUSE_TYPE        pauseType;
...
/******************************************************************************
 * Function:        TCPIP_MAC_RES DRV_ETHMAC_PIC32MACRxFilterHashTableEntrySet(DRV_HANDLE hMac, TCPIP_MAC_ADDR* DestMACAddr)
 *
 * PreCondition:    DRV_ETHMAC_PIC32MACInitialize() should have been called.
 *
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32MACProcess(DRV_HANDLE hMac)
{
    ...
    {
        int nRxScheduled;   // buffers currently RX scheduled
...
#if defined(TCPIP_STACK_DRAM_DEBUG_ENABLE)
        (*(TCPIP_MAC_PKT_AckFDbg)pMacD->mData.pktAckF)(ptrPacket, TCPIP_MAC_PKT_ACK_TX_OK, TCPIP_THIS_MODULE_ID);
#else
        (*pMacD->mData.pktAckF)(ptrPacket, TCPIP_MAC_PKT_ACK_TX_OK);
#endif  // defined(TCPIP_STACK_DRAM_DEBUG_ENABLE)
...
static TCPIP_MAC_RES _MacTxPendingPackets(DRV_ETHMAC_INSTANCE_DCPT* pMacD)
{
    ...
    while( (pPkt = (TCPIP_MAC_PACKET*)(pMacD->mData._TxQueue.head)) != 0)
    {
        ...
        {   // not transmitted
#if defined(TCPIP_STACK_DRAM_DEBUG_ENABLE)
            (*(TCPIP_MAC_PKT_AckFDbg)pMacD->mData.pktAckF)(pPkt, TCPIP_MAC_PKT_ACK_BUFFER_ERR, TCPIP_THIS_MODULE_ID);
#else
            (*pMacD->mData.pktAckF)(pPkt, TCPIP_MAC_PKT_ACK_BUFFER_ERR);
#endif  // defined(TCPIP_STACK_DRAM_DEBUG_ENABLE)
        }
    }
...
static void _MacTxDiscardQueues(DRV_ETHMAC_INSTANCE_DCPT* pMacD, TCPIP_MAC_PKT_ACK_RES ackRes)
{
    ...
    {   // acknowledge the packet
#if defined(TCPIP_STACK_DRAM_DEBUG_ENABLE)
        (*(TCPIP_MAC_PKT_AckFDbg)pMacD->mData.pktAckF)(pPkt, ackRes, TCPIP_THIS_MODULE_ID);
#else
        (*pMacD->mData.pktAckF)(pPkt, ackRes);
#endif  // defined(TCPIP_STACK_DRAM_DEBUG_ENABLE)
    }
#endif  // defined(ETH_PIC32_INT_MAC_ISR_TX)
