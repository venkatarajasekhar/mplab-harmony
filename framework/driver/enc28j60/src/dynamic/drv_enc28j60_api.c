...
static DRV_ENC28J60_ClientInfo drvENC28J60ClntInst[DRV_ENC28J60_CLIENT_INSTANCES];
...
{
    uint8_t x;
    for (x = 0; x < DRV_ENC28J60_CLIENT_INSTANCES; x++)
    {
...
SYS_MODULE_OBJ DRV_ENC28J60_Initialize(SYS_MODULE_INDEX index, SYS_MODULE_INIT * init)
{
    ...
    memset(pDrvInst, 0, sizeof(DRV_ENC28J60_Configuration));
...
TCPIP_MAC_RES DRV_ENC28J60_StatisticsGet(DRV_HANDLE hMac, TCPIP_MAC_RX_STATISTICS* pRxStatistics, TCPIP_MAC_TX_STATISTICS* pTxStatistics)
{
    ...
    if (pRxStatistics != NULL)
    {
        ...
        pRxStatistics->nRxErrorPackets = 0;
        pRxStatistics->nRxFragmentErrors = 0;
    }
    if (pTxStatistics != NULL)
    {
        ...
        pTxStatistics->nTxErrorPackets = 0;
        pTxStatistics->nTxQueueFull = 0;
    }
