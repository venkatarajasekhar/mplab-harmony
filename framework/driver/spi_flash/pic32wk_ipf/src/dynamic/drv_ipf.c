    ...
    switch(hDriver->bufferProcessState)
    {
        ...
        case DRV_IPF_WREN_EXECUTION_STATUS_CHECK:
        {
            ...
            if(DRV_SPI_BUFFER_EVENT_COMPLETE & DRV_SPI_BufferStatus(hDriver->spiBufferHandle))
            {
                ...
                else if(DRV_IPF_HW_BLOCK_PROT == bufferObj->operation)
                {
        ...
        case DRV_IPF_SEND_WRITE_CMD_ADDRESS_AND_DATA:
        {
			...
			memcpy(&hDriver->commandAddressData[4],bufferObj->buffer,writeSize);
...
)
{
	...
	DRV_IPF_SetBitInArray(clientObj->blockProtStatus, bitPosition); 
    ...
    {
        DRV_IPF_SetBitInArray(clientObj->blockProtStatus, bitPosition - 1); 
...
)
{
	...
	DRV_IPF_ClearBitInArray(clientObj->blockProtStatus, bitPosition); 
	DRV_IPF_ClearBitInArray(gDrvBlockProtReg, bitPosition);

    /* Checking if both Write and read protect to be removed */
    if(protMode && (protMode & (protMode - 1))) 
    {
        if((DRV_IPF_CheckBitInArray(clientObj->blockProtStatus, bitPosition - 1) == true) 
        && (DRV_IPF_CheckBitInArray(gDrvBlockProtReg, bitPosition - 1) == true))
        {
            DRV_IPF_ClearBitInArray(clientObj->blockProtStatus, bitPosition - 1); 
