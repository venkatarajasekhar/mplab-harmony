...
/* This function removes the buffer objects queued for the client object. */
static void DRV_SST25_RemoveClientBufferObjects
(
    const DRV_SST25_CLIENT_OBJ *clientObj,
    DRV_SST25_OBJ *dObj
)
{
    DRV_SST25_BUFFER_OBJ *bufferObj = NULL;
    DRV_SST25_BUFFER_OBJ *prevObj = NULL;
    ...
    {
        if (bufferObj->hClient == clientObj)
        {
            bufferObj->inUse = false;

            if(bufferObj->previous != NULL)
            {
        ...
        case DRV_SST25_TASK_TRANSFER:
            {
                ...
                switch (bufferObj->opType)
                {
                    ...
                    case DRV_SST25_OPERATION_TYPE_ERASE:
                        {
                            event = dObj->flashFunctions.erase(dObj, bufferObj->blockStart, bufferObj->nBlocks);
                            break;
                        }
