/*******************************************************************************
  AK4384 CODEC Driver Dynamic implemention.
...
size_t DRV_AK4384_BufferProcessedSizeGet(DRV_AK4384_BUFFER_HANDLE bufferHandle)
{
    ...
    if((DRV_AK4384_BUFFER_HANDLE_INVALID == bufferHandle) || (0 == bufferHandle))
    {
        return(DRV_AK4384_BUFFER_HANDLE_INVALID);
    }

    return DRV_I2S_BufferProcessedSizeGet((DRV_I2S_BUFFER_HANDLE)bufferHandle);
}
...
void DRV_AK4384_VolumeSet(DRV_HANDLE handle, DRV_AK4384_CHANNEL chan, uint8_t volume)
{
    ...
    if(!drvObj->isVolumeSetUnderProcess)
    {
        ...
        if(DRV_AK4384_CHANNEL_LEFT == chan)
        {
            ...
            drvObj->command = DRV_AK4384_COMMAND_VOLUME_SET_CHANNEL_LEFT_ONLY;
        ...
        else if(DRV_AK4384_CHANNEL_RIGHT == chan)
        {
            ...
            drvObj->command = DRV_AK4384_COMMAND_VOLUME_SET_CHANNEL_RIGHT_ONLY;
        ...
        else
        {
            ...
            drvObj->command = DRV_AK4384_COMMAND_VOLUME_SET_CHANNEL_LEFT;
            _DRV_AK4384_ConrolRegisterSet(drvObj,DRV_AK4384_CONTROL_REGISTER_LATT,volume);
        }
    }
    else
...
static void _DRV_AK4384_ControlTasks(DRV_AK4384_OBJ *drvObj)
{
    ...
    switch (drvObj->command)
    {
        ...
        {
            /* Generate master clocke from REFCLOCK for the given sampling rate */
        ...
        case DRV_AK4384_COMMAND_VOLUME_SET_CHANNEL_RIGHT_INIT: // Coming here after setting left channel
        {
            if (false == drvObj->controlCommandStatus)
            {
                drvObj->controlCommandStatus = true;
