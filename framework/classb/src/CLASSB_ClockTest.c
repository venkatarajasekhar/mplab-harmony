...
CLASSBRESULT CLASSB_ClockTest(unsigned int clockFrequency, unsigned int referenceFrequency, unsigned int testLengthMsec, unsigned int tolerance )
{
    ...
    unsigned int    tStart;
    unsigned int    tEnd;
    unsigned int    sysCntHiVal;
    unsigned int    sysCntLoVal;
    CLASSBRESULT     testResult=CLASSB_TEST_FAIL;
    unsigned long long    sysCountsPerTest= clockFrequency;
    unsigned long long    refCountsPerTest;
    ...
    sysCountsPerTest=(sysCountsPerTest * testLengthMsec)/2000;
    // This is the number of counts on the Timer to wait.
    refCountsPerTest=(testLengthMsec * referenceFrequency)/1000;   
    //  Calculate the high number of counts based on the target counts and tolerance.
    sysCntHiVal=sysCountsPerTest + ((sysCountsPerTest*tolerance)/100);     // high count limit
    //  Calculate the low number of counts based on the target counts and tolerance.
    sysCntLoVal=sysCountsPerTest - ((sysCountsPerTest*tolerance)/100);     // high count limit

    // enable timer1 using the Low Power RC (LPRC) Oscillator
    PLIB_TMR_Mode16BitEnable(CLOCK_TEST_TIMER);
    PLIB_TMR_Counter16BitSet(CLOCK_TEST_TIMER, refCountsPerTest);
    PLIB_TMR_PrescaleSelect(CLOCK_TEST_TIMER, TMR_PRESCALE_VALUE_1);
    PLIB_TMR_ClockSourceSelect(CLOCK_TEST_TIMER, TMR_CLOCK_SOURCE_PERIPHERAL_CLOCK );
