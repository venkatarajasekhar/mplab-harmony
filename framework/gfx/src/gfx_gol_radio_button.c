...
static GFX_GOL_RADIOBUTTON * _pListButtonsGroup[12];

GFX_GOL_RADIOBUTTON  *GFX_GOL_RadioButtonListCreate(
                                SYS_MODULE_INDEX    gfxIndex,
                                uint16_t            ID,
                                uint16_t            left,
                                uint16_t            top,
                                ...
                                uint16_t            numofGroups)
{
    ...
    pObject->hdr.groupNo = groupNo;
    // action get function
    pObject->hdr.actionGet = GFX_GOL_RadioButtonActionGet;
    // default action function
    pObject->hdr.actionSet = GFX_GOL_RadioButtonActionSet;  

    // set this new object to be the last radio button in the list
    pObject->pNext = NULL;
    
    if(_pListButtonsGroup[pObject->hdr.groupNo] == NULL)
    {
        
        _pListButtonsGroup[pObject->hdr.groupNo] = pObject;
        
        pObject->pHead = (GFX_GOL_OBJ_HEADER *)_pListButtonsGroup[pObject->hdr.groupNo];
    ...
    else
    {
        ...
        GFX_GOL_RADIOBUTTON * pointerTemp = (GFX_GOL_RADIOBUTTON *)_pListButtonsGroup[pObject->hdr.groupNo];
        while(pointerTemp->pNext != NULL)
        {
            pointerTemp = (GFX_GOL_RADIOBUTTON *) pointerTemp->pNext;
        }

        
        pointerTemp->pNext =  (GFX_GOL_OBJ_HEADER *) pObject;
        pObject->pNext = NULL;
    }    
...
//        _pListButtons[groupNo] = pObject;
//
//        // Attach the button to the list (for the first one,
//        // it will basically point to itself)
//        pObject->pHead = (GFX_GOL_OBJ_HEADER *)_pListButtons[groupNo];
//    }
//    else
//    {
...
//    }
    ...
    GFX_GOL_ObjectAdd(gfxIndex, (GFX_GOL_OBJ_HEADER *)pObject);
...
void GFX_GOL_RadioButtonTextSet(
                                ...
                                GFX_XCHAR * pText)
{
    if (pText == NULL || pObject->hdr.pGolScheme->pFont == NULL)
    {
        pObject->pText = NULL;
...
*/
// *****************************************************************************
inline void GFX_GOL_RadioButtonTextAlignmentSet(
                                GFX_GOL_RADIOBUTTON *pObject,
                                GFX_ALIGNMENT align)
{
...
*/
// *****************************************************************************
inline GFX_ALIGNMENT
GFX_GOL_RadioButtonTextAlignmentGet(GFX_GOL_RADIOBUTTON *pObject)
{
...
GFX_STATUS GFX_GOL_RadioButtonDraw(void *pObject)
{
    ...
    GFX_COLOR               faceClr;
    GFX_GOL_RADIOBUTTON     *pRb = (GFX_GOL_RADIOBUTTON *)pObject;
    GFX_FILL_STYLE          fillStyle;
    GFX_PREEMPTION_LEVEL    preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pRb->hdr.gfxIndex);

    while(1)
    {

        if (GFX_RenderStatusGet(pRb->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            case DRAW_RADIOBUTTON_START:
                ...
                GFX_DRIVER_SetupDrawUpdate( pRb->hdr.left,
                                            ...
                                            pRb->hdr.bottom);
#endif
                GFX_GOL_PanelBackgroundSet(pRb->hdr.gfxIndex, &pRb->hdr);
            ...
            case DRAW_RADIOBUTTON_PANEL:
                ...
                if(GFX_GOL_PanelDraw(pRb->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
                state = DRAW_RADIOBUTTON_UPDATE;
    ...
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
                ...
                GFX_LineStyleSet(pRb->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
            ...
            case DRAW_RADIOBUTTON_TEXT:
                ...
                if(pRb->pText != NULL && pRb->pText[0] != '\0' && pRb->hdr.pGolScheme->pFont != NULL)
                {
                    GFX_FontSet(pRb->hdr.gfxIndex, pRb->hdr.pGolScheme->pFont);
    
                    if(GFX_GOL_ObjectStateGet(
                            pRb,
                            GFX_GOL_RADIOBUTTON_DISABLED_STATE))
                    {
                        GFX_ColorSet(pRb->hdr.gfxIndex,
                                     pRb->hdr.pGolScheme->TextColorDisabled);
                    }
                    else
                    {
                        GFX_ColorSet(pRb->hdr.gfxIndex,
                                     pRb->hdr.pGolScheme->TextColor0);
                    }
            ...
            case DRAW_RADIOBUTTON_TEXT_RUN:
                ...
                temp = pRb->hdr.left + pRb->hdr.pGolScheme->EmbossSize +
                    (radius << 1) + RB_INDENT;
                if (GFX_TextStringBoxDraw(pRb->hdr.gfxIndex,
                        temp,
            ...
            case DRAW_RADIOBUTTON_CHECK:
                ...
                GFX_GOL_ObjectBackGroundSet(pRb->hdr.gfxIndex, &pRb->hdr);
                        ...
                        GFX_GOL_RADIOBUTTON_CHECKED_STATE))
                {
                    ...
                    GFX_FillStyleSet(pRb->hdr.gfxIndex, GFX_FILL_STYLE_COLOR);
                    if(GFX_GOL_ObjectStateGet(
                            pRb,
                            GFX_GOL_RADIOBUTTON_DISABLED_STATE))
                    {
                    ...
                    else
                    {
                        ...
                        GFX_ColorSet(pRb->hdr.gfxIndex, BLACK);
#else
                        GFX_ColorSet(pRb->hdr.gfxIndex,
                                     pRb->hdr.pGolScheme->TextColor0);
#endif
                ...
                else
                {
                    ...
                    {
                        GFX_ColorSet(pRb->hdr.gfxIndex,
                                     pRb->hdr.pGolScheme->ColorDisabled);
                    }
                    else
                    {
#if (GFX_CONFIG_COLOR_DEPTH == 1)
                        GFX_ColorSet(pRb->hdr.gfxIndex, WHITE);
#else
                        GFX_ColorSet(pRb->hdr.gfxIndex,
                                     pRb->hdr.pGolScheme->Color0);
#endif
                    }
                }
    ...
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
                ...
                GFX_FillStyleSet(pRb->hdr.gfxIndex, GFX_FILL_STYLE_COLOR);
#endif
                GFX_BackgroundTypeSet(pRb->hdr.gfxIndex,
                                      pRb->hdr.pGolScheme->CommonBkType);
            ...
            case DRAW_RADIOBUTTON_CHECK_RUN:
                ...
                if(GFX_CircleFillDraw(pRb->hdr.gfxIndex,
                        x, y,
            ...
            case DRAW_RADIOBUTTON_FOC:
                        ...
                        GFX_GOL_RADIOBUTTON_DRAW_FOCUS_STATE)))
                {
                    ...
                    {
                        GFX_LineStyleSet(pRb->hdr.gfxIndex,
            ...
            case DRAW_RADIOBUTTON_FOC_ACTUAL:
                ...
                if(GFX_RectangleDraw(pRb->hdr.gfxIndex,
                        pRb->hdr.left,
                        pRb->hdr.top,
                        pRb->hdr.right,
            ...
            case DRAW_RADIOBUTTON_HIDE:
                ...
                if (GFX_GOL_ObjectHideDraw(pRb->hdr.gfxIndex, &(pRb->hdr))
                        != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
