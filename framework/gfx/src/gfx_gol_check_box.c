                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    GFX_GOL_ObjectAdd(pObject->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pObject);
...
*/
// *****************************************************************************
inline void GFX_GOL_CheckBoxTextAlignmentSet(
                                GFX_GOL_CHECKBOX *pObject,
                                GFX_ALIGNMENT align)
{
...
*/
// *****************************************************************************
inline GFX_ALIGNMENT GFX_GOL_CheckBoxTextAlignmentGet(
                                GFX_GOL_CHECKBOX *pObject)
{
...
GFX_STATUS GFX_GOL_CheckBoxDraw(void *pObject)
{
    ...
    GFX_GOL_CHECKBOX        *pCb = (GFX_GOL_CHECKBOX *)pObject;
    GFX_PREEMPTION_LEVEL    preemptionLevel =
                   GFX_GOL_PreemptionLevelGet(pCb->hdr.gfxIndex);

    while(1)
    {
        if (GFX_RenderStatusGet(pCb->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            case DRAW_CHECKBOX_RUN_DRAW:
                ...
                if(GFX_GOL_PanelDraw(pCb->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case DRAW_CHECKBOX_TEXT_DRAW:
                ...
                if(pCb->pText != NULL)
                {
                    GFX_FontSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->pFont);
    
                    if(!GFX_GOL_ObjectStateGet(
                            pCb,
                            GFX_GOL_CHECKBOX_DISABLED_STATE))
                    {
                        GFX_ColorSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->TextColor0);
                    }
                    else
                    {
                        GFX_ColorSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->TextColorDisabled);
                    }
            ...
            case DRAW_CHECKBOX_TEXT_DRAW_RUN:
                if (GFX_TextStringBoxDraw(pCb->hdr.gfxIndex,
	        ...
	        GFX_DRIVER_SetupDrawUpdate( pCb->hdr.left,
                ...
                {
                    if (GFX_GOL_ObjectHideDraw(pCb->hdr.gfxIndex, &(pCb->hdr)) != GFX_STATUS_SUCCESS)
                        return (GFX_STATUS_FAILURE);
                        ...
                        GFX_GOL_CHECKBOX_DRAW_STATE))
                {
                    ...
                    GFX_ColorSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->CommonBkColor);
                    GFX_GOL_ObjectBackGroundSet(pCb->hdr.gfxIndex, &pCb->hdr);

                    if(GFX_RectangleFillDraw(pCb->hdr.gfxIndex,
                            pCb->hdr.left,
                            pCb->hdr.top,
                            pCb->hdr.right,
            ...
            case DRAW_CHECKBOX_BOX_DRAW:
                ...
                GFX_LineStyleSet(pCb->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
                        ...
                        GFX_GOL_CHECKBOX_DRAW_STATE))
                {
                    if(GFX_GOL_ObjectStateGet(
                            ...
                            GFX_GOL_CHECKBOX_DISABLED_STATE))
                    {
                        GFX_GOL_PanelParameterSet(pCb->hdr.gfxIndex,
                    ...
                    GFX_GOL_PanelBackgroundSet(pCb->hdr.gfxIndex, &pCb->hdr);
            ...
            case DRAW_CHECKBOX_CHECK_SET_DRAW:
                        ...
                        (GFX_GOL_CHECKBOX_DRAW_STATE | GFX_GOL_CHECKBOX_DRAW_CHECK_STATE)))
                {
                            ...
                            GFX_GOL_CHECKBOX_DISABLED_STATE))
                    {
                        ...
                        {
                            GFX_ColorSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->TextColor0);
                            GFX_FillStyleSet(pCb->hdr.gfxIndex, GFX_FILL_STYLE_COLOR);
                        }
                        else
                        {
                            GFX_GOL_ObjectBackGroundSet(pCb->hdr.gfxIndex, &pCb->hdr);
                            GFX_ColorSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->Color0);
                        }
                    }
                    else
                    {
                        ...
                        {
                            GFX_ColorSet(pCb->hdr.gfxIndex, pCb->hdr.pGolScheme->ColorDisabled);
                        }
                    }
            ...
            case DRAW_CHECKBOX_FOCUS_DRAW:
                        ...
                        GFX_GOL_CHECKBOX_DRAW_FOCUS_STATE)))
                {
                    ...
                    GFX_LineStyleSet(pCb->hdr.gfxIndex, GFX_LINE_STYLE_THIN_DASHED);
                    if(GFX_RectangleDraw(pCb->hdr.gfxIndex,
                            pCb->hdr.left,
                            pCb->hdr.top,
                            pCb->hdr.right,
                            pCb->hdr.bottom) != GFX_STATUS_SUCCESS)
