                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    GFX_GOL_ObjectAdd(pW->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pW);
...
*/
// *****************************************************************************
inline GFX_ALIGNMENT GFX_GOL_WindowTextAlignmentGet(
                                GFX_GOL_WINDOW *pObject)
{
...
*/
// *****************************************************************************
inline void GFX_GOL_WindowTextAlignmentSet(
                                GFX_GOL_WINDOW *pObject,
                                GFX_ALIGNMENT alignment)
{
...
GFX_STATUS GFX_GOL_WindowDraw(void *pObject)
{
    typedef enum
    {
        ...
        WND_DRAW_CLIENT_AREA
           ...
           int16_t         temp = 0;
           GFX_GOL_WINDOW  *pW   = (GFX_GOL_WINDOW *)pObject;
    static WND_DRAW_STATES state = WND_DRAW_IDLE;
    static uint16_t        embossSize;
    GFX_PREEMPTION_LEVEL   preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pW->hdr.gfxIndex);
    while(1)
    {
    
        if (GFX_RenderStatusGet(pW->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            ...
            case WND_DRAW_HIDE:
                ...
                if (GFX_GOL_ObjectHideDraw(pW->hdr.gfxIndex, &(pW->hdr))
                        != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
            ...
            case WND_DRAW_SET_CLIENT_AREA:
                ...
                GFX_LineStyleSet(pW->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
                
                // set panel parameters
                GFX_GOL_PanelParameterSet(pW->hdr.gfxIndex,
                        pW->hdr.left,
                        pW->hdr.top,
                        pW->hdr.right,
                        pW->hdr.bottom,
                ...
                GFX_GOL_PanelBackgroundSet(pW->hdr.gfxIndex, &pW->hdr);
            ...
            case WND_DRAW_CLIENT_AREA:
                ...
                if(GFX_GOL_PanelDraw(pW->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
                ...
                else
		{
            ...
            case WND_DRAW_TITLE_BAR_SET:
                        ...
                        GFX_GOL_WINDOW_DISABLED_STATE))
                {
                    ...
                    {
                        GFX_ColorSet(pW->hdr.gfxIndex,
                                     pW->hdr.pGolScheme->Color1);
                    }
                    else
                    {
                        GFX_ColorSet(pW->hdr.gfxIndex,
                                     pW->hdr.pGolScheme->Color0);
                    }
                }
                else
                {
                    GFX_ColorSet(pW->hdr.gfxIndex,
                                 pW->hdr.pGolScheme->ColorDisabled);
                }

                GFX_GOL_ObjectBackGroundSet(pW->hdr.gfxIndex, &pW->hdr);
            ...
            case WND_DRAW_TITLE_BAR:
                ...
                if(GFX_RectangleFillDraw(pW->hdr.gfxIndex,
            ...
            case WND_DRAW_TITLE_BAR_BITMAP:
                ...
                if(pW->pImage != NULL)
                {
                    if(GFX_ImageDraw(pW->hdr.gfxIndex,
                            pW->hdr.left + embossSize,
                            pW->hdr.top  + embossSize +
                            ((   GOL_OBJECT_WINDOW_TITLE_HEIGHT -
            ...
            case WND_DRAW_SET_TITLE_BAR_TEXT:
                ...
                GFX_FontSet(pW->hdr.gfxIndex, pW->hdr.pGolScheme->pFont);
                        ...
                        GFX_GOL_WINDOW_DISABLED_STATE))
                {
                    ...
                    {
                        GFX_ColorSet(pW->hdr.gfxIndex,
                                     pW->hdr.pGolScheme->TextColor1);
                    }
                    else
                    {
                        GFX_ColorSet(pW->hdr.gfxIndex,
                                     pW->hdr.pGolScheme->TextColor0);
                    }
                }
                else
                {
                    GFX_ColorSet(pW->hdr.gfxIndex,
                                 pW->hdr.pGolScheme->TextColorDisabled);
                }
            ...
            case WND_DRAW_TITLE_BAR_TEXT:
                ...
                // draw the title
                if (GFX_TextStringBoxDraw(pW->hdr.gfxIndex,
                        temp,
                        pW->hdr.top + embossSize,
