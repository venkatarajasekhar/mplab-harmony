...
/*******************************************************************************
* Function: NumberToString( uint32_t Value,XCHAR *pText, uint8_t NoOfDigits,
*                           uint8_t DotPos )
*
                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    GFX_GOL_ObjectAdd(pDm->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pDm);
...
*/
// *****************************************************************************
inline void GFX_GOL_DigitalMeterIncrement(
                                GFX_GOL_DIGITALMETER *pObject,
                                uint16_t delta)
{
    // store the previous and current value to be displayed
...
GFX_STATUS GFX_GOL_DigitalMeterDraw(void *pObject)
{
    ...
    GFX_GOL_DIGITALMETER    *pDm = (GFX_GOL_DIGITALMETER *)pObject;
    ...
    static uint16_t         charWidth;
           ...
           GFX_PREEMPTION_LEVEL  preemptionLevel =
                   GFX_GOL_PreemptionLevelGet(pDm->hdr.gfxIndex);

    while(1)
    {

        if (GFX_RenderStatusGet(pDm->hdr.gfxIndex ) == GFX_STATUS_BUSY_BIT)
            return (GFX_STATUS_FAILURE);
    
        switch(state)
        {
            case DM_STATE_DRAW_FRAME:
                if(GFX_RectangleDraw(pDm->hdr.gfxIndex,
                        pDm->hdr.left,
                        pDm->hdr.top,
                        pDm->hdr.right,
            ...
            case DM_STATE_IDLE:
                ...
                if(GFX_GOL_ObjectStateGet(pDm, GFX_GOL_DIGITALMETER_HIDE_STATE))
                {
                    ...
                    if (GFX_GOL_ObjectHideDraw( pDm->hdr.gfxIndex, &(pDm->hdr))
                                                != GFX_STATUS_SUCCESS)
                        return (GFX_STATUS_FAILURE);
                ...
                {
                    GFX_GOL_ObjectBackGroundSet(pDm->hdr.gfxIndex, &pDm->hdr);

#if defined (GFX_CONFIG_ALPHABLEND_DISABLE) && \
                    !defined (GFX_CONFIG_GRADIENT_DISABLE)
                    
                    // object do not support gradient
                    GFX_FillStyleSet(pDm->hdr.gfxIndex, GFX_FILL_STYLE_COLOR);
#else
                    GFX_FillStyleSet( pDm->hdr.gfxIndex,
                                      pDm->hdr.pGolScheme->fillStyle);
#endif
                    GFX_ColorSet( pDm->hdr.gfxIndex,
                                  pDm->hdr.pGolScheme->CommonBkColor);
            ...
            case DM_STATE_PANEL:
                ...
                if(GFX_RectangleFillDraw(pDm->hdr.gfxIndex,
                        pDm->hdr.left,
                        pDm->hdr.top,
                        pDm->hdr.right,
                        pDm->hdr.bottom) != GFX_STATUS_SUCCESS)
            ...
            case DM_STATE_FRAME:
                ...
                GFX_LineStyleSet(pDm->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
                if(!GFX_GOL_ObjectStateGet(
                        pDm,
                  ...
                  )
                {
                    // show enabled color
                    GFX_ColorSet( pDm->hdr.gfxIndex,
                                  pDm->hdr.pGolScheme->Color1 );
                }
                else
                {
                    // show disabled color
                    GFX_ColorSet( pDm->hdr.gfxIndex,
                                  pDm->hdr.pGolScheme->ColorDisabled );
                }
            ...
            case DM_STATE_INIT:
                ...
                if (GFX_RenderStatusGet(pDm->hdr.gfxIndex ) 
                                                    == GFX_STATUS_BUSY_BIT )
                    return (GFX_STATUS_FAILURE);
    
                // set clipping area, text will only
                // appear inside the text area.
                GFX_TextAreaLeftSet(pDm->hdr.gfxIndex, pDm->hdr.left +
                                        GFX_GOL_DIGITALMETER_INDENT);
                GFX_TextAreaTopSet(pDm->hdr.gfxIndex, pDm->hdr.top);
                GFX_TextAreaRightSet(pDm->hdr.gfxIndex, pDm->hdr.right -
                                        GFX_GOL_DIGITALMETER_INDENT);
                GFX_TextAreaBottomSet(pDm->hdr.gfxIndex, pDm->hdr.bottom);
                ...
                {
                    GFX_ColorSet( pDm->hdr.gfxIndex,
                                  pDm->hdr.pGolScheme->TextColor0 );
                }
                else
                {
                    GFX_ColorSet( pDm->hdr.gfxIndex,
                                  pDm->hdr.pGolScheme->TextColorDisabled);
                }
                ...
                GFX_FontSet(pDm->hdr.gfxIndex, pDm->hdr.pGolScheme->pFont);
            ...
            case DM_STATE_SETALIGN:
                ...
                if(!charCtr)
                {
                    ...
                    /* set position of the next character (based on alignment
                       and next character) */
                    textWidth = GFX_TextStringWidthGet( CurValue,
                                                    pDm->hdr.pGolScheme->pFont);
                    ...
                    switch (pDm->alignment & GFX_ALIGN_HORIZONTAL_MASK)
                    {
                        case GFX_ALIGN_LEFT:
                            ...
                            GFX_TextCursorPositionSet(pDm->hdr.gfxIndex,
                                    pDm->hdr.left +
                                        GFX_GOL_DIGITALMETER_INDENT,
                                    verticalPos);
                            break;
            ...
            case DM_STATE_SETTEXT:
                ...
                if (ch != 0)
                {
                    ...
                    tempStr[1] = 0;
                    tempStr[0] = pch;
                    charWidth = GFX_TextStringWidthGet( tempStr,
                                                   pDm->hdr.pGolScheme->pFont);
            ...
            case DM_STATE_ERASETEXT:
                ...
                if(GFX_RectangleFillDraw(pDm->hdr.gfxIndex,
                        GFX_TextCursorPositionXGet(pDm->hdr.gfxIndex),
                        pDm->hdr.top + 1,
                        GFX_TextCursorPositionXGet( pDm->hdr.gfxIndex) +
                                                    charWidth, //textWidth,
            ...
            case DM_STATE_DRAWTEXT:
                ...
                if (GFX_TextCharDraw(pDm->hdr.gfxIndex, ch) !=
                                                            GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
