                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
  ...
  //  uint16_t        width, height;
    ...
    pPicture = (GFX_GOL_PICTURECONTROL *)GFX_malloc(
            sizeof(GFX_GOL_PICTURECONTROL));
    if(pPicture == NULL)
        return (pPicture);

    pPicture->hdr.gfxIndex  = gfxIndex;
    pPicture->hdr.ID        = ID;
    ...
    // action get function
    pPicture->hdr.actionGet = GFX_GOL_PictureControlActionGet;
    // default action function
    pPicture->hdr.actionSet = NULL;                           
    ...
    GFX_GOL_ObjectAdd(pPicture->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pPicture);
...
GFX_STATUS GFX_GOL_PictureControlDraw(void *pObject)
{
    ...
    GFX_GOL_PICTURECONTROL  *pPict = (GFX_GOL_PICTURECONTROL *)pObject;
    GFX_PREEMPTION_LEVEL    preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pPict->hdr.gfxIndex);

    if (GFX_RenderStatusGet(pPict->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
    ...
    while(1)
    {
        switch(state)
        {
            case DRAW_PICTURE_START:
                ...
                GFX_GOL_PanelBackgroundSet(pPict->hdr.gfxIndex, &pPict->hdr);
            ...
            case DRAW_PICTURE_PANEL_BACKGROUND:
                ...
                if(GFX_GOL_PanelDraw(pPict->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case DRAW_PICTURE_IMAGE:
                ...
                {
                    if (GFX_RenderStatusGet(pPict->hdr.gfxIndex)
                            == GFX_STATUS_BUSY_BIT)
                        return (GFX_STATUS_FAILURE);

                    if(GFX_ImagePartialDraw(pPict->hdr.gfxIndex,
                            pPict->imageLeft,
                            pPict->imageTop,
            ...
            case DRAW_PICTURE_FRAME:
                ...
                {
                    GFX_LineStyleSet( pPict->hdr.gfxIndex,
                                      GFX_LINE_STYLE_THIN_SOLID);
                    GFX_ColorSet( pPict->hdr.gfxIndex,
                                  pPict->hdr.pGolScheme->TextColor0);
                    if(GFX_RectangleDraw(pPict->hdr.gfxIndex,
                            pPict->hdr.left,
                            pPict->hdr.top,
                            pPict->hdr.right,
            ...
            case DRAW_PICTURE_HIDE:
                ...
                if (    GFX_GOL_ObjectHideDraw(pPict->hdr.gfxIndex,
                                               &(pPict->hdr)) !=
                        GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
