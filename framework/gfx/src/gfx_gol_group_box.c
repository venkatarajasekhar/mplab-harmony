            ...
            GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    // draw function
    pGb->hdr.DrawObj    = GFX_GOL_GroupboxDraw;

    // free function
    pGb->hdr.FreeObj    = NULL;

    // action get function
    pGb->hdr.actionGet  = GFX_GOL_GroupboxActionGet;

    // default action function
    pGb->hdr.actionSet  = NULL;
    ...
    GFX_GOL_ObjectAdd(pGb->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pGb);
...
*/
// *****************************************************************************
inline void GFX_GOL_GroupboxTextAlignmentSet(
                                GFX_GOL_GROUPBOX *pObject,
                                GFX_ALIGNMENT align)
{
...
*/
// *****************************************************************************
inline GFX_ALIGNMENT GFX_GOL_GroupboxTextAlignmentGet(
                                GFX_GOL_GROUPBOX *pObject)
{
                                ...
                                GFX_GOL_MESSAGE *pMessage)
{
    ...
    if(pMessage->type == TYPE_TOUCHSCREEN)
    {
        ...
        // Check if it falls to the left or right of the center
        // of the thumb's face
        if
        (
...
GFX_STATUS GFX_GOL_GroupboxDraw(void *pObject)
{
    ...
    GFX_GOL_GROUPBOX        *pGb = (GFX_GOL_GROUPBOX *)pObject;
    GFX_PREEMPTION_LEVEL        preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pGb->hdr.gfxIndex);

    while(1)
    {

        if (GFX_RenderStatusGet(pGb->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            case GB_STATE_IDLE:
                        ...
                        GFX_GOL_GROUPBOX_HIDE_STATE))
                {
                    ...
                    if (GFX_GOL_ObjectHideDraw(pGb->hdr.gfxIndex, &(pGb->hdr))
                            != GFX_STATUS_SUCCESS)
                        return (GFX_STATUS_FAILURE);
            ...
            case GB_STATE_HIDETEXT:
                ...
                {                      
                            ...
                            GFX_BACKGROUND_IMAGE)
                    {
                        ...
                        // the text is located
                        if (GFX_ImagePartialDraw(pGb->hdr.gfxIndex,
                    ...
                    {
                        GFX_ColorSet( pGb->hdr.gfxIndex,
                                      pGb->hdr.pGolScheme->CommonBkColor);
                        if(GFX_BarDraw(pGb->hdr.gfxIndex,
                                pGb->hdr.left,
                                pGb->hdr.top,
                                pGb->hdr.right,
                                pGb->hdr.top +
            ...
            case GB_STATE_SETDIMENSION:
                ...
                {   // there is no text, use full dimensions
                    ...
                    textRight = textLeft;
                    // go to drawing of right top line
                    state = GB_STATE_DRAWTOPRIGHT;  
                    break;
                }
                else
                {   // text is present, set up dimensions with text
                    GFX_FontSet(pGb->hdr.gfxIndex, pGb->hdr.pGolScheme->pFont);
                    ...
                    /* 
                     ...
                     textLeft value calculated negative
                     */
                    if( textLeft < 0 )
                    {
                        textLeft = 0;
            ...
            case GB_STATE_DRAWTEXT:
                ...
                if (GFX_TextStringDraw(pGb->hdr.gfxIndex,
                        textLeft,
                        pGb->hdr.top,
                        pGb->pText,
                        0
...
#if (GFX_CONFIG_COLOR_DEPTH == 1)
                GFX_ColorSet(pGb->hdr.gfxIndex, WHITE);
#else
                GFX_ColorSet( pGb->hdr.gfxIndex,
                              pGb->hdr.pGolScheme->EmbossDkColor );
#endif
                state = GB_STATE_DRAWTOPRIGHT;
            ...
            case GB_STATE_DRAWTOPRIGHT:
                // top line at right
                if(GFX_LineDraw(pGb->hdr.gfxIndex,
                        textRight,
                        top + THREE_D_EFFECT,
                        pGb->hdr.right,
            ...
            case GB_STATE_2DRAWTOPLEFT:
                ...
                GFX_ColorSet( pGb->hdr.gfxIndex,
                              pGb->hdr.pGolScheme->EmbossLtColor );
