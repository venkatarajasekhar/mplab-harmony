                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    pTe->hdr.gfxIndex   = gfxIndex;
    pTe->hdr.ID         = ID;
    pTe->hdr.pNxtObj    = NULL;
    pTe->hdr.type       = GFX_GOL_TEXTENTRY_TYPE;   // set object type
    /* left parameter of the text-entry */
    pTe->hdr.left       = left;
    /* top parameter of the text-entry */
    pTe->hdr.top        = top;
    /* right parameter of the text-entry */
    pTe->hdr.right      = right;
    /* bottom parameter of the text-entry */
    ...
    //check if either values of horizontal keys and vertical keys are equal to
    //zero
    ...
    GFX_GOL_ObjectAdd(pTe->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pTe);
...
*/
// *****************************************************************************
inline GFX_XCHAR *GFX_GOL_TextEntryBufferGet(
                                GFX_GOL_TEXTENTRY *pObject)
{
...
GFX_GOL_TEXTENTRY_KEYMEMBER *GFX_GOL_TextEntryKeyListCreate(
                                ...
                                GFX_XCHAR *pText[])
{
    int16_t     width, height; // unused todo NumberOfKeys, 
    ...
    // todo unused - NumberOfKeys = pObject->horizontalKeys * pObject->verticalKeys;
    ...
    for(colcount = 0; colcount < pObject->verticalKeys; colcount++)
    {
        for(rowcount = 0; rowcount < pObject->horizontalKeys; rowcount++)
        {
            ...
            if (pKl->right  > GFX_MaxXGet(pObject->hdr.gfxIndex))
                pKl->right  = GFX_MaxXGet(pObject->hdr.gfxIndex);
            if (pKl->bottom > GFX_MaxYGet(pObject->hdr.gfxIndex))
                pKl->bottom = GFX_MaxYGet(pObject->hdr.gfxIndex);
            ...
            if(pKl->pKeyName != NULL)
            {
                ...
                pKl->textWidth  = GFX_TextStringWidthGet( pKl->pKeyName,
                                                pObject->hdr.pGolScheme->pFont);
                pKl->textHeight = GFX_TextStringHeightGet(
                        pObject->hdr.pGolScheme->pFont);
            } //end if
...
GFX_GOL_TRANSLATED_ACTION GFX_GOL_TextEntryActionGet(
                                ...
                                GFX_GOL_MESSAGE *pMessage)
{
    int16_t                     param1, param2; // unused todo NumberOfKeys
    ...
    // unused todo - NumberOfKeys = (pTe->horizontalKeys) * (pTe->verticalKeys);
    ...
    if((pMessage->type == TYPE_TOUCHSCREEN))
    {
        ...
        (
            ...
            (pTe->hdr.top + (GFX_TextStringHeightGet(pTe->pDisplayFont) +
                (pTe->hdr.pGolScheme->EmbossSize << 1)) < pMessage->param2) &&
            (pTe->hdr.bottom > pMessage->param2)
        )
        {

            /* If it fell inside the TextEntry panel, go through the link list 
               and check which one was pressed. At this point the touch screen
               event is either EVENT_MOVE or EVENT_PRESS.
            */
            ...
            while(pKeyTemp != NULL)
            {
                ...
                )
                {
                    ...
                    else
                    {
                        ...
                        // to shift the press to another key make sure that 
                        // there are no other keys currently pressed. If there
                        // is one it must be released first. Check if there are
                        // previously pressed keys.
                        if(GFX_GOL_ObjectStateGet(pTe,
                                           GFX_GOL_TEXTENTRY_KEY_PRESSED_STATE))
                        {
                        ...
                        else
                        {
                            ...
                            // check if the active key is not pressed. If not,
                            // set to press since the current touch event is
                            // either move or press check if there is an active
                            // key already set. If none, set the current key as
                            // active and return a pressed message.
                ...
                else
                {
                    ...
                    // if the key is in the pressed state and current touch is 
                    // not here then it has to be redrawn
...
GFX_STATUS GFX_GOL_TextEntryDraw(void *pObject)
{
    ...
    GFX_GOL_TEXTENTRY *pTe = (GFX_GOL_TEXTENTRY *)pObject;

    GFX_PREEMPTION_LEVEL    preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pTe->hdr.gfxIndex);

    while(1)
    {
        if (GFX_RenderStatusGet(pTe->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
	...
	switch(state)
	{
            case DRAW_TEXTENTRY_START:
                ...
                else
                {
                            ...
                            GFX_GOL_TEXTENTRY_DRAW_STATE)))
                    {
                        ...
                        GFX_GOL_PanelAlphaParameterSet(pTe->hdr.gfxIndex,
                                               pTe->hdr.pGolScheme->AlphaValue);
#endif
                        // set the background information
                        GFX_BackgroundSet(pTe->hdr.gfxIndex,
                                pTe->hdr.pGolScheme->CommonBkLeft,
                                pTe->hdr.pGolScheme->CommonBkTop,
                                pTe->hdr.pGolScheme->pCommonBkImage,
                                pTe->hdr.pGolScheme->CommonBkColor);
                        GFX_BackgroundTypeSet(pTe->hdr.gfxIndex,
                        pTe->hdr.pGolScheme->CommonBkType);
                        ...
                        GFX_GOL_TEXTENTRY_DRAW_STATE))
                        {
                            ...
                            GFX_LineStyleSet(pTe->hdr.gfxIndex,
            ...
            case DRAW_TEXTENTRY_HIDE:
                ...
                if (GFX_GOL_ObjectHideDraw(pTe->hdr.gfxIndex, &(pTe->hdr))
                        != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
            ...
            case DRAW_TEXTENTRY_PANEL:
                ...
                if(GFX_GOL_PanelDraw(pTe->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case DRAW_TEXTENTRY_KEY_PANEL_SET:
                ...
                if(CountOfKeys < (pTe->horizontalKeys * pTe->verticalKeys))
                {
                    ...
                    GFX_GOL_PanelGradientParameterSet(pTe->hdr.gfxIndex,
            ...
            case DRAW_TEXTENTRY_KEY_PANEL_DRAW:
                ...
                {
                    GFX_ColorSet(pTe->hdr.gfxIndex,
                                 pTe->hdr.pGolScheme->TextColorDisabled);
                }
                else
                {
                    if( (GFX_GOL_ObjectStateGet(
                    ...
                    {
                        GFX_ColorSet(pTe->hdr.gfxIndex,
                                     pTe->hdr.pGolScheme->TextColor1);
                    }
                    else
                    {
                        GFX_ColorSet(pTe->hdr.gfxIndex,
                                     pTe->hdr.pGolScheme->TextColor0);
                    }
                }

                // set the font to be used
                GFX_FontSet(pTe->hdr.gfxIndex, pTe->hdr.pGolScheme->pFont);
            ...
            case DRAW_TEXTENTRY_STRING_INIT:
                ...
                GFX_TextAreaLeftSet(pTe->hdr.gfxIndex,
                                    pTe->hdr.left  + embossSize);
                GFX_TextAreaTopSet(pTe->hdr.gfxIndex,
                                   pTe->hdr.top   + embossSize);
                GFX_TextAreaRightSet(pTe->hdr.gfxIndex,
                                     pTe->hdr.right - embossSize);
                GFX_TextAreaBottomSet(pTe->hdr.gfxIndex, pTe->hdr.top   +
                        embossSize +
                        GFX_TextStringHeightGet(pTe->pDisplayFont));
                                ...
                                GFX_GOL_TEXTENTRY_UPDATE_TEXT_STATE))
                {
                    ...
                    GFX_ColorSet(pTe->hdr.gfxIndex,
                                 pTe->hdr.pGolScheme->Color1);
                    GFX_GOL_ObjectBackGroundSet(pTe->hdr.gfxIndex, &pTe->hdr);
            ...
            case DRAW_TEXTENTRY_EDITBOX_CLEAR:
                ...
                if (GFX_RectangleFillDraw(pTe->hdr.gfxIndex,
                    pTe->hdr.left  + embossSize,
                    pTe->hdr.top   + embossSize,
                    pTe->hdr.right - embossSize,
                    pTe->hdr.top   +
            ...
            case DRAW_TEXTENTRY_STRING_UPDATE_PREPARE:
                ...
                switch(pTe->alignment & GFX_ALIGN_HORIZONTAL_MASK)
                {
                    ...
                    case GFX_ALIGN_RIGHT:
                        ...
                        leftPos = (int16_t)(pTe->hdr.right - 4 - embossSize -
                                textWidth);
                        break;
                    ...
                    default:
                        ...
                        // text is aligned horizontal center, starting position 
                        // of the first character is dependent on the width of
                        // the string and the width of the box.
                ...
                GFX_TextCursorPositionSet(pTe->hdr.gfxIndex,
                leftPos, pTe->hdr.top + embossSize);
            ...
            case DRAW_TEXTENTRY_STRING_UPDATE:
                ...
                GFX_ColorSet(pTe->hdr.gfxIndex,
                             pTe->hdr.pGolScheme->TextColor1);
                GFX_FontSet(pTe->hdr.gfxIndex, pTe->pDisplayFont);

                // this is manually doing the GFX_TextStringDraw() function but 
                // with the capability to replace the characters to the
                ...
                else
                {
                    if(GFX_GOL_ObjectStateGet(
                                    pTe,
                                    GFX_GOL_TEXTENTRY_ECHO_HIDE_STATE))
                        while(GFX_TextCharDraw(pTe->hdr.gfxIndex, 0x2A)
                                != GFX_STATUS_SUCCESS);
                    else
                        while(GFX_TextCharDraw(pTe->hdr.gfxIndex, XcharTmp)
                                != GFX_STATUS_SUCCESS);
            ...
            case DRAW_TEXTENTRY_EDITBOX_CHAR_UPDATE:
                ...
                if (GFX_RenderStatusGet(pTe->hdr.gfxIndex)
                        == GFX_STATUS_BUSY_BIT)
                    return (GFX_STATUS_FAILURE);
                counter++;
                state = DRAW_TEXTENTRY_STRING_UPDATE;
