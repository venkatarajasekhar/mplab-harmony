...
/*********************************************************************
* Function: GFX_GOL_SCROLLBAR *SldCreate( SYS_MODULE_INDEX, gfxIndex ,
*                                         uint16_t ID,
*                                         int16_t left,
*                                         int16_t top,
*                                         int16_t right,
...
*
* Notes: Creates a GFX_GOL_SCROLLBAR object and adds it to the current active 
*        list. If the creation is successful, the pointer to the created Object
                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    /* set object type */
    pSld->hdr.type      = GFX_GOL_SCROLLBAR_TYPE;

    /* left and right should be equal when oriented vertically */
    pSld->hdr.left      = left;

    /* top and bottom should be equal when oriented horizontally */
    pSld->hdr.top       = top;
    ...
    /* draw function */
    pSld->hdr.DrawObj   = GFX_GOL_ScrollBarDraw;

    /* message function */
    pSld->hdr.actionGet = GFX_GOL_ScrollBarActionGet;

    /* default message function */
    pSld->hdr.actionSet = GFX_GOL_ScrollBarActionSet;

    /* default free function */
    pSld->hdr.FreeObj   = NULL;
    ...
    GFX_GOL_ObjectAdd(pSld->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pSld);
    return (pSld);
}
...
*/
// *****************************************************************************
inline uint16_t GFX_GOL_ScrollBarRangeGet(
                                GFX_GOL_SCROLLBAR *pObject)
{
...
*/
// *****************************************************************************
inline uint16_t GFX_GOL_ScrollBarPageGet(
                                GFX_GOL_SCROLLBAR *pObject)
{
    return (pObject->page);
}
...
*/
// *****************************************************************************
inline uint16_t GFX_GOL_ScrollBarPositionGet(
                                GFX_GOL_SCROLLBAR *pObject)
{
    return (pObject->pos);
}
...
/*********************************************************************
* Function: int16_t GFX_GOL_ScrollBarThumbSizeSet( GFX_GOL_SCROLLBAR *pSld,
*                                                  int16_t high, int16_t low)
*
...
/*********************************************************************
* Function: void GFX_GOL_ScrollBarMinMaxPosGet( GFX_GOL_SCROLLBAR *pSld,
                                ...
                                GFX_GOL_MESSAGE *pMessage)
{
    ...
    if(pMessage->type == TYPE_TOUCHSCREEN)
    {
        ...
        {
            GFX_GOL_ObjectFocusSet(pSld->hdr.gfxIndex,
                                   (GFX_GOL_OBJ_HEADER *)pSld);
        }
    }
    ...
    if(pMessage->type == TYPE_TOUCHSCREEN)
    {
        if((translatedMsg == GFX_GOL_SCROLLBAR_ACTION_DEC) ||
                (translatedMsg == GFX_GOL_SCROLLBAR_ACTION_INC))
        {
                                ...
                                GFX_GOL_MESSAGE *pMessage)
{
    ...
    if(pMessage->type == TYPE_TOUCHSCREEN)
    {
        /* Check if it falls to the left or right of the center of the thumb's
           face */
...
GFX_STATUS GFX_GOL_ScrollBarDraw(void *pObject)
{
    ...
    GFX_GOL_SCROLLBAR       *pSld = (GFX_GOL_SCROLLBAR *)pObject;
    GFX_PREEMPTION_LEVEL    preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pSld->hdr.gfxIndex);

    while(1)
    {
        if (GFX_RenderStatusGet(pSld->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            case SLD_STATE_IDLE:
                ...
                GFX_GOL_PanelBackgroundSet(pSld->hdr.gfxIndex, &pSld->hdr);
    ...
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
                                            ...
                                            GFX_GOL_SCROLLBAR_HIDE_STATE))
                {
                    ...
                    GFX_ColorSet(pSld->hdr.gfxIndex,
                                 pSld->hdr.pGolScheme->CommonBkColor);
                ...
                else 
                {
                    ...
                    GFX_LineStyleSet(pSld->hdr.gfxIndex,
                                            ...
                                            GFX_GOL_SCROLLBAR_DRAW_STATE))
                    {
                        ...
                        // modify the color setting if scroll bar mode or
                        // slider mode
    			GFX_GOL_PanelParameterSet(pSld->hdr.gfxIndex,
                            pSld->hdr.left,
                            pSld->hdr.top,
                            pSld->hdr.right,
                            pSld->hdr.bottom,
            ...
            case SLD_STATE_HIDE:
                ...
                if (GFX_GOL_ObjectHideDraw(pSld->hdr.gfxIndex, &(pSld->hdr))
                                                != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
            ...
            case SLD_STATE_PANEL:
                ...
                if(GFX_GOL_PanelDraw(pSld->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
		...
		if(GFX_GOL_ObjectStateGet(  
            ...
            case SLD_STATE_THUMBPATH1:
                ...
                if(!GFX_GOL_ObjectStateGet( pSld,
                                            GFX_GOL_SCROLLBAR_VERTICAL_STATE))
                {
                    if(GFX_LineDraw(pSld->hdr.gfxIndex,
                            minPos,
                            midPoint,
                            maxPos,
		...
		state = SLD_STATE_THUMBPATH2;
            ...
            case SLD_STATE_CLEARTHUMB_CHECK:
                ...
                GFX_ColorSet(pSld->hdr.gfxIndex, colorTemp);
                
                // Remove the current thumb by drawing a bar 
                // with the background 
                GFX_GOL_ObjectBackGroundSet(pSld->hdr.gfxIndex, &pSld->hdr);
    ...
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
            ...
            case SLD_STATE_CLEARTHUMB:
                ...
                GFX_FillStyleSet(pSld->hdr.gfxIndex, fillStyle);
                
                if(!GFX_GOL_ObjectStateGet(
                                    pSld,
                                    GFX_GOL_SCROLLBAR_VERTICAL_STATE))
                {
                    if(GFX_RectangleFillDraw(pSld->hdr.gfxIndex,
            ...
            case SLD_STATE_THUMB:
                ...
                GFX_GOL_PanelGradientParameterSet(pSld->hdr.gfxIndex,
            ...
            case SLD_STATE_FOCUS:
                ...
                { // do not draw focus when in scroll bar mode
                                        ...
                                        GFX_GOL_SCROLLBAR_FOCUSED_STATE))
                    {
                        // draw the focus box
                        GFX_ColorSet(pSld->hdr.gfxIndex,
                                     pSld->hdr.pGolScheme->TextColor0);
                    }
                    else
                    {
                    ...
                    if(GFX_RectangleDraw(pSld->hdr.gfxIndex,
