                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    pSt->hdr.gfxIndex  = gfxIndex;
    /* Unique id assigned for referencing */
    pSt->hdr.ID        = ID;
    /* Initialize pointer to NULL */
    pSt->hdr.pNxtObj   = NULL;                      
    ...
    GFX_GOL_ObjectAdd(pSt->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pSt);
    return (pSt);
}
...
*/
// *****************************************************************************
inline GFX_XCHAR *GFX_GOL_StaticTextGet(
                                GFX_GOL_STATICTEXT *pObject)
{
...
*/
// *****************************************************************************
inline void GFX_GOL_StaticTextSet(
                                GFX_GOL_STATICTEXT *pObject,
                                GFX_XCHAR *pText)
{
    pObject->pText = pText;
...
*/
// *****************************************************************************
inline void GFX_GOL_StaticTextAlignmentSet(
                                GFX_GOL_STATICTEXT *pObject,
                                GFX_ALIGNMENT align)
{
...
*/
// *****************************************************************************
inline GFX_ALIGNMENT GFX_GOL_StaticTextAlignmentGet(
                                GFX_GOL_STATICTEXT *pObject)
{
...
GFX_STATUS GFX_GOL_StaticTextDraw(void *pObject)
{
    ...
    while(1)
    {
        if (GFX_RenderStatusGet(pSt->hdr.gfxIndex ) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            case DRAW_STATIC_TEXT_IDLE:
                ...
                if (GFX_GOL_ObjectHideDraw( pSt->hdr.gfxIndex,
                                            &(pSt->hdr)) != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
                ...
                if(GFX_GOL_ObjectStateGet(pSt, GFX_GOL_STATICTEXT_DRAW_STATE))
                {
                    ...
                    GFX_LineStyleSet(pSt->hdr.gfxIndex,
                                     GFX_LINE_STYLE_THIN_SOLID);
    
                    if(GFX_GOL_ObjectStateGet(pSt,
                                                GFX_GOL_STATICTEXT_FRAME_STATE))
                    {
                        ...
                        {    
                            // show enabled color
                            GFX_ColorSet(pSt->hdr.gfxIndex,
                                         pSt->hdr.pGolScheme->Color1);
                            if(GFX_RectangleDraw(pSt->hdr.gfxIndex,
                                pSt->hdr.left,
                                pSt->hdr.top,
                                pSt->hdr.right,
                        ...
                        else
                        {
                            // show disabled color
                            GFX_ColorSet(pSt->hdr.gfxIndex,
                                         pSt->hdr.pGolScheme->ColorDisabled);
                            if(GFX_RectangleDraw(pSt->hdr.gfxIndex,
                                pSt->hdr.left,
                                pSt->hdr.top,
                                pSt->hdr.right,
            ...
            case DRAW_STATIC_TEXT_CLEANAREA:
                    ...
                    (pSt->hdr.pGolScheme->fillStyle == GFX_FILL_STYLE_NONE))
                {
#ifdef GFX_CONFIG_ALPHABLEND_DISABLE
#ifndef GFX_CONFIG_TRANSPARENT_COLOR_DISABLE
                    if (GFX_TransparentColorStatusGet(pSt->hdr.gfxIndex) == GFX_FEATURE_ENABLED)
                    {
                        // disable the transparent color since we want the
                        // background to refresh with all pixels being drawn
                        GFX_TransparentColorDisable(pSt->hdr.gfxIndex);
                        transparentColorStatus = 1;
                        transparentColorSet = GFX_TransparentColorGet(pSt->hdr.gfxIndex);
                    }
#endif
#endif
                    if (GFX_ImagePartialDraw(
                            ...
                            pSt->hdr.pGolScheme->pCommonBkImage) != GFX_STATUS_SUCCESS)
                    {
                        return (GFX_STATUS_FAILURE);
                    }
                    ...
                    {
                        GFX_TransparentColorEnable(pSt->hdr.gfxIndex, transparentColorSet);
                    }
#endif
#endif
                }
                else
                {
                    if (pSt->hdr.pGolScheme->fillStyle != GFX_FILL_STYLE_NONE)
	                {
                        ...
                        GFX_ColorSet(pSt->hdr.gfxIndex,
                                     pSt->hdr.pGolScheme->CommonBkColor);
                        GFX_GOL_ObjectBackGroundSet(pSt->hdr.gfxIndex, &pSt->hdr);

                        if(GFX_RectangleFillDraw(pSt->hdr.gfxIndex,
                            pSt->hdr.left + 1,
                            pSt->hdr.top + 1,
                            pSt->hdr.right - 1,
            ...
            case DRAW_STATIC_TEXT_INIT:
                ...
                if (GFX_RenderStatusGet(pSt->hdr.gfxIndex ) ==
                        GFX_STATUS_BUSY_BIT)
                    return (GFX_STATUS_FAILURE);
                ...
                {
                    GFX_ColorSet(pSt->hdr.gfxIndex,
                                 pSt->hdr.pGolScheme->TextColor0);
                }
                else
                {
                    GFX_ColorSet(pSt->hdr.gfxIndex,
                                 pSt->hdr.pGolScheme->TextColorDisabled);
                }
    
                // use the font specified in the object
                GFX_FontSet(pSt->hdr.gfxIndex, pSt->hdr.pGolScheme->pFont);
            ...
            case DRAW_STATIC_TEXT_DRAWTEXT:
                ...
                if (GFX_TextStringBoxDraw(pSt->hdr.gfxIndex,   
                        pSt->hdr.left + 1, pSt->hdr.top + 1,
                        pSt->hdr.right  - pSt->hdr.left - 1,
                        pSt->hdr.bottom - pSt->hdr.top  - 1,
                        pSt->pText,                         
                        0, pSt->alignment)  != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
