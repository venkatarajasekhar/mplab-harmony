...
*
********************************************************************/
void GFX_GOL_ProgressBarBackGroundSet(GFX_GOL_OBJ_HEADER *pObject)
{
    GFX_GOL_PROGRESSBAR *pPb = (GFX_GOL_PROGRESSBAR *)pObject;
    
#if !defined (GFX_CONFIG_ALPHABLEND_DISABLE)
    // when alpha blending is enabled
    // set up the background info
    GFX_AlphaBlendingValueSet( pPb->hdr.gfxIndex,
                               pPb->hdr.pGolScheme->AlphaValue);
    GFX_FillStyleSet( pPb->hdr.gfxIndex,
                      pPb->hdr.pGolScheme->fillStyle);
    GFX_BackgroundTypeSet(pPb->hdr.gfxIndex, pPb->hdr.pGolScheme->CommonBkType);

#else
    // progress bar does not support gradient
    // therefore, for other backgrounds, flat color is used

    GFX_FillStyleSet(pPb->hdr.gfxIndex, GFX_FILL_STYLE_COLOR);
    GFX_BackgroundTypeSet(pPb->hdr.gfxIndex, GFX_BACKGROUND_COLOR);
#endif
    GFX_BackgroundSet(pPb->hdr.gfxIndex, pPb->hdr.pGolScheme->CommonBkLeft,
                      pPb->hdr.pGolScheme->CommonBkTop,
                      pPb->hdr.pGolScheme->pCommonBkImage,
                      pPb->hdr.pGolScheme->CommonBkColor);
                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    // action get function
    pObject->hdr.actionGet  = GFX_GOL_ProgressBarActionGet; 
...
GFX_STATUS GFX_GOL_ProgressBarDraw(void *pObject)
{
    ...
    GFX_PREEMPTION_LEVEL    preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pPb->hdr.gfxIndex);

    while(1)
    {
        if (GFX_RenderStatusGet(pPb->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            case DRAW_PROGRESSBAR_PANEL_DRAW:
                ...
                if(GFX_GOL_PanelDraw(pPb->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case DRAW_PROGRESSBAR_REMOVE:
                ...
                {
                    GFX_ColorSet( pPb->hdr.gfxIndex,
                                  pPb->hdr.pGolScheme->CommonBkColor);

                    if (GFX_GOL_ObjectHideDraw( pPb->hdr.gfxIndex,
                                                &(pPb->hdr)) !=
                                                            GFX_STATUS_SUCCESS)
                    {
                        return (GFX_STATUS_FAILURE);
                        ...
                        GFX_GOL_PROGRESSBAR_DRAW_STATE))
                {
                    ...
                    GFX_GOL_PanelBackgroundSet(pPb->hdr.gfxIndex, &pPb->hdr);
            ...
            case DRAW_PROGRESSBAR_BAR_DRAW:
                ...
                if(GFX_BarDraw(pPb->hdr.gfxIndex,
                        intLeft,
                        intTop,
                        intRight,
            ...
            case DRAW_PROGRESSBAR_TEXT_DRAW2:
                ...
                GFX_ColorSet( pPb->hdr.gfxIndex,
                              pPb->hdr.pGolScheme->TextColor0 );
    
                GFX_FontSet(pPb->hdr.gfxIndex, pPb->hdr.pGolScheme->pFont);
            ...
            case DRAW_PROGRESSBAR_TEXT_DRAW3:
                ...
                if (GFX_TextStringBoxDraw(pPb->hdr.gfxIndex,
