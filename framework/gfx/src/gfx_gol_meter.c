                                ...
                                GFX_GOL_OBJ_SCHEME      *pScheme)
{
    ...
    pMtr->hdr.gfxIndex  = gfxIndex;
    // unique id assigned for referencing
    pMtr->hdr.ID        = ID;
    // initialize pointer to NULL
    pMtr->hdr.pNxtObj   = NULL;                     
    ...
    GFX_GOL_ObjectAdd(pMtr->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pMtr);
...
void MtrCalcDimensions(GFX_GOL_METER *pMeter)
{
    ...
    /*
     Work around in case
     needle length is calculated negative.
     */
    if( pMeter->radius < 0 )
        pMeter->radius = 1;
...
*/
// *****************************************************************************
inline void GFX_GOL_MeterValueToCoordinatePosition(
                                GFX_GOL_METER *pMeter,
                                int16_t angle,
                                uint16_t radius,
                                int16_t *pX,
...
*/
// *****************************************************************************
inline void GFX_GOL_MeterStartEndGet(
                                GFX_GOL_METER *pMeter,
                                int16_t *pStart,
                                int16_t *pEnd)
{
...
*/
// *****************************************************************************
inline int16_t GFX_GOL_MeterAngleGet(
                                GFX_GOL_METER *pMeter,
                                int16_t start,
                                int16_t end)
{
...
*/
// *****************************************************************************
inline void GFX_GOL_MeterIncrement(
                                GFX_GOL_METER *pObject, 
                                uint16_t delta)
{
...
*/
// *****************************************************************************
inline void GFX_GOL_MeterDecrement(
                                GFX_GOL_METER *pObject,
                                uint16_t delta)
{
    GFX_GOL_MeterValueSet(pObject, pObject->value - delta);
...
*/
// *****************************************************************************
inline void GFX_GOL_MeterScaleColorsSet(
                                GFX_GOL_METER *pObject,
                                GFX_COLOR color1,
                                GFX_COLOR color2,
                                GFX_COLOR color3,
...
GFX_STATUS GFX_GOL_MeterDraw(void *pObject)
{
    ...
    // add one more space here for the NULL character
    static GFX_XCHAR            strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT + 1];
    static int16_t              degreeStart, degreeEnd;
    static int16_t              startX, startY;
#ifndef GFX_CONFIG_OBJECT_METER_DISPLAY_VALUES_DISABLE

    // NULL is pre-defined here
    static GFX_XCHAR            tempXchar[2] = {'8',0};

#endif
    static float                radian;
    static METER_INT32_UNION    dTemp;

    GFX_GOL_METER               *pMtr = (GFX_GOL_METER *)pObject;
    GFX_FILL_STYLE              fillStyle;
    uint8_t                     octant;
    uint16_t                    boxXStart, boxYStart, boxXEnd, boxYEnd;
    GFX_PREEMPTION_LEVEL        preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pMtr->hdr.gfxIndex);
    
    while(1)
    {

        if (GFX_RenderStatusGet(pMtr->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
            return (GFX_STATUS_FAILURE);

        switch(state)
        {
            // location of this case is strategic so the partial redraw of
            // the Meter will be faster
            case FRAME_DRAW:

                if(GFX_GOL_PanelDraw(pMtr->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
                {
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case IDLE:
                ...
                */
                ...
                if(GFX_GOL_ObjectStateGet(pMtr, GFX_GOL_METER_HIDE_STATE))
                {
                    ...
                    if (GFX_GOL_ObjectHideDraw(pMtr->hdr.gfxIndex, &(pMtr->hdr))
                            != GFX_STATUS_SUCCESS)
                        return (GFX_STATUS_FAILURE);
                ...
                GFX_LineStyleSet(pMtr->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);

                if(GFX_GOL_ObjectStateGet(pMtr, GFX_GOL_METER_DRAW_STATE))
                {
                    // set up the background
                    GFX_GOL_PanelBackgroundSet(pMtr->hdr.gfxIndex, &pMtr->hdr);

#if defined (GFX_CONFIG_ALPHABLEND_DISABLE) && \
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
            ...
            case TEXT_DRAW:
                ...
                GFX_ColorSet(pMtr->hdr.gfxIndex,
                             pMtr->hdr.pGolScheme->TextColor1);
                GFX_FontSet(pMtr->hdr.gfxIndex, pMtr->pTitleFont);
                ...
                {
                    case GFX_GOL_METER_QUARTER_TYPE:
                        startX = ((pMtr->hdr.right + pMtr->hdr.left) >> 1) -
                                  (temp >> 1);
            ...
            case TEXT_DRAW_RUN:
                ...
                if (GFX_TextStringDraw(pMtr->hdr.gfxIndex,
                        startX,
                        startY,
                        pMtr->pText,
                        0)
            ...
            case ARC_DRAW_SETUP:
                ...
                else
                {
                    ...
                    // set the arc radii: x1 smaller radius and x2 as the
                    // larger radius
                    x1 = pMtr->radius + 2;
            ...
            case ARC_DRAW:
                ...
                while(temp)
                {
                    ...
                    {
                        case 6:
                            GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->color1);
                            octant = 0x20;
                            break;
    
                        case 5:
                            GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->color2);
                            octant = 0x40;
                            break;
    
                        case 4:
                            GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->color3);
                            octant = 0x80;
                            break;
    
                        case 3:
                            GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->color4);
                            octant = 0x01;
                            break;
    
                        case 2:
                            GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->color5);
                            octant = 0x02;
                            break;
    
                        case 1:
                            GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->color6);
                            octant = 0x04;
                            break;
                    ...
                    if(GFX_ThickBevelDraw(pMtr->hdr.gfxIndex,
                                    pMtr->xCenter,
                                    pMtr->yCenter,
                                    pMtr->xCenter,
                                    pMtr->yCenter,
                ...
                GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->hdr.pGolScheme->Color1);
                i = degreeEnd;
                state = SCALE_COMPUTE;
            ...
            case SCALE_DRAW:
                ...
                // now draw the scales
                if(GFX_LineDraw(pMtr->hdr.gfxIndex, x1, y1, x2, y2)
                        != GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
                ...
                if((i % 45) == 0)
                {
                    ...
                    temp = (pMtr->maxValue - pMtr->minValue) * (radian /
                    ...
                    // get the ones value first and account for the required
                    // decimal point if enabled
                    if(GFX_GOL_ObjectStateGet(pMtr,
                       GFX_GOL_METER_ACCURACY_STATE))
                    {
                        ...
                        if((dTemp.int16Data[0]) >
                                (GFX_CONFIG_OBJECT_METER_RESOLUTION >> 1))
                            temp += ( GFX_CONFIG_OBJECT_METER_RESOLUTION -
                                      dTemp.int16Data[0]);
                        temp /= GFX_CONFIG_OBJECT_METER_RESOLUTION;
                    }
                    ...
                    do
                    {
                        strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT - j] =
                                                            (temp % 10) + '0';
                        if(((temp /= 10) == 0) ||
                                (j >= GFX_CONFIG_OBJECT_METER_SCALE_COUNT))
                            break;
                        j++;
                    } while(j <= GFX_CONFIG_OBJECT_METER_SCALE_COUNT);
    
                    // the (&strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT-j])
                    // removes the leading zeros.
                    // if leading zeroes will be printed change
                    // (&strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT-j])
                    // to simply strVal and remove the break statement above in
                    // the do-while loop
                    x2 = GFX_TextStringWidthGet( 
                            (&strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT - j]),
                            pMtr->hdr.pGolScheme->pFont);
            ...
            case NEEDLE_ERASE:
                ...
                if(GFX_GOL_ObjectStateGet(pMtr, GFX_GOL_METER_UPDATE_DRAW_STATE))
                {
                    ...
                    // to update the needle, redraw the old position with
                    // background color
                    GFX_ColorSet(pMtr->hdr.gfxIndex,
                                 pMtr->hdr.pGolScheme->Color0);
                    GFX_GOL_ObjectBackGroundSet(pMtr->hdr.gfxIndex, &pMtr->hdr);

#if defined (GFX_CONFIG_ALPHABLEND_DISABLE) && \
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
                    // object do not support gradient
                    GFX_FillStyleSet(pMtr->hdr.gfxIndex, GFX_FILL_STYLE_COLOR);

#else
                    GFX_FillStyleSet(pMtr->hdr.gfxIndex,
                                     pMtr->hdr.pGolScheme->fillStyle);
#endif
                    ...
                    if(GFX_RectangleFillDraw(pMtr->hdr.gfxIndex,
                            boxXStart, boxYStart,
            ...
            case NEEDLE_DRAW:
                ...
                if (GFX_RenderStatusGet(pMtr->hdr.gfxIndex)
                        == GFX_STATUS_BUSY_BIT)
                    return (GFX_STATUS_FAILURE);

                // At this point, pMtr->value is assumed to contain the new
                // value of the meter.
                ...
                GFX_LineStyleSet(pMtr->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
                if(GFX_LineDraw(pMtr->hdr.gfxIndex,
                        pMtr->xCenter,
                        pMtr->yCenter,
                        pMtr->xPos,
            ...
            case VALUE_ERASE:
                ...
                // display the value
                // erase previous value first. The temp>>1 accomodates fonts
                // with characters that has unequal widths
                temp = GFX_TextStringWidthGet( tempXchar, pMtr->pValueFont);
                temp = temp * GFX_CONFIG_OBJECT_METER_SCALE_COUNT + (temp >> 1);
    
                GFX_ColorSet(pMtr->hdr.gfxIndex, pMtr->hdr.pGolScheme->Color0);
                GFX_GOL_ObjectBackGroundSet(pMtr->hdr.gfxIndex, &pMtr->hdr);
    ...
    !defined (GFX_CONFIG_GRADIENT_DISABLE)
            ...
            case VALUE_DRAW:
                ...
                GFX_FontSet(pMtr->hdr.gfxIndex, pMtr->pValueFont);
                ...
                // get the ones value first and account for the required decimal
                // point if enabled
                if(GFX_GOL_ObjectStateGet(pMtr, GFX_GOL_METER_ACCURACY_STATE))
                {
                    strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT - j] = 
                            (((i % GFX_CONFIG_OBJECT_METER_RESOLUTION)) /
                            (GFX_CONFIG_OBJECT_METER_RESOLUTION / 10)) + '0';
                    i /= GFX_CONFIG_OBJECT_METER_RESOLUTION;
                    j++;
                    strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT - j] = '.';
                    j++;
                ...
                do
                {
                    strVal[GFX_CONFIG_OBJECT_METER_SCALE_COUNT - j] = (i % 10) +
                                                                        '0';
                    if(((i /= 10) == 0) ||
                            (j >= GFX_CONFIG_OBJECT_METER_SCALE_COUNT))
                        break;
