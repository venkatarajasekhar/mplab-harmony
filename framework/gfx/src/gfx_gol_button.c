...
)
{
	...
	// trim the radius amount
	if(pB->radius > (right - left) / 2)
		pB->radius = ((right - left) / 2) + 1;
	
	if(pB->radius > (bottom - top) / 2)
		pB->radius = ((bottom - top) / 2) + 1;
    ...
    GFX_GOL_ObjectAdd(pB->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pB);
    ...
    {
        GFX_GOL_ObjectFocusSet(pB->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pB);
    }
#endif
#endif
    return (pB);
...
*/
// *****************************************************************************
inline void GFX_GOL_ButtonTextAlignmentSet(
                                GFX_GOL_BUTTON *pObject,
                                GFX_ALIGNMENT align)
{
...
*/
// *****************************************************************************
inline GFX_ALIGNMENT GFX_GOL_ButtonTextAlignmentGet(
                                GFX_GOL_BUTTON *pObject)
{
...
*/
// *****************************************************************************
GFX_GOL_TRANSLATED_ACTION GFX_GOL_ButtonActionGet( void *pObject,
                                                   GFX_GOL_MESSAGE *pMessage )
{
    GFX_GOL_BUTTON *pB;
...
/*********************************************************************
* Function: inline uint16_t DrawButtonFocus(
                                            GFX_GOL_BUTTON *GFX_GOL_BUTTON,
                                            int16_t radius,
                                            BTN_DRAW_STATES *current_state )
  
********************************************************************/
inline GFX_STATUS GFX_GOL_ButtonFocusDraw(
                    GFX_GOL_BUTTON *pB,
                    int16_t radius,
                    DRAW_BUTTON_STATES *current_state)
{
    ...
    *current_state = DRAW_BUTTON_FOCUS;
    ...
    if (GFX_RenderStatusGet(pB->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        return (GFX_STATUS_FAILURE);

    if(GFX_GOL_ObjectStateGet(pB, GFX_GOL_BUTTON_FOCUSED_STATE))
    {
        GFX_LineStyleSet(pB->hdr.gfxIndex, GFX_LINE_STYLE_THIN_DASHED);
        if(GFX_GOL_ObjectStateGet(pB, GFX_GOL_BUTTON_PRESSED_STATE))
        {
            GFX_ColorSet(pB->hdr.gfxIndex, pB->hdr.pGolScheme->TextColor1);
        }
        else
        {
            GFX_ColorSet(pB->hdr.gfxIndex, pB->hdr.pGolScheme->TextColor0);
        }
        ...
        if(!pB->radius)
        {
            if(GFX_RectangleDraw(
                    pB->hdr.gfxIndex,
                        pB->hdr.left +
                            pB->hdr.pGolScheme->EmbossSize + 2,
                        pB->hdr.top +
                            pB->hdr.pGolScheme->EmbossSize + 2,
                        pB->hdr.right -
                            pB->hdr.pGolScheme->EmbossSize - 2,
        ...
        GFX_LineStyleSet(pB->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
    }
...
GFX_STATUS GFX_GOL_ButtonDraw(void *pObject)
{
    ...
    static uint16_t             radius; // todo used width, height;
    static GFX_COLOR            faceClr;
    static GFX_RESOURCE_HDR     *pImage;
           GFX_COLOR            embossLtClr, embossDkClr;
           GFX_GOL_BUTTON       *pB = (GFX_GOL_BUTTON *)pObject;
           GFX_PREEMPTION_LEVEL preemptionLevel =
                   GFX_GOL_PreemptionLevelGet(pB->hdr.gfxIndex);
    ...
    switch(state)
    {
        case DRAW_BUTTON_START:
            ...
            {                       
                ...
                if (GFX_GOL_ObjectHideDraw( pB->hdr.gfxIndex, &(pB->hdr)) !=
                                            GFX_STATUS_SUCCESS)
                    return (GFX_STATUS_FAILURE);
            ...
            */
            ...
            radius = pB->radius;
            // get width
            // - todo unused width = (pB->hdr.right - pB->hdr.left) - (radius * 2);
            // get height
            // - todo unused height = (pB->hdr.bottom - pB->hdr.top) - (radius * 2);
            ...
            if(!GFX_GOL_ObjectStateGet(pB, GFX_GOL_BUTTON_NOPANEL_STATE))
            {
                ...
                GFX_GOL_PanelBackgroundSet(pB->hdr.gfxIndex, &pB->hdr);
        ...
        case DRAW_BUTTON_BACKGROUND:
            ...
            if (pB->hdr.pGolScheme->pCommonBkImage != NULL)
            {
                ...
                {
                    GFX_TransparentColorEnable( pB->hdr.gfxIndex,
                                                transparentColorSet);
                }
#endif
            }
        ...
        case DRAW_BUTTON_ROUND:
	        ...
	        {
                    if(GFX_GOL_TwoTonePanelDraw( pB->hdr.gfxIndex ) !=
                                                 GFX_STATUS_SUCCESS )
                    {
                        return (GFX_STATUS_FAILURE);
                    }
	        ...
	        {
                    if(GFX_GOL_PanelDraw(pB->hdr.gfxIndex ) != 
                                                          GFX_STATUS_SUCCESS )
		    {
                        return (GFX_STATUS_FAILURE);
                    }
        ...
        case DRAW_BUTTON_TEXT:
            if(pB->pText != NULL)
            {
                if(!GFX_GOL_ObjectStateGet(pB, GFX_GOL_BUTTON_DISABLED_STATE))
                {
                    if(GFX_GOL_ObjectStateGet(pB, GFX_GOL_BUTTON_PRESSED_STATE))
                    {
                        GFX_ColorSet( pB->hdr.gfxIndex,
                                      pB->hdr.pGolScheme->TextColor1 );
                    }
                    else
                    {
                        GFX_ColorSet( pB->hdr.gfxIndex,
                                      pB->hdr.pGolScheme->TextColor0 );
                    }
                }
                else
                {
                    GFX_ColorSet( pB->hdr.gfxIndex,
                                  pB->hdr.pGolScheme->TextColorDisabled );
                }

                if (pB->hdr.pGolScheme->pFont != NULL)
                {
                    GFX_FontSet(pB->hdr.gfxIndex, pB->hdr.pGolScheme->pFont);
            ...
            {
                if(GFX_GOL_ButtonFocusDraw(pB, radius, &state) == 
                        GFX_STATUS_SUCCESS )
                {
#ifdef GFX_CONFIG_BISTABLE_DISPLAY_AUTO_REFRESH_ENABLE
                    GFX_DRIVER_CompleteDrawUpdate(  pB->hdr.left,
        ...
        case DRAW_BUTTON_TEXT_RUN:
            ...
            if (GFX_TextStringBoxDraw(pB->hdr.gfxIndex,
