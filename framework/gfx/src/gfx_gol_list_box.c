                                ...
                                GFX_GOL_OBJ_SCHEME  *pScheme)
{
    ...
    // draw function
    pLb->hdr.DrawObj    = GFX_GOL_ListBoxDraw;

    // free function
    pLb->hdr.FreeObj    = GFX_GOL_ListBoxItemListRemove;

    // action get function
    pLb->hdr.actionGet  = GFX_GOL_ListBoxActionGet;

    // default action function
    pLb->hdr.actionSet  = GFX_GOL_ListBoxActionSet;     
    ...
    // Add items if there's an initialization text 
    // ( each line will become one item, lines must be separated by '\n'
    // character )
    if(pText != NULL)
    {
        pointer = pText;
    ...
    GFX_GOL_ObjectAdd(pLb->hdr.gfxIndex, (GFX_GOL_OBJ_HEADER *)pLb);
...
*/
// *****************************************************************************
inline uint16_t GFX_GOL_ListBoxVisibleItemCountGet(
                                GFX_GOL_LISTBOX *pObject)
{
    uint16_t count;
...
GFX_STATUS GFX_GOL_ListBoxDraw(void *pObject)
{
    ...
    GFX_GOL_LISTBOX         *pLb = (GFX_GOL_LISTBOX *)pObject;
    GFX_XCHAR               *pChar;
    uint16_t                imageWidth;
    GFX_PREEMPTION_LEVEL    preemptionLevel =
                                GFX_GOL_PreemptionLevelGet(pLb->hdr.gfxIndex);
    
    /*
     Workaround for crash in case:
     text height > list box height + EmbossSize
    */
    if(( pLb->hdr.bottom - pLb->hdr.top ) <
            ( GFX_TextStringHeightGet( pLb->hdr.pGolScheme->pFont ) +
              pLb->hdr.pGolScheme->EmbossSize ) + 5 )
    {

        pLb->hdr.bottom = pLb->hdr.top +
                        GFX_TextStringHeightGet( pLb->hdr.pGolScheme->pFont ) +
                        pLb->hdr.pGolScheme->EmbossSize + 5;
    ...
    while(1)
    {
        ...
        if (GFX_RenderStatusGet(pLb->hdr.gfxIndex) == GFX_STATUS_BUSY_BIT)
        ...
        switch(state)
        {
            ...
            case LB_STATE_PANEL:
                ...
                if(GFX_GOL_PanelDraw(pLb->hdr.gfxIndex) != GFX_STATUS_SUCCESS)
		{
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case LB_STATE_START:
                ...
                if(GFX_GOL_ObjectStateGet(pLb, GFX_GOL_LISTBOX_HIDE_STATE))
                {
                    ...
                    if (GFX_GOL_ObjectHideDraw(pLb->hdr.gfxIndex, &(pLb->hdr))
                            != GFX_STATUS_SUCCESS)
                        return (GFX_STATUS_FAILURE);
                ...
                if(GFX_GOL_ObjectStateGet(pLb, GFX_GOL_LISTBOX_DRAW_STATE))
                {
                    ...
                    GFX_GOL_PanelBackgroundSet(pLb->hdr.gfxIndex, &pLb->hdr);
            ...
            case LB_STATE_DRAW_ITEMS:
                ...
                GFX_FontSet(pLb->hdr.gfxIndex, pLb->hdr.pGolScheme->pFont);
            ...
            case LB_STATE_SET_ERASEITEM:
                ...
                if(GFX_GOL_ObjectStateGet(
                        ...
                        GFX_GOL_LISTBOX_DISABLED_STATE))
                {
                    GFX_ColorSet( pLb->hdr.gfxIndex,
                                  pLb->hdr.pGolScheme->ColorDisabled);
                }
                else
                {
                    GFX_ColorSet( pLb->hdr.gfxIndex,
                                  pLb->hdr.pGolScheme->Color0);
                    if(pCurItem != NULL)
                        if( pCurItem->status &
                            GFX_GOL_LISTBOX_ITEM_STATUS_SELECTED)
                        {
                            GFX_ColorSet( pLb->hdr.gfxIndex,
                                          pLb->hdr.pGolScheme->Color1);
                        }
                }
                ...
                GFX_GOL_ObjectBackGroundSet(pLb->hdr.gfxIndex, &pLb->hdr);
            ...
            case LB_STATE_ERASEITEM:
                ...
                if(GFX_RectangleFillDraw(pLb->hdr.gfxIndex,
                        left,
                        tempTop,
                        right,
                        tempTop + entryHeight)
            ...
            case LB_STATE_SET_IMAGE_TEXT:
                ...
                {
                    GFX_ColorSet( pLb->hdr.gfxIndex,
                                  pLb->hdr.pGolScheme->TextColorDisabled);
                }
                else
                {
                    if(pCurItem->status & GFX_GOL_LISTBOX_ITEM_STATUS_SELECTED)
                    {
                        GFX_ColorSet( pLb->hdr.gfxIndex,
                                      pLb->hdr.pGolScheme->TextColor1);
                    }
                    else
                    {
                        GFX_ColorSet( pLb->hdr.gfxIndex,
                                      pLb->hdr.pGolScheme->TextColor0);
                    }
                }
            ...
            case LB_STATE_BITMAP:
                if(GFX_ImageDraw(pLb->hdr.gfxIndex,
                    left,
                    top + ((pLb->textHeight -
                           GFX_ImageHeightGet(
                        GFX_GOL_ListBoxItemImageGet(pCurItem))) >> 1),
            ...
            case LB_STATE_TEXT:
                ...
                {
                    if (GFX_TextStringBoxDraw(pLb->hdr.gfxIndex,
                            left + imageWidth,
                            top,
                            right - left,
                            entryHeight,
            ...
            case LB_STATE_SET_ITEMFOCUS:
                ...
                {
                    if (GFX_RenderStatusGet(pLb->hdr.gfxIndex)
                            == GFX_STATUS_BUSY_BIT)
                        return (GFX_STATUS_FAILURE);
                    ...
                    GFX_LineStyleSet(pLb->hdr.gfxIndex,
            ...
            case LB_STATE_ITEMFOCUS:
                ...
                if (top < bottom)
                {
                    if( GFX_RectangleDraw(pLb->hdr.gfxIndex,
                            left, 
                            tempTop,
                            right,
                ...
                GFX_LineStyleSet(pLb->hdr.gfxIndex, GFX_LINE_STYLE_THIN_SOLID);
