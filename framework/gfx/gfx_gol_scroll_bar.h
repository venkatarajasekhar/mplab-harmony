...
typedef struct
{
    ...
    /* User defined range of the slider. Minimum value at 0 and maximum at
     0x7FFF. */
    uint16_t            range;
    ...
    /* User specified resolution to incrementally change the position in range
     domain. */
    uint16_t            page;       

    /* Thumb width. This is computed internally. User must not change this
     value. */
    uint16_t            thWidth; 
...
*/
// *****************************************************************************
void GFX_GOL_ScrollBarPositionSet(GFX_GOL_SCROLLBAR *pObject,
                                  uint16_t position);
