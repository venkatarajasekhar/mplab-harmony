...
typedef enum
{
    ...
    /* This type is used to assign an "enter" carriage return) command on
     a key. */
...
typedef struct
{
    ...
    /* State of the key. Either Pressed (GFX_GOL_TEXTENTRY_KEY_PRESSED_STATE)
       or Released (0) */
    uint16_t        state;
    ...
    /* Command of the key. Either GFX_GOL_TEXTENTRY_DELETE_COM,
       GFX_GOL_TEXTENTRY_SPACE_COM or GFX_GOL_TEXTENTRY_ENTER_COM.*/
    GFX_GOL_TEXTENTRY_KEY_COMMAND_TYPE  command; 
                                

    /* Pointer to the custom text assigned to the key. This is displayed
       over the face of the key. */
    GFX_XCHAR       *pKeyName;  

    /* Computed text width, done at creation. Used to predict size and position
       of text on the key face. */
    int16_t         textWidth;  

    /* Computed text height, done at creation. Used to predict size and
...
typedef struct
{
    ...
    /* Pointer to the buffer assigned by the user which holds the text shown in
       the edit box.*/
    GFX_XCHAR                       *pTeOutput;
    ...
    /* Current length of the string in the buffer. The maximum value of this is
       equal to outputLenMax. User creates and manages the buffer. Buffer can
       also be managed using the APIs provided to add a character, delete the
       last character or clear the buffer. */
    uint16_t                        CurrentLength;  

    /* Maximum expected length of output buffer. Object will update this
       parameter when adding, removing characters or clearing the buffer and
       switching buffers. */
    uint16_t                        outputLenMax;   

    /* Pointer to the active key. This is only used by the object. User must
       not change the value of this parameter directly.*/
...
/*  GFX GOL text entry key list create.
    ...
    Returns:
        ...
        A NULL is returned if the list is not created successfully.
