...
typedef enum
{
    ...
    /* Property bit when set, the values will have accuracy for 1 decimal
       place. */
...
///***************************************************************************
// Overview: This is a Meter compile time option to disable the display
//           of the values. 
...
/*************************************************************************** 
 Overview: This is a Meter compile time option to define how many characters 
           ...
           Example: 
           if maxValue-minValue = 500, GFX_CONFIG_OBJECT_METER_SCALE_COUNT
           should be 3.
           if maxValue-minValue = 90, GFX_CONFIG_OBJECT_METER_SCALE_COUNT = 2
           You must include the decimal point if this feature is enabled 
           (see GFX_GOL_METER_ACCURACY_STATE state bit).
***************************************************************************/
...
/*************************************************************************** 
 Overview: This is a Meter compile time option to define the factor 
           ...
           and current value. Used only when GFX_GOL_METER_ACCURACY_STATE 
           state bit is set.
***************************************************************************/
...
typedef struct
{
    ...
    /* The following three points define three fonts used in meter widget, 
       they can be different from the scheme font. Note that the sizes of these
       fonts are not checked with the meter dimension. In cases where font sizes
       are larger than the meter dimension, some overlaps will occur. */

    /* Pointer to the font used in the title of the meter */
    GFX_RESOURCE_HDR    *pTitleFont;

    /* Pointer to the font used in the current reading (if displayed) of the
       meter */
