...
extern __inline__ uint32_t  __attribute__((always_inline)) _TCPIPStackNetAddress(TCPIP_NET_IF* pNetIf)
{
...
extern __inline__ bool  __attribute__((always_inline)) TCPIP_STACK_AddressIsOfNet(TCPIP_NET_IF* pNetIf, const IPV4_ADDR* pIpAdd)
{
...
extern __inline__ const uint8_t*  __attribute__((always_inline)) _TCPIPStack_NetMACAddressGet(TCPIP_NET_IF* pNetIf)
{
...
extern __inline__ TCPIP_MAC_RES __attribute__((always_inline)) _TCPIPStackPacketTx(TCPIP_NET_IF* pNetIf, TCPIP_MAC_PACKET * ptrPacket)
{
    ptrPacket->pktIf = pNetIf; 
    return (*pNetIf->pMacObj->TCPIP_MAC_PacketTx)(pNetIf->hIfMac, ptrPacket);
}
