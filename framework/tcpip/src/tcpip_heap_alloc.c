...
// extern functions needed when not inlined
void* TCPIP_HEAP_Malloc(TCPIP_STACK_HEAP_HANDLE h, size_t nBytes)
{
    return (*((TCPIP_HEAP_OBJECT*)h)->TCPIP_HEAP_Malloc)(h, nBytes);
}

void* TCPIP_HEAP_Calloc(TCPIP_STACK_HEAP_HANDLE h, size_t nElems, size_t elemSize)
{
    return (*((TCPIP_HEAP_OBJECT*)h)->TCPIP_HEAP_Calloc)(h, nElems, elemSize);
}

size_t TCPIP_HEAP_Free(TCPIP_STACK_HEAP_HANDLE h, const void* ptr)
{
