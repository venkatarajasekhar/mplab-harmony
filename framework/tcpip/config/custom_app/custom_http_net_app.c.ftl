...
TCPIP_HTTP_DYN_PRINT_RES TCPIP_HTTP_Print_name(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const TCPIP_HTTP_DYN_VAR_DCPT *vDcpt)
{
    if (s_scanResultIsValid)
    {
        ...
        {
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)s_scanResult->ssid, false);
        }
        else
        {
            uint8_t ssidLongString[33];
            memcpy(ssidLongString, s_scanResult->ssid, 32); // in this case, SSID can only be 32-byte long
            ssidLongString[32] = 0;
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)ssidLongString, false);
        }
    }
    else
    {
...
{
    TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)g_redirectionConfig.prevSSID, false);
    return TCPIP_HTTP_DYN_PRINT_RES_DONE;
}
...
{
    TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)g_redirectionConfig.ssid, false);
    return TCPIP_HTTP_DYN_PRINT_RES_DONE;
}
...
TCPIP_HTTP_DYN_PRINT_RES TCPIP_HTTP_Print_prevWLAN(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const TCPIP_HTTP_DYN_VAR_DCPT *vDcpt)
{
    if (g_redirectionConfig.prevNetworkType == WF_NETWORK_TYPE_INFRASTRUCTURE)
        ...
        TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"Soft AP (BSS)", false);
    else
...
TCPIP_HTTP_DYN_PRINT_RES TCPIP_HTTP_Print_currPrivacy(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const TCPIP_HTTP_DYN_VAR_DCPT *vDcpt)
{
    s_httpapp_get_param.config.data = &g_wifi_cfg;
    iwpriv_get(CONFIG_GET, &s_httpapp_get_param);
    switch (g_wifi_cfg.securityMode)
    {
        case DRV_WIFI_SECURITY_OPEN:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"None", false);
            break;
        case DRV_WIFI_SECURITY_WEP_40:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WEP", false);
            break;
        case DRV_WIFI_SECURITY_WEP_104:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WEP", false);
            break;
        case DRV_WIFI_SECURITY_WPA_WITH_KEY:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPA-PSK", false);
            break;
        case DRV_WIFI_SECURITY_WPA_WITH_PASS_PHRASE:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPA-PSK", false);
            break;
        case DRV_WIFI_SECURITY_WPA2_WITH_KEY:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPA2-PSK", false);
            break;
        case DRV_WIFI_SECURITY_WPA2_WITH_PASS_PHRASE:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPA2-PSK", false);
            break;
        case DRV_WIFI_SECURITY_WPA_AUTO_WITH_KEY:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPA-PSK/WPA2-PSK AUTO", false);
            break;
        case DRV_WIFI_SECURITY_WPA_AUTO_WITH_PASS_PHRASE:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPA-PSK/WPA2-PSK AUTO", false);
            break;
        case DRV_WIFI_SECURITY_WPS_PUSH_BUTTON:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPS PBC", false);
            break;
        case DRV_WIFI_SECURITY_WPS_PIN:
            TCPIP_HTTP_NET_DynamicWriteString(vDcpt, (const char *)"WPS PIN", false);
            break;
        default:
...
TCPIP_HTTP_DYN_PRINT_RES TCPIP_HTTP_Print_ipaddr(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const TCPIP_HTTP_DYN_VAR_DCPT *vDcpt)
{
    IPV4_ADDR ipAddress;
    TCPIP_NET_HANDLE hNet;
    char *ipAddStr;
    ...
    if (pDynBuffer == 0)
        return TCPIP_HTTP_DYN_PRINT_RES_AGAIN; // failed to get a buffer; retry
