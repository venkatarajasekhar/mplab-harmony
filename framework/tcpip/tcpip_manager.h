...
/*
   ...
   Description:     
    This function sets the types of masks used in the stack decision of
    ...
    To decide if a destination IP address (destAdd) is local to
    a network interface (having the netAdd address) the following calculation is performed:
    if( ((destAdd & andMask) | orMask) == ((netAdd & andMask) | orMask)), the destination 
	address is considered to be a local address; otherwise, the destination address is non-local.
  ...
  <code>
   ...
   Remarks:         
    The default value for both AND and OR masks is set to TCPIP_LOCAL_MASK_ZERO 
	so that any destination network is considered to be local!
  
 */
bool                TCPIP_STACK_SetLocalMasksType(TCPIP_NET_HANDLE netH, 
             TCPIP_LOCAL_MASK_TYPE andType, TCPIP_LOCAL_MASK_TYPE orType);
...
/*
   ...
   Description:     
    This function sets the masks used in the stack decision of
    a destination address being a local or non-local address.
