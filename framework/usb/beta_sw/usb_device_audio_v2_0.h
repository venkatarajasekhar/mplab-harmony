...
#define USB_DEVICE_AUDIO_TRANSFER_HANDLE_INVALID  /*DOM-IGNORE-END*/((USB_DEVICE_AUDIO_TRANSFER_HANDLE)(-1))/*DOM-IGNORE-END*/
...
/* USB Device Audio Function Driver Events
    ...
    )
    {
        switch (event)
        {
            ...
            case USB_DEVICE_AUDIO_EVENT_CONTROL_SET_CUR:
                ...
                // application should indentify the entity type based on the
  ...
  Remarks:
    ...
    response for a finite time duration before timing out and cancelling the
...
typedef enum
{
    /* This event occurs when a write operation scheduled by calling the
       ...
       USB_DEVICE_AUDIO_EVENT_WRITE_COMPLETE_DATA type. */
    ...
    /* This event occurs when a read operation scheduled by calling  the
       ...
       the event handler will point to USB_DEVICE_AUDIO_EVENT_READ_COMPLETE_DATA
       type.  */
    ...
    /* This event occurs when the Audio function driver receives a control
       ...
       /* This event occurs when the Audio function driver receives a class
       control transfer request. The host may send such a request to modify or
       request the status of audio function entity such as a unit or a terminal.
       The pData parameter will point to the 8-byte Setup packet received in the
       control transfer request. The application must analyze this Setup packet
       and use the USB_DEVICE_ControlSend() or
       USB_DEVICE_ControlReceive() or the USB_DEVICE_ControlStatus()
...
} USB_DEVICE_AUDIO_EVENT_DATA_SET_ALTERNATE_INTERFACE;
...
/* Function:
  ...
  Description:
    ...
    USB_DEVICE_AUDIO_Write() function or the
    USB_DEVICE_AUDIO_SerialStateNotificationSend() functions. If a transfer is
    still in the queue and its processing has not started, then the transfer is
    cancelled completely. A transfer that is in progress may or may not get
    cancelled depending on the transaction that is presently in progress. If the
    last transaction of the transfer is in progress, then the transfer will not
    be cancelled.  If it is not the last transaction in progress, the
    in-progress will be allowed to complete. Pending transactions will be
    cancelled. The first transaction of an in progress transfer cannot be
    cancelled.
  ...
  Parameters:
    ...
    transferHandle - Transfer handle of the transfer to be cancelled.

  Returns:
    USB_DEVICE_AUDIO_RESULT_OK    - The transfer will be cancelled completely or
                              partially.

    USB_DEVICE_AUDIO_RESULT_ERROR - The transfer could not be cancelled because it has
