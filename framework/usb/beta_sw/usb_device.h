...
typedef struct 
{
    ...
    /* If true, USB module will stop when CPU enters
       Idle Mode */
    bool stopInIdle;     

    /* If true, USB module will suspend when the microcontroller
       enters sleep mode.*/
    bool suspendInSleep; 
    ...
    /* Pointer to an byte array whose size is USB_DEVICE_ENDPOINT_TABLE_SIZE
       and who start address is aligned at a 512 bytes address boundary. */
    void * endpointTable;
    ...
    /* Size of Endpoint read queue for this instance of USB Device  */
    uint16_t queueSizeEndpointRead;

    /* Size of Endpoint write queue for this instance of USB Device  */
...
/* Function:
  ...
  Example:
    ...
    // a Device Layer Event Handler. It is assumed that device layer
    ...
    // We will not use the context parameter in this example.
...
// *****************************************************************************
/* USB Device Layer Endpoint Table Size

  Summary:
    Size of the Device Layer Endpoint Table.

  Description:
    This is the size of the device layer endpoint table. The application should
    allocate a byte array of this array of this size. The array should be
    aligned on a 512 byte address boundary. The pointer to this array is passed
    in device layer initialization data structure.
...
#define USB_DEVICE_ENDPOINT_TABLE_SIZE /*DOM-IGNORE-BEGIN*/(DRV_USB_ENDPOINTS_NUMBER * DRV_USB_ENDPOINT_TABLE_ENTRY_SIZE)/*DOM-IGNORE-END*/
...
/* Function:
  ...
  Returns:
    Current USB Device Layer version in numerical format.

  Example:
    <code>
    unsigned int version;

    version = USB_DEVICE_VersionGet();
...
/* Function:
  ...
  Returns:
    Current USB Device Layer version in the string format.

  Example:
    <code>
	//Allocate memory in application's space
    char version[10];

    version = USB_DEVICE_VersionStrGet ( );
